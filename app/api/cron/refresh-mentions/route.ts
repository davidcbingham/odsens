/**
 * `/api/cron/refresh-mentions` — thin wrapper `route → lib/jobs/refreshMentions()` (04 §2.4; SC-12;
 * SC-13 via the job; SC-16 no-key skip in the job; 01 INV-41 no-store; ADR-0002 C15 maxDuration 300;
 * 02 §1.4 / §2.10; 00 S1.8 AC7; 05 T-ACT-33, T-ACT-71; vercel.json schedule `37 * * * *`, 04 §6).
 *
 ...
 * no side effects (no `sync_runs` row, no upstream request, no `mentions` write). No query
 * parameter is read — `?full=1` belongs to `/api/cron/sync-youtube` alone (04 §2.4).
 * Success → 200 `JobSummary` (an SC-13 lock skip `{ok:true, skipped:'running'}` and the no-key run
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const summary = await refreshMentions({ trigger: 'cron' });
    if (!summary.ok) {
