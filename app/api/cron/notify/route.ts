/**
 * `/api/cron/notify` — thin wrapper `route → lib/jobs/notify.ts runNotify()` (04 §2.4 notify row;
 * SC-12; SC-13 via the job; 02 §1.4 + RP-17 (`maxDuration` 60 — ADR-0002 C15); 01 INV-41 no-store;
 * ADR-0030 D2; 05 T-ACT-33, T-ACT-70; vercel.json schedule `*\/5 * * * *`, 04 §6).
 *
 ...
 * `JobSummary` (`items` = delivered count; a lock skip is `{ok:true, skipped:'running'}`, still
 * 200). Failure → 500 `{ok:false, source, run_id, error:{code:'job_failed', message}}` — already
 * recorded in `sync_runs` by the job (SC-11). `triggerSync` never runs `notify` (04 §1.7).
 */
