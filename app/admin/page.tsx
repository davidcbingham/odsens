...
/**
 * `/admin` — the dashboard, S1.4 state (02 §1.3 `/admin` row: Slice "S1.1 gate; S1.2
 ...
 * real answer. Sync sources = modrinth + curseforge only at S1.2 (03 §2.10 `SyncStatus` Slice
 * cell "S1.2 (Modrinth/CF) · S1.6 (YouTube) · S1.8" — the jobs the row can trigger today);
 * `canTrigger` = role admin (ADR-0002 C7; moderators see "Sync now" disabled, never absent —
 * 02 §1.3).
 *
...
export default async function AdminPage() {
  ...
  const [heldCount, draftCount, syncSources] = await Promise.all([
    countHeldComments(),
    countDraftProjects(),
