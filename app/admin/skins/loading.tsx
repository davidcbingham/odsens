...
/**
 * `/admin/skins` loading state (02 §1.3 Files cell `app/admin/skins/loading.tsx`; RP-10; 03 G-01):
 * the page's real rhythm so nothing jumps — eyebrow + intro lines, the "Add a skin" slab (two
 * field rows, the well, the button), the ORDER rows, then the admin-table shell (header row + four
 * ≥44px rows) — DESIGN.md §11.1 Skeleton. Region carries `aria-busy` with one visually-hidden
 * "Loading…".
 */
export default function AdminSkinsLoading() {
  return (
    <div className={styles.loading} role="region" aria-busy="true" aria-label="Loading">
      ...
      <div className={styles['loading-section']}>
        <Skeleton kind="text" width="140px" height="22px" />
        <div className={styles['loading-panel']}>
          <div className={styles['loading-panel-row']}>
        ...
        <div className={styles['loading-order']}>
          {Array.from({ length: 2 }, (_, i) => (
            <div key={i} className={styles['loading-order-row']}>
              <Skeleton kind="media" width="40px" height="40px" />
