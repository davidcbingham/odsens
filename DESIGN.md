...
> v1.10 (2026-09-12): S1.5c Project editor v2 (ADR-0039) — §11.3 #20 added: the **section sidebar** (the §6 #9 admin recipe; a scrolling chip row on phone), the **unsaved dot** on the active section, the leave `Dialog` on a `--scrim` backdrop, and the **Markdown toolbar + Preview pane** for the Description / Notes fields; one new derived token, `--scrim` (`--ink` at .6 — listed in 03 §9). No other visual change. There is no pass-4 mockup — the recipes are pinned by ADR-0039 D5.
