...
> v1.2a (2026-08-17): decisions — supporters wall shows handle + amount; handle validation is structural only (no real-name detection state). 
...
7. **Support** — gold hatched panel with $1 / $3 / $5 / Other (one preselected) and a single send button, so tipping is one or two clicks; Ko-fi handles the payment; "What it pays for" slab in plain copy; reserved dashed slot for the future supporters wall / leaderboard (**handle + amount**; unlinked tips as "Anonymous · $X" — decided 2026-08-17, supersedes the earlier "no amounts" note).
...
## 12. Addenda (2026-08-17) — to be designed in code from existing components (or a Claude Design pass 3 if Oliver wants to art-direct)

**12.1 Seen on (v1).** Project detail gets a **SEEN ON** row (section title in gold Bungee): mention cards = video click-to-load facade (§11) + a creator line (creator name 14px 700, platform glyph in a neutral slab using the official mark, view count in Silkscreen emerald, date in mute-dim). Click opens the embed inline (facade behaviour) with an "on YouTube ↗" ghost link; non-YouTube platforms link out. Home gets an **IN THE WILD** strip: 3–4 featured mentions + a reach line in Silkscreen ("1.2M VIEWS · 6 VIDEOS · 4 CREATORS"). No empty state shown when there are no mentions. Voice: "Seen on", "In the wild" — the numbers brag, the copy doesn't. Footer adds one dry line: "Creators featuring the mods aren't affiliated with odsens."

**12.1b Admin Settings — Notifications matrix (v1).** Replaces the four email switches from pass 2 with a **grid**: rows = events (New comment · Held for review · Reported · Sync failed · [P2: New order · New tip]), columns = channels (Email · Discord), each cell the square worded ON/OFF toggle. Above the grid: Discord webhook URL field (masked, "Test" secondary button that sends a sample), and Admin emails list (add/remove chips). Row labels 14px 700, column headers Silkscreen. Phone: rows stack, toggles inline.

**12.2 Workrooms (Phase 2).** A workroom page = the project-detail layout with a **PRIVATE** badge (gold-wash, exclusive-badge construction, "🔒 PRIVATE" → use the lock glyph from the icon set, not emoji), the GET IT rail replaced by **STATUS** (milestone pill set: BRIEF · QUOTE · IN PROGRESS · REVIEW · DELIVERED · CLOSED, worded, one accent), **FILES** (files table + upload well with a client-upload variant showing limits), and **PARTICIPANTS** (avatar + handle + role tag; the moderator row reads "MOD · here to keep things safe"). Body = Oliver's posts (markdown + gallery). Comments = the standard thread scoped to members. Header line, honest: "Private workroom. Payment happens on Ko-fi. No contracts, no invoices yet." Admin: "Orders & Workrooms" list with unread markers.
