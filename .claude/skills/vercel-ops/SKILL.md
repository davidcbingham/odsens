...
## Environments
- **Preview**: every branch push; uses the persistent **`staging` Supabase branch** (git branch `staging`, ADR-0010) — the Preview environment carries staging's `NEXT_PUBLIC_SUPABASE_URL` + the legacy key pair `NEXT_PUBLIC_SUPABASE_ANON_KEY` / `SUPABASE_SERVICE_ROLE_KEY` (Production carries both pairs; `lib/env.ts` accepts the new names `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` / `SUPABASE_SECRET_KEY` as aliases — either pair is ADR-0010-valid, names verified by `deploy-checker` 2026-09-11); no branch-scoped Supabase vars (the S0 `feat/S0-scaffold` placeholders are removed); `NEXT_PUBLIC_SITE_URL` is **derived** in code from `VERCEL_BRANCH_URL` (needs "Enable access to System Environment Variables" on) — never set it per branch (ADR-0010). `deploy-checker` fetches protected previews with the `x-vercel-protection-bypass` header (`VERCEL_AUTOMATION_BYPASS_SECRET`, tooling-only).
