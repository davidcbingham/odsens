...
- **Owner bootstrap (one-time per environment — staging, then production; 00-O-10 / ADR-0002 #23):** `oddsense` is on the reserved-handle list so nobody can take it through onboarding — the owner claims it by SQL, not by the form. After Oliver's first Google sign-in on that environment (his `profiles` row exists with `handle NULL`), David runs in that environment's Supabase SQL editor (Dashboard → SQL; on staging pick the `staging` branch): `update public.profiles set handle = 'oddsense', role = 'admin' where id = (select id from auth.users where lower(email) = lower('<Oliver\'s Google address>')) and (handle is null or handle = 'oddsense');` → Oliver reloads and lands on the home page signed in as `oddsense` with the Admin menu item. Promoting a second admin (David): same statement without the `handle` assignment (`set role = 'admin'`). The SQL editor runs as `postgres`, so `profiles_guard` and the reserved list do not apply; the e-mail address stays in the dashboard — never in code, docs or chat logs beyond this step.
