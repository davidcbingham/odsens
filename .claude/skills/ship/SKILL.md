...
- Deployment Protection = Standard: previews and `*.vercel.app` URLs need a Vercel login to view — but **production is public on www.odsens.com** (the attached custom domain is outside Deployment Protection; live since the S1.1 merge, 2026-08-21). `deploy-checker` uses "Protection Bypass for Automation" for previews.
...
8. **Merge.** `gh pr merge --squash --delete-branch`. `main` auto-deploys to production — **public immediately on www.odsens.com** (only `*.vercel.app` URLs and previews sit behind Deployment Protection).
