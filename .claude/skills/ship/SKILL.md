...
7. **Gates — one background batch.** Spawn `spec-drift-reviewer` (always) + `design-fidelity-reviewer`, `frontend-reviewer`, `security-reviewer`, `backend-reviewer`, `supabase-reviewer` — all seven run on every v1 slice (00 §1.7; a gate with nothing in scope returns PASS) — then `deploy-checker` on the preview URL. Paste each `GATE:` block into the PR body verbatim. A ❌ goes back to the owner once; a second ❌ on the same item → stop and ask David. Nothing merges with an open ❌.
