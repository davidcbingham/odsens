...
- Deployment Protection = Standard: previews and `*.vercel.app` URLs need a Vercel login to view — but **production is public on odsens.com** (apex; `www.odsens.com` 308 → apex since 2026-09-06 — the attached custom domain is outside Deployment Protection; live since the S1.1 merge, 2026-08-21). `deploy-checker` uses "Protection Bypass for Automation" for previews.
...
- Required CI checks on `main`: `lint`, `unit`, `db`, `build`, `e2e` (05 CI-8). Tags per 00 §1.4: S0 `v0.1`, S1.1 `v0.2` … S1.9 `v0.10`, S1.10 `v1.0.0`; an inserted slice (`S1.5a`, `S1.5b` — ADR-0036) tags `v0.<n>.<k>` (`v0.6.1`, `v0.6.2`); Phase 2 `v1.<n>`. Fix branches get no tag.
...
8. **Merge.** `gh pr merge --squash --delete-branch`. `main` auto-deploys to production — **public immediately on odsens.com** (only `*.vercel.app` URLs and previews sit behind Deployment Protection).
9. **Tag** (slice merges only, 00 §1.4): `git switch main && git pull`, then `git tag -a v0.<n> -m "S1.x <name>"` and `git push origin v0.<n>`.
10. **Verify production.** Since ADR-0029 the merge build waits for the Supabase integration to apply the slice's migrations before `next build` runs (`scripts/wait-for-schema.mjs` — look for `wait-for-schema: ready` in the Vercel build log); if that wait times out (20 min) the build fails with a line naming the missing migrations/relations — wait for `supabase migration list --linked` to catch up, then ONE `vercel redeploy <deployment-url>` (no `--yes` flag) is the fallback. Then verify (`https://odsens.com` from S1.10; the production Vercel URL before that): `/`, `/projects`, one project detail (from S1.2), the sign-in round-trip start (from S1.1), headers via `curl -sI` (CSP, `X-Frame-Options: DENY`, `Strict-Transport-Security`), and each cron route in the 04 §6 table using `CRON_SECRET` from Vercel — never paste the value.
