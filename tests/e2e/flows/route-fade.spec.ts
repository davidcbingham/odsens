/**
 * T-E2E-50 — route-change fade + avatar outline (ADR-0035 D2/D3; DESIGN.md §3, §8 v1.8).
 * The nav avatar's border is 2px; navigating `/` → `/projects` through the nav remounts the
...
test.describe('route fade + avatar outline (T-E2E-50)', () => {
  test('T-E2E-50 avatar border is 2px; the page wrapper fades on navigation; nav is untouched', async ({
    ...
    const border = await avatar.evaluate(
      (el) => getComputedStyle(el.parentElement as HTMLElement).borderTopWidth,
    );
