...
test.describe('http smoke', () => {
  ...
  test('T-E2E-46 cron routes (S1.2, S1.6, S1.8): no/wrong header → 401 JSON, no side effects; POST/HEAD → 405', async ({
    request,
  }) => {
    for (const path of [
      ...
      '/api/cron/refresh-mentions', // S1.8 (04 §2.4; same SC-12 wrapper)
    ]) {
