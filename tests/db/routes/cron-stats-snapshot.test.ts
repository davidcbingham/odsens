/**
 * tests/db/routes/cron-stats-snapshot.test.ts — T-ACT-33 for `/api/cron/stats-snapshot` + the route
 * side of T-ACT-70 (04 SC-12, §2.4; 02 §1.4 / §2.10; ADR-0002 C15; 00 S1.9 AC1 "401 without secret";
 * 05 §7.2; ADR-0049 D8). Handlers are imported from the route file and invoked directly (the route reads
 * no cookies). `mutatesSeed`: every authorized run adds a `sync_runs` row and rewrites today's
 * `stats_daily` rows (the SEED-12 pair among them), so the file snapshots/restores the content
 * tables (`stats_daily` + `sync_runs` included — 05 H-1).
 *
 * `spyFetch` key = the real request prefix `${YOUTUBE_API_BASE}/channels` — ALWAYS routed, the 401
 * rows included: the base is a loopback host, and an unrouted loopback URL passes through `spyFetch`
 * to the real network instead of throwing. The fixture `youtube/channels.json` answers the seed
 * channel (views 1,284,530 · subs 21,400).
 *
 * The 500 row forces the job's first read (`projects`) to fail through `withDbFault`: a gather
 * failure → the job returns `ok=false` with zero writes and the route wraps it as `job_failed` while
 * the run row is still finalized (SC-11). The guard rows mock the job and assert the options it
 * received — this route reads no query parameter (`?full=1` is `/api/cron/sync-youtube`'s alone,
 * 04 §2.4).
 */
import { NextRequest } from 'next/server';
import { afterAll, afterEach, beforeAll, describe, expect, it, vi } from 'vitest';
import * as route from '@/app/api/cron/stats-snapshot/route';
...
vi.mock('@/lib/jobs/snapshotStats', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@/lib/jobs/snapshotStats')>();
  const snapshotStats: typeof actual.snapshotStats = (opts) =>
    jobOverride.run ? jobOverride.run(opts) : actual.snapshotStats(opts);
  return { ...actual, snapshotStats };
});
...
const ROUTE_URL = 'http://localhost:3000/api/cron/stats-snapshot';
...
/** Every `stats_daily` row with its `updated_at` — equality proves "nothing written" (≤ 1,000 rows). */
...
describe('T-ACT-33 /api/cron/stats-snapshot', () => {
  it('T-ACT-33 no Authorization header → 401 exact JSON, no sync_runs row, no upstream request, no stats_daily write (00 S1.9 AC1)', async () => {
    const runsBefore = await syncRunCount();
    const rowsBefore = await statsRows();
    ...
    expect(await statsRows()).toEqual(rowsBefore);
  ...
  it('T-ACT-33 correct CRON_SECRET → 200 JSON JobSummary {ok, source, run_id, items, ms, day, rows, purged, orphans, units, errors}; the run wrote today’s rows and its sync_runs row', async () => {
    const runsBefore = await syncRunCount();
    const rowsBefore = await statsRows();
    const fetchSpy = spyFetch(FIXTURE_OK);
    const tags = spyRevalidateTag();
    ...
    expect(body.items).toBeGreaterThan(10);
    ...
    expect(body).toMatchObject({
      units: 1,
      errors: [],
      rows: { site: 10, channel: 2 },
      orphans: { capped: false },
    });
    expect(body.day).toMatch(/^\d{4}-\d{2}-\d{2}$/);

    // Today's rows exist now (more rows than before — the seed pair alone was there).
    const rowsAfter = await statsRows();
    expect(rowsAfter.filter((row) => row.day === body.day).length).toBe(body.items);
    ...
    expect(tags.calls).toEqual([]); // 04 §3.5 revalidates nothing
    ...
    const { data } = await service
      ...
      .eq('id', body.run_id)
      .single();
    expect(data?.source).toBe('stats');
    expect(data?.finished_at).not.toBeNull();
    expect(data?.ok).toBe(true);
    expect(data?.items).toBe(body.items);
    expect(data?.error).toBeNull();
  });

  it('T-ACT-33 a second authorized tick is idempotent → 200, the same items, the same row count for the day, nothing revalidated (00 S1.9 AC1 "twice = same rows")', async () => {
    const rowsBefore = await statsRows();
    spyFetch(FIXTURE_OK);
    const tags = spyRevalidateTag();
    const res = await route.GET(authorized());
    ...
    expect(rowsAfter.filter((row) => row.day === body.day)).toHaveLength(body.items);
    expect(tags.calls).toEqual([]);
  });
  ...
  it('T-ACT-33 job failure (a failed projects read) → 500 {ok:false, source, run_id, error:{code:job_failed}}; sync_runs finalized ok=false; no stats_daily row changed', async () => {
    const rowsBefore = await statsRows();
    spyFetch(FIXTURE_OK);
    ...
    expect(body.error.message).toMatch(/^projects read failed/);
    ...
    expect(data?.error).toMatch(/^projects read failed/);
    expect(await statsRows()).toEqual(rowsBefore);
  });

  it("T-ACT-70 an open stats run 5 min old → 200 {ok:true, skipped:'running'}, no second row, no upstream request, no write", async () => {
...
// run_id '', one log line)
// ---------------------------------------------------------------------------------------------
describe('T-ACT-33 /api/cron/stats-snapshot route guards (job mocked)', () => {
  afterEach(() => {
  ...
  it.each(['', '?full=1', '?full=true'])(
    ...
    async (query) => {
      ...
      jobOverride.run = async (opts) => {
        received.push(opts);
        return { ok: true, source: 'stats', run_id: 't_run', items: 0, ms: 1 };
      };
  ...
  it.each<{ name: string; thrown: unknown; logged: string }>([
    ...
    async ({ thrown, logged }) => {
      ...
      expect(lines[0]?.job).toBe('snapshotStats');
