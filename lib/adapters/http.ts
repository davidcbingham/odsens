 ...
 *   truncated to 300 chars (A4).
 * - Every request carries `User-Agent` = the caller's `ua` (= `env.MODRINTH_USER_AGENT`, SC-10 —
 *   also sent to CurseForge/YouTube/OG fetches) and `Accept: application/json`.
 * - `fetch` is injectable: factories pass theirs down (SC-25) and unit tests use `mockFetch` (05 H-5).
 *   `onResponse` lets the Modrinth adapter watch quota headers (04 §4.1) without a second HTTP path.
 ...
 * `not_found`/`rejected`/`unsupported` are thrown by later-slice adapters (04 §4.3–§4.6,
...
export type FetchJsonOptions = {
  ...
  ua: string;
  /** Extra request headers (e.g. CurseForge `x-api-key`). Never echoed into errors. */
  headers?: Record<string, string>;
  /** Injectable transport (SC-25 / 05 H-5). Defaults to the global fetch. */
  fetch?: typeof fetch;
  /** Observes every received response (any status) — Modrinth quota headers (04 §4.1). */
...
/**
 * The SC-09 HTTP call. GET only in v1 (every §4 adapter row this slice needs is a GET); parses the
 * response as JSON. Throws `AdapterError` — callers map it to `upstream_error` (actions) or
 * `summary.errors[]` / `sync_runs.error` (jobs) per 04 §7.
 */
export async function fetchJson<T = unknown>(url: string, options: FetchJsonOptions): Promise<T> {
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    ...
    try {
      response = await fetchImpl(url, {
        headers: { Accept: 'application/json', ...options.headers, 'User-Agent': options.ua },
        signal: AbortSignal.timeout(timeoutMs),
      });
    } catch (error) {
      // Network error / timeout — no response, no headers: plain backoff only.
      lastError = new AdapterError(
        `GET ${safeUrl} → network_error (${redactSecrets(String(error))})`,
        { status: 0, code: 'network_error', body: '' },
      );
      if (attempt < retries) {
    ...
    if (response.ok) {
      ...
      try {
        ...
        throw new AdapterError(`GET ${safeUrl} → parse_error (invalid JSON)`, {
    ...
    lastError = new AdapterError(`GET ${safeUrl} → ${response.status}`, {
      status: response.status,
      code: 'http_error',
      body,
    });
  ...
  throw (
    lastError ??
    new AdapterError(`GET ${safeUrl} → network_error`, {
      status: 0,
      code: 'network_error',
      body: '',
    })
