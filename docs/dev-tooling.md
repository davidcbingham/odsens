...
- **`odsens`** — team `studiobing`, project id `prj_fTdiX6oYxyQ8CnAmzSzKnCb74MkU`, org `team_uH24XLt4wLRlbcqDi84oT14Y`. Git: `davidcbingham/odsens`, production branch `main`. Preset Next.js, root `.`, Node 24.x. **Deployment Protection: Standard** (Vercel Auth on all `*.vercel.app` URLs; a custom domain would be public — none attached yet).
...
- **Env vars seeded** (all "Encrypted"): non-secret config (`NEXT_PUBLIC_SITE_URL`, `YOUTUBE_CHANNEL_ID`, `MODRINTH_USER`, `MODRINTH_USER_AGENT`, `CURSEFORGE_MEMBER`, `KOFI_PAGE`, `NOTIFY_FROM_EMAIL`) in prod+preview+dev; secrets (`CRON_SECRET`, `YOUTUBE_API_KEY`) in prod+preview; **Supabase vars production only** — previews get branch-specific values from the Supabase↔Vercel integration once Branching is enabled. Still to add when obtained: `CURSEFORGE_API_KEY`, `RESEND_API_KEY`, `DISCORD_WEBHOOK_URL`, `KOFI_WEBHOOK_VERIFICATION_TOKEN`.
