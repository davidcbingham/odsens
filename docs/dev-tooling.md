...
- **Builds wait for the schema (ADR-0029, S1.5):** `pnpm build` = `node scripts/wait-for-schema.mjs && next build` — on Vercel the script polls the target Supabase project (OpenAPI root + RPC `migration_versions()`) for up to 20 min until every repo migration is applied and every relation is in PostgREST's schema cache, so a merge no longer races the Supabase GitHub integration; locally/CI it passes at once against the local stack (`SKIP_SCHEMA_WAIT=1` bypasses).
