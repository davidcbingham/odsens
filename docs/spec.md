...
| Platform | URL | Notes |
|---|---|---|
| YouTube | https://www.youtube.com/@OdSens | Channel ID `UCR7s27wznG-Utxm1OLSvqow` (what the @OdSens handle resolves to — corrected 2026-09-19; the id recorded before belonged to another channel, and the "666 subscribers, 21 videos" read at audit time may have been that channel's — re-read at S1.9). Keyless RSS + Data API available. |
...
*Revision log*
- 2026-09-26 — **S1.9 Stats built** on `feat/S1.9-stats` (Session A build pass; ADR-0049 — the S1.9 Session A findings, D1–D37 — filed, Proposed). §7 Analytics / §5 Admin: a daily 03:00 UTC snapshot job writes one `stats_daily` row per metric, source and entity (project download totals per source, site sums, comments and held comments, likes, the member count as one number, YouTube views / likes per video, channel views / subscribers, yesterday's direct downloads per project) and does the housekeeping (download-log rows older than 90 days, rate-limit rows older than a day, upload objects older than 24 h that no row references); Admin → Stats shows four tiles (downloads this week and all time, comments with the held count, tips), a flat stacked bar chart of the last 30 days by source (15 two-day bars on a phone), the honest line and a SYNC board over every feeding job with "Sync now"; moderators see the page with zeros and "Only admins see the numbers.". The `sign_in` event was already live (verified). PR opens at Session-A close; gates + merge (tag `v0.10`) follow.
- 2026-09-25 — **S1.7 Skins + Art merged** (PR #35 → `ef29152` on `main`, tag `v0.9`; ADR-0047 / ADR-0048 Accepted, D1–D31). §5 Skins / Art / Admin Skins + Art are live on odsens.com: `/skins` (a live 3D viewer with Spin / Walk / Front–Back and a Slim switch, a grid of rendered busts with the pinned 64×64 source, DOWNLOAD PNG through the counted download route), `/art` (natural-size masonry, kind filters, a lightbox with title / year / optional download), Admin → Skins and Admin → Art (add / edit / reorder / publish, the 64×64 rule, the 10 MB rule, the bust rendered on save by the software rasterizer). Session B gate rounds fixed a model-change re-render, the lightbox image box, the S1.2 project-gallery lightbox that opened empty, and the e2e run's missing seed-object upload (ADR-0048 D29–D31). Both pages show their empty states until Oliver adds content.
- 2026-09-25 — **S1.7 Skins + Art built** on `feat/S1.7-skins-art` (Session A build pass; ADR-0047 — the bust renderer is a software rasterizer, no `gl` — and ADR-0048 — the S1.7 Session A findings, D1–D28 — filed, Proposed). §5 Skins / Art / Admin Skins + Art: `/skins` with a live 3D viewer (spin / walk / front-back, Slim), a 4-up grid of rendered busts (a live bust where none is cached yet), DOWNLOAD PNG through the counted download route, `/art` with the kind filter, a natural-size masonry and a lightbox with an optional download, Admin → Skins / Art (add, edit, order, publish; uploads with the 64×64 rule and the 10 MB picture cap). PR opens at Session-A close; gates + merge (tag `v0.9`) in Session B.
- 2026-09-25 — **S1.8 follow-ups fix pass built** on `fix/S1.8-followups` (ADR-0046 Accepted, D1–D6): the admin preview card and the by-hand fields show the link that will be stored; error pages are no longer read in full; the filter buttons keep the gold focus ring; four boundary tests. No schema, no tag.
- 2026-09-25 — **S1.8 Seen on merged** (PR #31 → `82cfe47` on `main`, tag `v0.8`; ADR-0045 Accepted, D1–D30). §5 Seen on is live on odsens.com: Admin → Mentions (paste a link → preview → publish, feature / hide, reorder, Sync now), the SEEN ON row on project pages, the IN THE WILD strip with the reach line on Home, `/seen-on` with its three totals and platform / project filters, the hourly YouTube view-count refresh, and the footer's second line. All seven gates PASS first-pass; their advisories → `docs/questions.md` "S1.8 follow-ups", which David chose to clear in a fix pass before S1.7 — Skins + Art (`v0.9`).
- 2026-09-25 — **S1.8 Seen on built** on `feat/S1.8-seen-on` (Session A — begun 2026-09-19, cut off, finished 2026-09-25; ADR-0045 S1.8 Session A findings filed, Proposed, D1–D30). §5 Seen on: Oliver pastes a link on `/admin/mentions`, the server reads its title, creator, views and date (YouTube through oEmbed + the Data API, anything else through the page's Open Graph tags behind a strict server-side URL guard), he assigns it to a project or to "About OddSense generally" and publishes; a SEEN ON row on the project page, an IN THE WILD strip on Home with the reach line, and `/seen-on` with three stat tiles, platform / project filters and the mention grid; an hourly job refreshes YouTube view counts; the footer's second dry line. PR opens at Session-A close; gates + merge (tag `v0.8`) in Session B.
- 2026-09-19 — **Build order changed (David; ADR-0044):** §5 Seen on is built before §5 Skins / Art — S1.8 — Seen on next (`v0.8`), then S1.7 — Skins + Art (`v0.9`). No scope change to either.
- 2026-09-19 — **YouTube channel id corrected** (David confirmed: @OdSens is the channel). §2 held `UCo3X_c7MqfC_ub-sMJZmmOA`, which is a different creator's channel; @OdSens is `UCR7s27wznG-Utxm1OLSvqow`. The first production sync after the S1.6 merge (07:27Z) pulled 53 of the other channel's videos onto `/videos`. Fixed the same morning: `YOUTUBE_CHANNEL_ID` corrected in Vercel (all environments) + production redeployed; `.env.example`, `docs/platform-audit.md`, `docs/setup-google-cloud.md` corrected; migration `20260919073500_videos_wrong_channel_cleanup.sql` removes the wrong rows. Details: `docs/questions.md`.
- 2026-09-19 — **S1.6 Videos merged** (PR #27 → `12c46ed` on `main`, tag `v0.7`; ADR-0043 Accepted, D1–D25). §3 YouTube + §5 Videos / Home are live on odsens.com: the hourly YouTube sync, `/videos` (click-to-load `youtube-nocookie` player, Up next, MORE VIDEOS, Shorts), Home "Latest videos" beside "Find me" and the tip panel, and Oliver's Hidden / Short / Auto switches on `/admin`. All seven gates PASS after one fix round (a pressed / hover state on two links; D25 records the Shorts chip's top-left position). Gate advisories → `docs/questions.md` "S1.6 follow-ups". Next: S1.7 — Skins + Art *(superseded the same day by ADR-0044 — S1.8 — Seen on is next)*.
- 2026-09-18 — **S1.6 Videos built** on `feat/S1.6-videos` (Session A; ADR-0043 S1.6 Session A findings filed, Proposed, D1–D25). §3 YouTube + §5 Videos / Home: hourly YouTube sync into `videos`, `/videos` with click-to-load `youtube-nocookie` facades, Up next, a MORE VIDEOS grid and the Shorts row, Home "Latest videos" beside "Find me" and the tip panel, and hide / Short switches for Oliver on `/admin`. PR opens at Session-A close; gates + merge (tag `v0.7`) in Session B.
- 2026-09-18 — **Support fix pass merged** (PR #25 → `db14c17` on `main`, no tag; ADR-0042 Accepted; DESIGN.md v1.11). After David's first use of the live Support page: Ko-fi's panel cannot be handed an amount (checked against the live panel — its only developer surface is an after-the-fact payment webhook), so the $1 / $3 / $5 / Other picker showed one amount while Ko-fi's panel showed another. The gold card now has one **TIP ON KO-FI** button; pressing it replaces the card with Ko-fi's panel ("← Back" returns it), one box on the page. The amount is chosen in Ko-fi; the suggested default is Oliver's own Ko-fi setting. The floating ♥ button stays a link to `/support`; opening a pop-up of our own from it is parked for David + Oliver (`docs/questions.md`).
- 2026-09-18 — **S1.5b Support page merged** (PR #23 → `285fab6` on `main`, tag `v0.6.3`; ADR-0041 S1.5b Session A findings Accepted, D1–D8; the slice itself was inserted by ADR-0036). §4 goal 5 (Support) is live: `/support` with the $1 / $3 / $5 / Other picker whose CONTINUE ON KO-FI button opens Ko-fi's tip panel in place (an "on Ko-fi ↗" link leaves the site instead), "What it pays for", and the Supporters board in its empty state; with no Ko-fi page name saved in Admin → Settings the page says "Tips open soon." and the buttons are off. A gold ♥ SUPPORT button floats on every public page except `/support` (a 52px heart square on phones; hides on scroll-down, returns on scroll-up), and the home page has a compact "Support OddSense on Ko-fi" panel. §7 Analytics: the `tip_click` event fires from all three. Ko-fi stays one iframe on one page by a code rule (no Ko-fi script anywhere); the leaderboard's data and the Ko-fi webhook remain Phase 2 (S2.1). Also merged since the last entry, without their own line here: S1.5a Cross-posted projects (`v0.6.1`, ADR-0037) and S1.5c Project editor v2 (`v0.6.2`, ADR-0039 / ADR-0040) — records in `docs/questions.md`.
- 2026-09-11 — **S1.5 Notifications merged** (PR #10 → `1e981ef` on `main`, tag `v0.6`; ADR-0029 builds wait for the schema + ADR-0030 S1.5 contract clarifications Accepted, D1–D20; ADR-0031/0032/0033 Accepted alongside). `/admin/settings` live end-to-end: moderation mode + comments-closed default, the notification matrix (EMAIL · DISCORD per event, COMING LATER rows greyed), Discord webhook with Test + masked storage, admin emails as chips, Moderators table through `setUserRole`, Ko-fi page name; the notify pipeline — `notifyFanOut` → `notifyDeliver` every 5 min on the shared job runner, Resend emails from allay@odsens.com (CommentNew · CommentHeld · CommentReported · SyncFailed · Digest) and Discord embeds, `sync.failed` / `sync.stale` from the Modrinth/CurseForge jobs, `deleteAccount` scrubbing event payloads. Schema: `notification_matrix` (seeded), `notification_recipients`, RPC `migration_versions()`, every-role RPC revokes, four FK indexes. Also merged ahead as PR #11 (`b209df1`): the missing "New exclusive project" button on `/admin/projects`. Cross-cutting fixes since v0.5: production sign-out 403 (apex is canonical, ADR-0031), crown-only site mark (ADR-0032), admin header link back to the site (ADR-0033). Production: the merge build (`odsens-853rkueuc`) was ADR-0029's first real pass — `wait-for-schema: waiting (missing migrations: …)` then `wait-for-schema: ready (27 migrations, 20 relations)` 45 s later, `Build Completed` at 1 min, no redeploy needed; `supabase migration list --linked` shows all five `20260903120000..120400` applied. Verified on `https://odsens.com`: `/`, `/projects` (20 cards in headless Chromium — the grid is a client island, so `curl` sees only the streamed shell), `/projects/disabilities`, `/support`, sitemap + robots 200; `/admin` and `/admin/settings` unauthenticated → the root 404 body (streamed 200, ADR-0025); CSP / `X-Frame-Options: DENY` / HSTS present; cron routes 401 without the secret, with it `notify` ok (fan_out + deliver, 0 items, `sync_runs` rows every 5 min), `sync-curseforge` `skipped: not_configured`, `sync-modrinth` ok on the S1.5 shared runner (items 1; the hourly 15:07 / 16:07 / 17:07 runs were ok too); zero browser console errors. All seven gates ✅.
- 2026-09-03 — **S1.4 Comments merged** (PR #8 → `0c92ef0` on `main`, tag `v0.5`; ADR-0028 S1.4 contract clarifications Accepted, D1–D13). Authenticated comments live end-to-end on project pages: post / reply (one level, `@handle` for deeper) / like / edit within 15 min / delete / report, first-timer hold mode, banned and closed states, the moderator Moderate switch with Approve · Hide · Unhide · Delete · Ban · Rename handle, the `/admin/comments` queue with the sidebar held count, and `notification_events` written (log only — delivery is S1.5). Schema: `comments`, `comment_likes`, `comment_reports`, `notification_events`, view `comments_public`, RPC `moderator_thread`, `can_comment()` / `comment_target_visible()`, counter + guard triggers, column-level insert grants and a direct-insert backstop. All seven gates ✅ (spec-drift after two doc-only rounds).
- 2026-08-27 — **S1.3 Exclusive projects merged** (PR #6 → `a21f443` on `main`, tag `v0.4`; ADR-0026 version identity + ADR-0027 S1.3 contract clarifications Accepted). Exclusive projects live end-to-end: create/edit/publish via `/admin/projects/new` + the `/admin/projects/[id]` exclusive editor, two-phase uploads to the new `project-media`/`project-files` buckets (server magic-byte re-validation, sha512 stored + shown), counted direct downloads at `/api/download/[fileId]` (60 s signed URLs, `Content-Disposition: attachment`, 30/min rate limit, hashed-only `project_downloads` log), `ExclusiveBadge` on cards/hero/detail with the gold outline. Production verified on www.odsens.com; ADR-0026's `external_id` version keying ended the S1.2 duplicate-`version_number` sync error (production sync now runs errors-free).
- 2026-08-27 — **S1.2 Projects (synced) merged** (PR #3 banned-self-delete merged first no-tag, then PR #4 → `aeaf2b3` on `main`, tag `v0.3`; ADR-0021..0025 Accepted — incl. ADR-0023 CI build job starts local Supabase, ADR-0024 admin page-scoped server glue, ADR-0025 interim notFound status tolerance + noindex). Production verified: first Modrinth sync imported the live 18-project catalogue; `/projects` grid + detail pages public on www.odsens.com; admin curation live for `oddsense`/`bizarobot` (owner bootstrap done on staging + production).
