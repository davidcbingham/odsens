...
| `/banned` | S1.1 (ADR-0019) | dynamic | `user`, banned (redirects: anon → `/`; signed in and not banned → `/`) | own `profiles` row (`getViewer()`) | §11.3 #19 (the #18 admin-gate slab) | — (page-level markup in the route file, 03 C-21) | `app/(onboarding)/layout.tsx` (minimal shell — wordmark + Sign out only), `app/(onboarding)/banned/page.tsx`; noindex | `Banned` | none (the proxy sends every navigation of a banned account here — §3 M4b) |
