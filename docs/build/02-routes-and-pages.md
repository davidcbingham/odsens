...
| SM-29 | `next build` route table (CI) | `/`, `/projects`, `/projects/[slug]`, `/videos`, `/skins`, `/art`, `/seen-on`, `/support`, `/privacy`, `/how-comments-work` are ISR (`revalidate` 600) once their slice ships (C20 placeholders are static `○` before that — RP-16); `/welcome`, `/profile`, `/admin/*`, `/api/*` are dynamic | T-E2E-8, CI-4 |
| SM-30 | client bundle grep (`.next/static`) | the 01 INV-29 list (owner; superset of 05 CI-4: `SERVICE_ROLE`, `sb_secret`, `CURSEFORGE_API_KEY`, `YOUTUBE_API_KEY`, `RESEND_API_KEY`, `DISCORD_WEBHOOK`, `KOFI_`, `CRON_SECRET`, `GOOGLE_OAUTH`, `HASH_SECRET`, plus `[^_]SENTRY_DSN` from S1.10) → empty (01 INV-29 as amended by ADR-0002 C13; `scripts/check-bundle-secrets.mjs`) | T-E2E-44, CI-4 |
