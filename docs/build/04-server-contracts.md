...
| SC-04 | Auth is resolved with `lib/auth.ts` (01 INV-32) → `getUser()` (anon → `null`), `getProfile()`, `requireUser()` (→ `unauthenticated`), `requireOnboarded()` (handle null → `onboarding_required`), `requireRole('moderator'\|'admin')` (→ `forbidden`), `safeNext(next)` (02 RP-20), plus `getSession()` (raw session for the SSR client; not used for auth decisions). Role order: `user < moderator < admin`. Every admin/moderator action calls `requireRole` server-side even though RLS also enforces it (defense in depth). The export set is exactly 01 INV-32's (`getUser, getSession, getProfile, requireUser, requireOnboarded, requireRole, safeNext`; `_registry.md`). Roles are bootstrapped by SQL (local `seed.sql`; one documented SQL for prod after first sign-in) until `/admin/settings` ships whole in S1.5 (ADR-0002 C2, #23). Role split per ADR-0002 C7: **admin** for curation/sync/media/mentions/videos/skins/art/exclusives/settings; **moderator** for comment moderation only. |
...
| | `CURSEFORGE_MEMBER` — **removed** from `.env.example` (unused in v1, Q39 manual ids — ADR-0002 registry) |
...
| | `HASH_SECRET` — **required from S1.3** (≥ 32 random bytes, server-only) — SC-17 (ADR-0002 C13) |
...
| SC-17 | Hashing helpers (`lib/hash.ts`; 01 INV-50): `ipHash(ip) = createHmac('sha256', HASH_SECRET).update(`${ip}\|${utcDay}`).digest('hex')`; `uaHash(ua) = createHmac('sha256', HASH_SECRET).update(ua).digest('hex')`; `emailHash(email) = createHmac('sha256', HASH_SECRET).update(email.trim().toLowerCase()).digest('hex')` (ADR-0002 C13 — keyed; the DB trigger and Ko-fi matching use the same keyed hash, data-model §2.8). Raw IP / UA are never stored or logged. `HASH_SECRET` is the one and only hashing secret (`HASH_SALT`/`IP_HASH_SALT` do not exist — ADR-0002 C13). |
...
| `triggerSync` | `lib/actions/admin.ts` | S1.2 | `requireRole('admin')` | 1 / 60 s / source (lock SC-13) | per job | per job |
...
| Rate limit | `assertRateLimit('report', profile_id, 10, '1 hour')` counting `comment_reports.reporter_id, created_at` (ADR-0002 #69). |
...
| Effects | Update `site_settings where id = 1`; upsert `notification_matrix` rows; `revalidateTag('settings')` **only** — no extra `projects` tag: `/projects/[slug]` carries `settings` (02 RP-23), so a `comments_closed_default` change already refreshes detail pages, and listing pages do not show comments state. `discord_webhook_url` is never returned to the client — action returns `discord_webhook_set: boolean` and `discord_webhook_tail` (last 4 chars, `maskSecret`). SC-24 audit line. |
...
| Returns | `{ok:true, data:{override}}`. Errors: `forbidden`, `not_found`, `validation`. Tests (05): T-ACT-40 (mod = D `forbidden`, admin = A); T-RLS-41, T-RLS-42; T-E2E-34. |
...
| A3 | Read `profiles.handle` (trigger has created the row): null → 307 `/welcome?next=<next>`; else → 307 `<next>` (same-origin only via `safeNext`). |
...
| D3 | Rate limit: `assertRateLimit('download', ipHash, 30, '1 minute')` — scope source `project_downloads.ip_hash` for `project_file`; `rate_limit_hits` for `skin` → **429**, `Retry-After: 60`, JSON body `{ok:false, error:{code:'rate_limited', message:'Slow down a little.'}}` (SC-03 / ADR-0002 C14, C17). |
...
`rate_limit_hits` (created S1.1, service-role only — ADR-0002 #14) is purged by `snapshotStats` (`purge_rate_limit_hits(1)`). 01 INV-69's surface list = this table verbatim (05 T-UNIT-37 asserts `SCOPES`). Copy for `rate_limited`: "Slow down a little." (01 INV-69).
...
| `tip_click` | `{amount?: number, from: 'support'\|'tip-panel'\|'floating'\|'nav'}` (`amount` = the chosen preset or "Other" value in whole currency units; absent when no amount is chosen yet, e.g. `FloatingSupportButton`, `TipPanel`, nav Support button) | `AmountPicker` (`support`), `TipPanel` (`tip-panel`), `FloatingSupportButton` (`floating`), nav Support `Button` (`nav`) |
...
## 11. Registry + sibling amendments + tests map

### 11.1 Registry additions folded into `_registry.md` (ADR-0002)

Registry additions folded into `_registry.md` (ADR-0002). Contracts that v0.2 drafted under this heading now live in their sections — sibling citations of "04 §11.1" resolve as: `deleteAccount` → §1.1 · `renameUserHandle` → §1.2 · `site_settings.owner_profile_id` / `getOwnerProfileId()` → §1.2 (`CommentView` CREATOR note) · `setUserRole` → §1.3 · `updateVideo` → §1.8 · adapter export names → §4 · `rate_limit_hits` → SC-08 / §5.5.

### 11.2 Sibling amendments (applied by ADR-0002 in the same PR — one truth per fact; listed so gates can spot regressions)
...
| 01 | INV-47, INV-53, INV-51/33 | storage paths per SC-21; two-phase uploads baseline (§1.4.5) |
...
| 02 | §1.3, §8, §4, §2.9, §5, §2.7 | `/admin/settings` whole in S1.5; callback per §2.1; download 429 JSON + HEAD 405 (D1–D7); `toggleLike` → `project:<slug>`, `banUser` → none, `deleteAccount` → `project:<slug>` per touched target, `updateSettings` → `settings` only, `updateVideo` (§1.8) → `videos`; Ko-fi per §5.7 (empty `kofi_page` = "Tips open soon.") |
| 03 | `GoogleSignInButton`, `AvatarUpload` (file input `name`, no `action` prop), `Comment` CREATOR, `TrackedLink` allowlist + emitters, `ReportPicker` note (300), `ModActionRow` "Rename handle" (§2.4) | per §2.0, §1.1, §1.2 (`ownerProfileId`), §5.6 (value sets verbatim: `download.from` incl. `skin` + `SkinCard` emitter; `tip_click {amount?: number, from: support\|tip-panel\|floating\|nav}`; `video_play.kind` without `upnext`; `sign_in.from` = `nav\|prompt\|admin`), §1.2 |
| 05 | T-ACT-0/3/7/15/17/19/21/27/28/38/39/40/41/42/45/56/58/61/62–68, T-ADP-5/7/9/14–18, T-UNIT-16/23/26/38, T-E2E-11/27/49, SEED-13 | shapes/names/roles per this doc (mod = D `forbidden` on curation/mentions/sync — ADR-0002 C7; download 429 JSON; `video_play {youtube_id, kind}`; `/support` iframe in place; T-ACT-15 `target_type ≠ 'project'` → `validation` (§1.2 `TARGET`); T-ACT-27 `updateSettings` revalidates `settings` only; T-E2E-49 `tip_click from:'floating'` (not `'fsb'`); T-ACT-65/66/67/68 cite §1.1/§1.3/§1.2/§1.8, not §11.1) |
...
| `.env.example` | `HASH_SECRET=` added at S1.3; `CURSEFORGE_MEMBER` removed | SC-16/17 |
| `security-check` SKILL.md | Uploads: commit-phase re-validation line; Abuse: SC-24 audit line | §1.4.5, SC-24 |

### 11.3 Tests map (05 owns IDs; RA-10 mapping applied — every former "to add" item now has a number unless listed under *unnumbered*)
...
v0.3 consistency pass (2026-08-17, after ADR-0002) — applied in this doc: SC-04 export set = 01 INV-32 (adds `getSession()`) and 02 rule numbers re-cited (`safeNext` = RP-20, un-onboarded sign-out = RP-21, no `revalidatePath` = RP-22, cron `maxDuration` = RP-17); SC-08 `rate_limit_hits (scope, key, ts)` per data-model §2.10; §1.1 `updateProfile` trigger = `AvatarUpload` file input (no `action` prop, 03 §2.5); §1.2 `TARGET` = `z.literal('project')` in v1 (matches 05 T-ACT-15; DB column keeps four values, ADR-0002 C21); §1.2 `renameUserHandle` control = 03 §2.4 `ModActionRow`; §1.0/§1.3 `updateSettings` revalidates `settings` only (02 RP-23 — `/projects/[slug]` carries `settings`; the extra `projects` tag and its "(02 §5)" citation are dropped — **05 T-ACT-27 to assert `settings` only**); §5.6 value sets fixed as the allowlist (adds `tip_click.from = 'nav'` and `amount?: number` for the nav Support emitter / "Other" amounts; keeps `download.from = 'skin'` + `SkinCard`, `video_play.kind` without `upnext`, `sign_in.from = nav|prompt|admin`); §5.7 empty-`kofi_page` copy cites 00 S1.9.AC4 (not 02 O-8); §8/§11.3 number `site_settings_public` → T-RLS-132 and `can_comment()` → T-RLS-133; §11.1 kept as a one-line pointer stub so existing "04 §11.1" citations resolve.

Sibling / registry lines that still disagree with this doc (04 or ADR-0002 owns the fact; **not** edited from here — listed for the owners of 00/01/02/03/05/`_registry.md`):
- `_registry.md` Comments row: `CommentThread` (server shell) + `CommentList` C leaf → must read `CommentThread` (**C**, one file `components/comments/CommentThread.tsx`; `CommentList` dropped — ADR-0002 C1, 03 §2.4, this doc §1.2 reads); 00 S1.4 risk line "`CommentList` performs no optimistic insert" → `CommentThread`.
- `_registry.md` Route registry / repo layout: `/auth/callback` + `/auth/sign-out` are route handlers at `app/auth/callback/route.ts` and `app/auth/sign-out/route.ts` (SC-01, 02 RP-11) — add them to the "Route files" line and `app/auth/` to the repo layout; they are not under `app/(public)/`.
- 00 and 02 cite 04 conventions as `04 C-nn` (e.g. "04 C-04", "C-11–C-13", "C-07", "C-19", "C-22", "C-16"); this doc's IDs are **`SC-nn`** (`C-nn` is 03's namespace) — replace every `04 C-nn` with `04 SC-nn`.
- 02 §5 revalidation matrix: `deleteAccount` row must be `project:<slug>` per distinct comment target (not `projects, skins, art, videos`) — 02 RP-22 says the table equals §1.0; `updateSettings` row already `settings` (matches §1.3 as amended); `updateVideo` row cite `04 §1.8` (not "§11").
- 02 §2.7: empty `kofi_page` line = "Tips open soon." (00 S1.9.AC4, this doc §5.7), not "Not yet. Soon." (that is the C20 placeholder-page copy).
- 02 §2.9: download flow is `04 §2.3 D1–D7` (+ Errors row), not D1–D8.
- 01 INV-50: `emailHash` = keyed `HMAC-SHA256(HASH_SECRET, lower(trim(email)))` (SC-17, ADR-0002 C13, data-model §2.1/§2.8), not plain sha256hex.
- 03 `TrackedLink` allowlist + emitters and `GoogleSignInButton` `from` values: mirror §5.6 verbatim (add `SkinCard` + `download.from='skin'`; drop `video_play.kind='upnext'`; `sign_in.from` `thread`→`prompt`, `admin-gate`→`admin`; `tip_click.from` keeps `nav`); 03 O-15 cite `04 §1.2` (`CommentView` CREATOR note).
- 05: T-E2E-49 `tip_click` `from:'floating'` (not `'fsb'`); T-ACT-27 revalidates `settings` only; T-ACT-65/66/67/68 cite §1.1/§1.3/§1.2/§1.8 (the §11.1 stub redirects meanwhile).
