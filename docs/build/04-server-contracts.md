...
Status: **v1.0 — FROZEN 2026-08-17** (changes only via ADR + doc edit in the same PR; `spec-drift-reviewer` enforces) — amended by ADR-0009, ADR-0010, ADR-0012, ADR-0013 (2026-08-20) — amended by ADR-0014 (2026-08-20) — amended by ADR-0015 (2026-08-20) — amended by ADR-0019 (2026-08-21) — amended by ADR-0020 (2026-08-21)
...
| SC-05 | Banned check: **every user action** returns `error.code='banned'` when `profiles.is_banned = true`, before touching the DB — the account actions (`checkHandle`, `completeOnboarding`, `updateProfile`, `deleteAccount`) through `requireUser()` / `requireOnboarded()` (`lib/auth.ts` throws `AuthError('banned', "This account is banned.")` from the caller's own row, before the rate limiter records a hit — ADR-0019) and every action that inserts on behalf of a user (`postComment`, `editComment`, `toggleLike`, `reportComment`). The proxy keeps a banned browser on `/banned` (02 §3 M4b); this is the server-side half. |
...
| Returns | `{ok:true, data:{deleted:true}}`. Errors: `unauthenticated`, `banned` (ADR-0019 — a banned account cannot delete itself; removal under a ban is an admin act), `rate_limited`, `internal`. |
