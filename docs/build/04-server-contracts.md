...
Status: **v1.0 — FROZEN 2026-08-17** (changes only via ADR + doc edit in the same PR; `spec-drift-reviewer` enforces) — amended by ADR-0009, ADR-0010, ADR-0012, ADR-0013 (2026-08-20) — amended by ADR-0014 (2026-08-20) — amended by ADR-0015 (2026-08-20) — amended by ADR-0019 (2026-08-21) — amended by ADR-0020 (2026-08-21) — amended by ADR-0021 (2026-08-27) — amended by ADR-0024 (2026-08-27) — amended by ADR-0027 (2026-08-27) — amended by ADR-0028 (2026-09-03) — amended by ADR-0030 (2026-09-03) — amended by ADR-0031 (2026-09-06) — amended by ADR-0034 (2026-09-11) — amended by ADR-0036 (2026-09-11) — amended by ADR-0037 (2026-09-11) — amended by ADR-0038 (2026-09-12) — amended by ADR-0041 (2026-09-18) — amended by ADR-0042 (2026-09-18; §5.6 `tip_click` emitters, §5.7 — no amount, TIP ON KO-FI) — amended by ADR-0043 (2026-09-18; S1.6 Session A: SC-09 `fetchText`, §1.8 `updateVideo` Trigger / Effects, §1.7 Trigger, §3.3 Steps + Quota + Revalidate + Tests, §4.3 `fetchRss` / `listVideos` + return types, §5.3, §5.6 `video_play` emitter, §8 tests map §1.8 row, §10 OPEN-7; SC-09 error-body redaction) — amended by ADR-0045 (2026-09-25; S1.8 Session A: SC-09 `fetchPage`, §1.6 the three mention actions as built, §1.7 Trigger, §3 J-S `mentions`, §3.4 as built, §4 head + §4.3 `listVideoStats` / `getVideoMeta` / `OEMBED_BASE` + §4.4 `oembed` as built, §5.4 steps 3–5, §8 tests map) — amended by ADR-0046 (2026-09-25; S1.8 follow-ups: SC-09 the non-2xx read under `maxBytes`)
...
|---|---|
| SC-01 | Server Actions live in `lib/actions/<area>.ts` (`accounts.ts`, `comments.ts`, `settings.ts`, `projects.ts`, `uploads.ts`, `skins.ts`, `art.ts`, `mentions.ts`, `videos.ts`, `admin.ts`; shared types in `result.ts`) and are marked `'use server'`. Route handlers live under `app/api/**/route.ts` and `app/auth/**/route.ts` (`/auth/callback` GET, `/auth/sign-out` POST — there is no `/auth/sign-in` route, ADR-0002 C3). Jobs live in `lib/jobs/<name>.ts`. Adapters live in `lib/adapters/<name>.ts`. Deliverers live in `lib/notify/deliver/{email,discord}.ts`. `triggerSync` lives in `lib/actions/admin.ts` (01 INV-72 grep target; ADR-0002 C16). Exception (ADR-0024): dynamic `app/admin/**` route files may declare module-private page-scoped `'use server'` glue — input mapping + exactly one `lib/actions` call (+ optional `redirect()`), no own auth/zod/DB/fetch; the called action carries every guard. |
| SC-02 | Every action's input is parsed with a zod schema named `<actionName>Input`, exported from the action file's sibling module `lib/actions/<area>.schema.ts` (a plain module with no directive — Next 16 allows only async-function exports from a `'use server'` file; the action file imports it and tests import it from there — ADR-0013; 01 INV-18; ADR-0002 C16) — e.g. `postCommentInput` in `lib/actions/comments.schema.ts`. Parsing failure → `{ok:false, error:{code:'validation', message, issues:[{path,message}]}}` — issues are plain-language, no zod internals. |
| SC-03 | Return shape of every action (01 INV-19, types in `lib/actions/result.ts`): `ActionResult<T> = {ok:true, data:T} \| {ok:false, error:{code: ActionErrorCode, message: string, field?: string, issues?: {path:string, message:string}[]}}`. `ActionErrorCode` = the §7 union. Actions **never throw** to the client: every action body is `return runAction(name, schema, input, fn)` (`lib/actions/run.ts` — ADR-0013), which maps a thrown error whose `code` ∈ `ActionErrorCode` (`AuthError`, `RateLimitError`) to `fail(code, message)`, a `ZodError` to `validation` with plain issues, and any other exception to one SC-15 log line + `error.code='internal'` (`Something broke.`). Route handlers return the same JSON `{ok:false, error:{code, message}}` with 4xx/5xx (01 INV-44; ADR-0002 C14 — incl. cron 401 and the download route's 429 + `Retry-After: 60`). In this doc "Errors: `x`, `y`" means `error.code ∈ {x, y}`. |
| SC-04 | Auth is resolved with `lib/auth.ts` (01 INV-32) → **`getViewer()`** (built on `getUser()`; returns `{user, profile} \| null` — the one call pages/actions use to know who is asking — ADR-0002 A15), `getUser()` (anon → `null`), `getProfile()` (own row `id, handle, avatar_path, role, is_banned, handle_changed_at` — `handle_changed_at` is read server-side only; `ViewerProvider` keeps the five C-17a columns — ADR-0014), `requireUser()` (→ `unauthenticated`; `banned` when the caller's own `profiles.is_banned` is true — one own-row PK read, ADR-0019), `requireOnboarded()` (`banned` first, then handle null → `onboarding_required` — ADR-0019), `requireRole('moderator'\|'admin')` (→ `forbidden`), `safeNext(next)` (02 RP-20). **No `getSession()` export** (ADR-0002 A15) — session refresh is `proxy.ts`'s job (01 INV-32; the Next 16 middleware file — ADR-0009); nothing else needs the raw session. Role order: `user < moderator < admin`. Every admin/moderator action calls `requireRole` server-side even though RLS also enforces it (defense in depth). The export set is exactly `getViewer, getUser, getProfile, requireUser, requireOnboarded, requireRole, safeNext` (= 01 INV-32 + ADR-0002 A15; `_registry.md` Modules `auth.ts` names `safeNext` as the S0 export — the rest land in S1.1). `lib/auth.ts` additionally value-exports the class `AuthError` (thrown by the `require*` helpers; `code` ∈ {`unauthenticated`, `onboarding_required`, `forbidden`, `banned` (ADR-0019)}), and `safeNext` is a re-export of the pure function in the client-safe `lib/validation/next.ts` (ADR-0013). Roles are bootstrapped by SQL (local `seed.sql`; one documented SQL for prod after first sign-in) until `/admin/settings` ships whole in S1.5 (ADR-0002 C2, #23). Role split per ADR-0002 C7: **admin** for curation/sync/media/mentions/videos/skins/art/exclusives/settings; **moderator** for comment moderation only. |
| SC-05 | Banned check: **every user action except `deleteAccount`** returns `error.code='banned'` when `profiles.is_banned = true`, before touching the DB — the account actions (`checkHandle`, `completeOnboarding`, `updateProfile`) through `requireUser()` / `requireOnboarded()` (`lib/auth.ts` throws `AuthError('banned', "This account is banned.")` from the caller's own row, before the rate limiter records a hit — ADR-0019) and every action that inserts on behalf of a user (`postComment`, `editComment`, `toggleLike`, `reportComment`). The one exception (ADR-0021, David's S1.1 merge decision): `deleteAccount` passes `requireOnboarded({allowBanned:true})` — a banned account may delete itself; no other action may adopt the opt-in without its own ADR. The proxy keeps a banned browser on `/banned` (02 §3 M4b); this is the server-side half. |
...
| SC-07 | Revalidation: after a successful write, call `revalidateTag(tag)` for every tag listed in the contract; tags are exactly the registry set `projects`, `project:<slug>`, `videos`, `skins`, `art`, `mentions`, `settings`. `revalidatePath` is not used (02 RP-22). (Next 16.3 requires a cacheLife-profile second argument — call sites use `revalidateTag(tag, 'max')`, on-demand expiry, tag set unchanged; prose clarification, no rule change, 00 §1.5 F-2, 2026-08-27.) |
...
| SC-09 | Every external HTTP call goes through `lib/adapters/http.ts` → `fetchJson(url, {timeoutMs:10000, retries:3, ua})` (or `fetchText`, below, for a non-JSON body — ADR-0043 D3; or `fetchPage`, the same loop for a page the adapter does not own — resolves to `{status, location, contentType, text}` and carries three OPT-IN guards: `redirect: 'manual'` (a 301/302/303/307/308 is RETURNED with its raw `Location`, never followed), `contentTypes` (a 2xx of another media type → `unsupported`, body unread), `maxBytes` (a larger `Content-Length` refused before the read, otherwise the body is streamed and cut at the DECODED byte cap; under `maxBytes` a 4xx / 5xx body too is read only as far as its 300-char excerpt needs — `min(maxBytes, 1200)` decoded bytes, the rest cancelled, never thrown over as `unsupported`: the status is the error — ADR-0046); left unset, nothing changes for `fetchJson` / `fetchText`, whose error bodies are still read whole before the cut — the S1.8 Open Graph read, ADR-0045 D5): `AbortSignal.timeout(10000)` (**10 s**); retry on HTTP 429/5xx and network errors, backoff 1 s → 2 s → 4 s (honour `Retry-After` / `X-Ratelimit-Reset` if larger, capped at 30 s), **max 3 retries**; 4xx other than 429 is not retried; final failure throws `AdapterError {status, code, body(≤300)}`. `fetchJson` also accepts `method: 'POST'` + `body` (JSON, `Content-Type: application/json`) for the resend/discord adapters and `retryOn?: (status) => boolean`, which can only narrow the retried set (default 429 + 5xx; the discord adapter uses it so a 429 follows §4.6's retry-once rule instead of the SC-09 backoff); a 2xx with an empty body resolves to `null`; request bodies never appear in `AdapterError` messages/bodies or logs (ADR-0030 D6). An upstream error body is redacted for key-like query params (`redactSecrets`, exported from `http.ts` for adapters that build their own `parse_error` label from a keyed URL) **before** it is cut to 300 chars — an upstream that echoes the request URL cannot put a key, or the head of one cut at char 300, into `AdapterError.body`; this holds for every adapter (S1.6, ADR-0043 D17). **`fetchText(url, opts)`** (S1.6, ADR-0043 D3 — the YouTube RSS feed is Atom XML and `fetchJson` `JSON.parse`s every 2xx body) is the same request loop for a non-JSON body: GET only (no `method` / `body`), the same timeout, retry set, backoff, `User-Agent`, key redaction and `AdapterError`; `Accept` = the caller's `accept` (default `*/*`; options type `FetchTextOptions` = `FetchJsonOptions` without `method` / `body`, plus `accept?`); it resolves to the 2xx body verbatim (empty → `''`), never looks at the response `Content-Type`, and the caller owns the parse and its typed `parse_error`. No XML parser dependency: `fetchRss` parses the feed with a hand-rolled Atom parser (§4.3; 01 INV-78 unchanged). |
...
| SC-16 | Env is read only via `lib/env.ts` (zod-validated at import; 01 INV-36). Table = every name in `.env.example` (ADR-0002 #18: the boot-required set is exactly the **8 required** rows below — **9 from S1.1**, `HASH_SECRET` included (ADR-0012); browser-safe names live in `lib/env/public.ts`): |
| | `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY` — **required** — `lib/supabase/client.ts`, `server.ts` (on previews — the persistent `staging` Supabase branch — the Vercel Preview env carries `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`; `lib/env.ts` / `lib/env/public.ts` accept it as an alias when the canonical name is blank, canonical wins — ADR-0010) |
| | `SUPABASE_URL`, `SUPABASE_ANON_KEY` — **CLI only** (Supabase CLI / `config.toml`); not read by `lib/env.ts` (ADR-0002 #18) |
| | `SUPABASE_SERVICE_ROLE_KEY` — **required** — `lib/supabase/admin.ts` only (preview alias `SUPABASE_SECRET_KEY`, acts as service_role — ADR-0010) |
...
| | `CURSEFORGE_API_KEY` — optional, degrades: `syncCurseforge` skipped run, `linkProjectListing` `curseforge` case (was `setProjectLink` — ADR-0037 D1) → `upstream_error` — `lib/adapters/curseforge.ts` |
...
| | Boot-required set = `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `NEXT_PUBLIC_SITE_URL`, `CRON_SECRET`, `MODRINTH_USER`, `MODRINTH_USER_AGENT`, `YOUTUBE_CHANNEL_ID` (ADR-0002 #18) **+ `HASH_SECRET` from S1.1 = 9 names (ADR-0012)**; everything else is optional-with-degradation as stated per row. `lib/env.ts` fails fast only on those 9 (05 T-UNIT-16, T-UNIT-35). |
...
| SC-22 | Notification events are inserted via `lib/notify/emit.ts` → `emit(kind, {actorId?, subjectType, subjectId, payload})`; `kind` must be a member of the catalog in `docs/notifications.md` (`comment.new`, `comment.held`, `comment.reported`, `comment.reply`, `comment.approved`, `sync.failed`, `sync.stale`, P2 kinds). `payload` never contains emails or Google identity; user references are `{profile_id, handle}`. `deleteAccount` scrubs every payload reference `{profile_id, handle}` to the caller to `{profile_id: null, handle: null}` before `auth.admin.deleteUser` (`lib/notify/emit.ts` `scrubProfileFromEvents`); the rows themselves are kept (ADR-0030 D4). |
...
| `deleteAccount` (ADR-0002 #28) | `lib/actions/accounts.ts` | S1.1 | `requireOnboarded({allowBanned:true})` (ADR-0021) | 1 / day / user | `project:<slug>` per touched target | — |
...
| `linkProjectListing` (was `setProjectLink` — ADR-0037 D1) | `lib/actions/projects.ts` | S1.2 (`curseforge`) · S1.5a (`modrinth`) | `requireRole('admin')` | 30 / hour / user (`rate_limit_hits`, scope `project_link`) | `projects`, `project:<slug>` (+ `project:<redirect slug>` when the fold ran) | — |
| `unlinkProjectListing` (ADR-0037 D1) | `lib/actions/projects.ts` | S1.5a | `requireRole('admin')` | 30 / hour / user (`rate_limit_hits`, scope `project_link` — shared with `linkProjectListing`) | `projects`, `project:<slug>` | — |
...
Role rule (**DECIDED — ADR-0002 C7**, flagged [DAVID] for awareness): **`admin`** is required for curation, sync, media, mentions, videos, skins, art, exclusive projects, settings and roles — `curateProject`, `linkProjectListing` / `unlinkProjectListing` (was `setProjectLink` — ADR-0037 D1), `triggerSync`, `uploadProjectMedia`, `uploadProjectFile`, `create/update/publish` exclusive project, `create/updateSkin`, `create/updateArt`, `fetchMentionPreview`, `createMention`, `updateMention`, `updateVideo`, `updateSettings`, `testDiscordWebhook`, `setUserRole`. **`moderator`** is required only for comment moderation — `moderateComment`, `banUser`, `renameUserHandle`, `deleteComment` (as non-author) — and for *reading* `/admin/*` pages (02's remit; wrong role → `notFound()`, ADR-0002 C4). Matches `docs/spec.md` ("mods can delete/hide comments and ban users"). RLS is unchanged (writes on `project_overrides`, `project_links`, `mentions`, `videos`, `skins`, `art`, `projects` = `is_admin()`; actions use the service client after `requireRole`).
...
| Handle rules (H) | H1 `^[A-Za-z0-9_]{3,20}$` (rejects `@`, spaces, dots, email-likes by construction). H2 case-insensitive unique (`citext`). H3 not in `RESERVED_HANDLES` (case-insensitive) — list: `admin, administrator, oddsense, odsens, moderator, mod, mods, root, system, support, allay, api, staff, help, null, undefined, anonymous, deleted, me, you, everyone, here` (first five mandated by `security-check`; the full 22-entry list is DECIDED — ADR-0002 #63); the DB twin `is_reserved_handle()` binds the owner's direct write too — the `profiles_guard` trigger refuses a reserved first handle on a `profiles` PATCH, and every own-row write by a banned account (42501) — ADR-0020. H4 no name/email detection (Q34; DESIGN.md §12.5). H5 the same list lives in SQL once — function `is_reserved_handle`, called by `check_handle` and by `profiles_guard` (ADR-0020) — and in `lib/validation/handle.ts` `RESERVED_HANDLES` (+ `validateHandle`; the zod `handleSchema` lives in `lib/actions/accounts.schema.ts` — ADR-0013); 05 T-UNIT-2 asserts the TS list, T-ACT-7 the SQL ↔ TS parity. |
| Preconditions | Session valid; **not banned** — `requireUser()` throws `banned` before the limiter (ADR-0019; the earlier "a banned account may still finish onboarding so it can be identified" rule is withdrawn — the proxy sends a banned account to `/banned`, 02 M4b). |
...
| Returns | `{ok:true, data:{handle, avatar_path}}`. Errors: `handle_taken`, `handle_reserved`, `validation`, `conflict`, `storage_error`, `rate_limited`, `unauthenticated`, `banned` (ADR-0019). |
...
| Returns | `{ok:true, data:{handle, avatar_path}}`. Errors: `handle_taken`, `handle_reserved`, `validation`, `rate_limited`, `storage_error`, `banned` (ADR-0019). |
...
| Auth | SQL: `security definer`, `grant execute to authenticated` only; action: `requireUser()` (→ `banned` for a banned caller, before the limiter — ADR-0019). |
...
| Item | Contract |
|---|---|
| Trigger | `/profile` "Delete account" (danger, inline confirm; DESIGN.md §11.3 p.11; 02 O-6) and, since ADR-0021, `/banned`'s `BannedDelete` control (same inline confirm; onboarded banned accounts only). |
| Auth | `requireOnboarded({allowBanned:true})` — the SC-05 exception (ADR-0021): a banned caller passes; a NULL-handle caller (banned or not) still gets `onboarding_required`. Input (`deleteAccountInput`) `{confirm: z.literal(true)}`. Rate limit `assertRateLimit('delete_account', profile_id, 1, '1 day')`. |
| Effects (service role, one transaction where possible) | `comments where author_id = me` → `status='deleted'` (slot stays, body retained per §1.2 `deleteComment`); `comment_likes where user_id = me` deleted (trigger fixes `like_count`); `comment_reports where reporter_id = me` deleted; avatar object removed (deletes only objects under `avatars/{caller_id}/` — `isOwnAvatarPath`, ADR-0015); `profiles` row deleted via `auth.admin.deleteUser(id)` cascade — comments keep `author_id` as a dangling reference rendered as `author: null` ("Deleted." slot). `revalidateTag('project:<slug>')` for every distinct comment target (not the four site tags). Sign the user out (cookies cleared) → client redirects `/`. `notification_events` rows by or about the caller are kept (admin audit; `actor_id` nulls through the FK) with every `{profile_id, handle}` payload reference scrubbed to `{null, null}` before `deleteUser` (ADR-0030 D4); the scrub runs after the comment cascade and the avatar removal, right before `deleteUser`, so a scrub failure leaves the cascade applied and its `project:<slug>` tags fired with the account intact — the delete is retryable and completes on the next call. |
| Returns | `{ok:true, data:{deleted:true}}`. Errors: `unauthenticated`, `onboarding_required`, `rate_limited`, `internal` — never `banned` (ADR-0021 supersedes the ADR-0019 line here: a banned account may delete itself; a banned account that never onboarded still cannot — its removal is an admin act). |
...
- **Body rules (B)** (`lib/validation/comment.ts` `validateBody`, `stripHtml`, `countLinks` — zod-free, island-safe; the zod `commentBodySchema` refines on `validateBody` from `lib/actions/comments.schema.ts`, the `handleSchema` precedent — ADR-0008 D3 / ADR-0028 D5): B1 strip HTML tags (`<script>…</script>` blocks whole, then `/<[^>]*>/g` → '' — ADR-0028 D11), then trim; B2 length 1..1000 code points (after B1) — DB check `char_length(body) <= 1000`; B3 links counted with `/(https?:\/\/[^\s]+|www\.[^\s]+)/gi` → count ≤ 1 else `too_many_links`; B4 stored as plain text; rendering auto-linkifies with `linkify()` from `lib/validation/comment.ts` (never `lib/markdown.ts`, which is server-only — ADR-0002 C16); error copy per code from `commentErrorLine(code)` in the same module (05 T-UNIT-40); B5 no ` `; B6 message copy for B2/B3 as per DESIGN.md §11.2 ("Too many links.").
...
- **`CommentView`** (type home `lib/data/comments.ts`; returned by `postComment`/`editComment` and consumed by 03 `CommentThread`/`Comment`/`Composer.onPosted`): `{id: string, body: string, status: 'published'\|'held'\|'hidden'\|'deleted', createdAt: string, editedAt: string\|null, parentId: string\|null, likeCount: number, likedByViewer: boolean, isFirstComment?: boolean, reportCount?: number /* mods only — ADR-0028 D9 */, author: {id: string, handle: string, avatarUrl: string\|null, role: 'user'\|'moderator'\|'admin'} \| null}`. `avatarUrl` = public URL of `avatar_path` or null. **CREATOR tag** (03 `Comment`): `author.id === site_settings.owner_profile_id` (ADR-0002 #55; read via view `site_settings_public` → `lib/data/settings.ts` `getOwnerProfileId()`, C6).
...
| Preconditions | Comment exists, is not `deleted`, and its target is visible (`can_comment` binds the RLS insert) — held/hidden comments accept the report with no status change (§5.1 M8; ADR-0028 D2); not own comment (`validation`, "You can't report your own comment."). |
...
| Auth | `requireRole('moderator')`. |
| Input (`renameUserHandleInput`) | `{profile_id: uuid, handle: string}` — the H-rules verdict comes from RPC `check_handle` below, not the schema (ADR-0028 D10). |
| Preconditions | Target exists; target `role='user'` unless actor is `admin` (mods cannot rename mods/admins → `forbidden`); RPC `check_handle` (run under the moderator's session — a rename onto the moderator's own handle is answered by the unique index; ADR-0028 D10) → `handle_taken` / `handle_reserved` / `validation`. |
...
| Input (`updateSettingsInput`) | all optional (partial update): `moderation_mode: z.enum(['auto','hold_first_time'])` · `admin_notify_emails: z.array(email).max(10)` (each `z.string().email().max(254)`, lowercased, de-duplicated; never pre-filled from the session — 00 S1.5.AC4) · `discord_webhook_url: z.string().regex(/^https:\/\/(discord|discordapp)\.com\/api\/webhooks\/\d+\/[A-Za-z0-9_-]+$/).or(z.literal(''))` — omitted = unchanged (UI shows it masked), `''` = clear · `kofi_page: /^[A-Za-z0-9_-]{1,40}$/ \| ''` (`''` = clear → NULL, the webhook rule — ADR-0030 D14) · `comments_closed_default: boolean` · `announcement_md: string ≤ 2000 \| null` · `matrix: z.array({kind: catalogKind, channel: z.enum(['email','discord']), enabled: boolean})` — kinds outside the v1 `notification_matrix` set (`comment.new, comment.held, comment.reported, sync.failed, sync.stale`) rejected with `validation` (COMING LATER rows are display-only). |
| Effects | Update `site_settings where id = 1`; upsert `notification_matrix` rows; `revalidateTag('settings')` **only** — no extra `projects` tag: `/projects/[slug]` carries `settings` (02 RP-23), so a `comments_closed_default` change already refreshes detail pages, and listing pages do not show comments state (04 owns tags per 02 RP-22; 01 INV-40 / 02 §5 / 05 T-ACT-27 still carry the old "+ `projects`" clause — see §12 outstanding list). `discord_webhook_url` is never returned to the client — action returns `discord_webhook_set: boolean` and `discord_webhook_tail` (the raw last 4 chars via `secretTail`, `null` when unset; `maskSecret` renders `…<tail>` — ADR-0030 D14). SC-24 audit line. |
...
| Effects | `adapters/discord.postEmbed(url, {title:'Test — odsens', description:'The allay says hi.', color: INDIGO})` (§4.6); nothing stored; URL never logged. Order as built (ADR-0030 D14): input URL else stored → neither = `validation` (field `url`, records no hit) → `assertRateLimit('discord_test')` → post; an HTTP failure answers `upstream_error` "Discord said no: <status>", a transport failure "Discord didn't answer." |
...
| Preconditions | Target exists (`not_found`); target ≠ self when demoting (`forbidden`, avoids locking out the last admin); at least one admin must remain (`conflict` — admins are counted before and after the write and the write is reverted, guarding the concurrent-demotion window; with the actor an admin and the target ≠ self the count is otherwise ≥ 1 — ADR-0030 D14); target not banned when `role ≠ 'user'` (`conflict` — "Unban that account first."; §1.2 `banUser` refuses staff, this refuses the other direction — ADR-0030 D14). |
...
| Effects | Insert `projects {source:'odsens', external_id:null, status:'draft', downloads_* 0, published_at:null}`; slug conflict (citext, incl. Modrinth slugs, **and a slug present in `project_redirects`** — ADR-0037 D4) → `conflict` (the taken-slug message). No revalidation (draft invisible). SC-24. |
...
| Item | Contract |
|---|---|
| Auth | `requireRole('admin')`. Input (`updateExclusiveProjectInput`) `{id: uuid} & Partial<createExclusiveProject input>` (slug change allowed while `status='draft'` only, else `conflict`; a slug present in `project_redirects` → `conflict`, the taken-slug message — ADR-0037 D4); the four clearable optionals `license`/`source_url`/`issues_url`/`discord_url` also accept `null` to clear the stored value (absent = keep — ADR-0027); `source`, `external_id`, `downloads_*` never accepted; `gallery?: [{url, title?: ≤ 120 \| null, description?: ≤ 500 \| null, ordering: int 0..999, featured?: boolean}] max 20` edits the project's own gallery in place — every `url` must already be stored (uploads add entries; an unknown url → `validation` "That image isn't in this project's gallery."), an entry left out is removed — ADR-0038 D3. |
| Preconditions | `projects.source = 'odsens'` else `forbidden` (synced rows are curated via `curateProject`). |
| Effects | Update columns; a removed `gallery` entry's `project-media` object is deleted after the update (ADR-0038 D3, best-effort); `revalidateTag('projects')`, `revalidateTag('project:<slug>')` (old and new slug if changed). SC-24. |
...
| Preconditions | Every `source` (was `source='odsens'` — ADR-0037 D5(c); for synced rows the durable hide remains `project_overrides.hidden`, since the sync re-asserts `status='published'` for listed rows, so the editor offers publish controls on `source='odsens'` rows only). To `published`: `icon_url` not null AND ≥ 1 `project_versions` with ≥ 1 `project_files` (`storage_path` not null) → else `precondition_failed` with message listing what's missing ("Nothing to download yet." when no file; DECIDED — ADR-0002 #65). |
...
| Input (`curateProjectInput`) | **either** the batch shape `{reorder: [{project_id: uuid, featured_order: int 1..99}] max 99}` (one call, one transaction, one revalidate — ADR-0002 A11) **or** the per-project shape `{project_id: uuid, featured?: boolean, featured_order?: int 1..99 \| null, hidden?: boolean, title_override?: 1..80 \| null, description_override?: 1..256 \| null, extra_gallery?: [{path: string, title?: ≤120, description?: ≤ 500, ordering: int}] max 20, gallery_overrides?: [{url: https URL ≤ 2048, hidden?: boolean, title?: ≤ 120 \| null}] max 40 (ADR-0038 D3 — per-image curation of the synced gallery, keyed by `projects.gallery[].url`; applied on read by `mergeGallery`), notes_md?: ≤ 20000 \| null, comments_enabled?: boolean}` — every **new** `extra_gallery.path` must match `^project-media/<this project_id>/gallery/[A-Za-z0-9._-]+\.(png\|jpg\|webp)$`; an entry whose `path` is already present in the stored `project_overrides.extra_gallery` skips the `<this project_id>` folder check (it was validated when added; a folded duplicate's entries live under the old folder — ADR-0037 D5(e)); every entry must exist in bucket `project-media` (HEAD check); this action **reorders/edits/removes** gallery entries — adding one is `uploadProjectMedia kind='gallery'`. |
| Effects | Per-project: upsert `project_overrides` (PK project_id); an `extra_gallery` path stored on the row and absent from the input has its `project-media` object removed after the upsert (ADR-0038 D3; best-effort, one warn line on failure); `revalidateTag('projects')`, `revalidateTag('project:<slug>')`. Batch `reorder`: upsert `project_overrides.featured_order` for every listed id in one transaction, then **one** `revalidateTag('projects')` (no per-slug tags — cards/home strip read the `projects` tag). SC-24. |
...
#### `linkProjectListing` (replaces `setProjectLink` — ADR-0037 D1)
| Item | Contract |
|---|---|
| Trigger | `/admin/projects/[id]` "Modrinth listing (URL, slug or id)" and "CurseForge id or URL" `Field`s, each with a Link `Button` (Q39 manual entry; 02 §1.3, 03 §2.10). |
| Auth | `requireRole('admin')` (ADR-0002 C7). Rate limit `assertRateLimit('project_link', profile_id, 30, '1 hour')`. |
| Input (`linkProjectListingInput`) | `{project_id: uuid, platform: z.enum(['modrinth','curseforge']), ref: string 1..300}`. `curseforge`: `ref` is either digits (`/^\d{1,10}$/`) or a CurseForge URL `^https://(www\.)?curseforge\.com/minecraft/(mc-mods|texture-packs|data-packs|bukkit-plugins|modpacks|shaders)/([a-z0-9-]+)` (`adapters/curseforge.parseRef`). `modrinth`: `ref` is a Modrinth URL (`https://(www.)?modrinth.com/(mod\|plugin\|datapack\|resourcepack\|shader\|modpack\|project)/<slug-or-id>[/…]`), a bare slug or a bare id (`/^[A-Za-z0-9._-]{1,64}$/`; a dots-only ref — `.`, `..` — is refused, in the URL path too), parsed by the pure adapter export `parseModrinthRef(ref) → {ref} \| null` — any other host → `validation`. Removing a link is `unlinkProjectListing` (the former `ref: null` clause). |
| Preconditions | `curseforge`: `CURSEFORGE_API_KEY` set else `upstream_error` message "CurseForge key not configured"; an exclusive may gain a CurseForge link and loses its badge by design (the former "`source='odsens'` → `validation`" refusal is dropped — ADR-0037 D1). `modrinth`, checked **before any write**: the project exists (`not_found`); `projects.source = 'odsens'` — a `source='modrinth'` row IS its listing (`validation` "This project is synced from Modrinth already."); this project has no `modrinth` link to a *different* listing (`conflict` "Remove the current Modrinth listing first." — replacing goes through unlink so the un-adopt runs; re-linking the same id is idempotent); the listing is not linked to another project (`conflict` "That listing is already linked to <title>." — the unique index `project_links_platform_external_id_key (platform, external_id)` is the atomic backstop: a 23505 on the upsert maps to this `conflict`). |
| Effects | `curseforge` (unchanged from `setProjectLink`): URL → resolve id via `adapters/curseforge.searchBySlug(slug)` (§4.2); id → `adapters/curseforge.getMod(id)`; upsert `project_links {project_id, platform:'curseforge', external_id: String(id), url: data.links.websiteUrl, downloads: data.downloadCount, synced_at: now()}` **and set `projects.downloads_curseforge` immediately** (05 T-ACT-41). `modrinth` (ADR-0037 D1): resolve via `adapters/modrinth.getProject(idOrSlug)` → `GET /v2/project/{id\|slug}` (SC-09; 404 → `not_found` "Nothing on Modrinth matches that."; other upstream failure → `upstream_error`); then **in this order** — (a) upsert `project_links {project_id, platform:'modrinth', external_id: raw.id, url: modrinthListingUrl(raw.id), downloads: raw.downloads, synced_at: now()}` (PK `(project_id, platform)`; the URL is built from the adapter's id, never stored from the pasted string); (b) `projects.downloads_modrinth = raw.downloads` immediately; (c) if a `projects` row `(source='modrinth', external_id = raw.id)` exists → RPC `fold_project(duplicate, this)` (§3 J-D exception; data-model §2.11) — the link is written first so a fold failure leaves a state `syncModrinth` converges from (§3.1 step 2 (i) routes the listing into this project, step 4 hides the stale duplicate, re-linking re-runs the fold); (d) `revalidateTag('projects')`, `revalidateTag('project:<slug>')`, and when (c) ran also `revalidateTag('project:<redirect_slug>')` (the RPC's return value). Versions arrive on the next `syncModrinth` run (hourly, or "Sync now") — the action never calls the job and never calls `listVersions`. SC-24. |
| Returns | `{ok:true, data:{link}}`. Errors: `forbidden`, `not_found`, `validation`, `conflict`, `upstream_error`, `rate_limited`. |
| Tests (05) | T-ACT-41 (`curseforge`), T-ACT-79 (`modrinth`), T-ACT-81 (the fold); T-ADP-7 (fixtures `mod.json`, `search.json`), T-ADP-22 (`getProject`, `parseModrinthRef`; fixtures `project/sd000101.json`, `project/sd000199.json`). |

#### `unlinkProjectListing` (ADR-0037 D1)
| Item | Contract |
|---|---|
| Trigger | The Remove `Button` beside a linked listing on `/admin/projects/[id]`. |
| Auth | `requireRole('admin')` (ADR-0002 C7). Rate limit `assertRateLimit('project_link', profile_id, 30, '1 hour')` (shared scope). |
| Input (`unlinkProjectListingInput`) | `{project_id: uuid, platform: z.enum(['modrinth','curseforge'])}`. |
| Preconditions | The project exists (`not_found`); `modrinth` on a `source='modrinth'` row → `validation` "This project is synced from Modrinth already." (its listing is its home — there is no link row and nothing to un-adopt). No link row for that platform is **not** an error: the action is idempotent — it un-adopts (modrinth), deletes zero rows, still zeroes the platform count and returns `{link: null}` (the state converges either way; ADR-0037 D1, 05 T-ACT-80). No Modrinth call. |
| Effects | `modrinth`, **in this order**: (1) **un-adopt** — every `project_versions` row on this project with `external_id IS NOT NULL` **that carries ≥ 1 hosted file** (`project_files.storage_path IS NOT NULL`) gets `external_id = NULL`, at most one row per `version_number` (the row holding a hosted `primary` file, else the newest `date_published`; any other same-numbered adopted row keeps its `external_id` — `project_versions_exclusive_version_key` is a partial unique); rows with no hosted file keep their `external_id` and follow the listing on the next sync (§3.1 step 3 re-parent); CDN-only file rows on un-adopted versions are kept (nothing but the fold deletes a synced row) and are not rendered while the project has no platform home (02 §2.3 #4); (2) delete the `project_links` row; (3) `projects.downloads_modrinth = 0`. `curseforge`: delete the row, `downloads_curseforge = 0`. Both: `revalidateTag('projects')`, `revalidateTag('project:<slug>')`. SC-24. |
| Returns | `{ok:true, data:{link: null}}`. Errors: `forbidden`, `not_found`, `validation`, `rate_limited`. |
| Tests (05) | T-ACT-80. |
...
| Effects | `icon`: `projects.icon_url = path` on **every** `source` (was `odsens` only — ADR-0037 D5(a); on a `source='modrinth'` row the sync leaves a Storage-path icon alone, §3.1 step 2). `gallery`: `source='odsens'` → append `{url: path, title, description, ordering: max+1, featured:false}` to `projects.gallery` (a 21st image → `validation` "20 images maximum." — ADR-0038 D3); `source='modrinth'` → upsert `project_overrides` and append `{path, title, description, ordering: max+1}` to `extra_gallery` — a 21st image → `validation` "20 images maximum." (the `curateProject` bound, enforced on the upload side too — ADR-0038 D3). Revalidate `projects`, `project:<slug>`. SC-24. |
...
| Item | Contract |
|---|---|
| Auth | `requireRole('admin')`. Bucket `project-files` (**private**). Every `source` (was `source='odsens'` only — ADR-0037 D5(b)). |
| Input (begin) | `{phase:'begin', project_id, version_number: /^[0-9A-Za-z.\-+_]{1,32}$/, filename (ext ∈ .jar .zip .mrpack after SC-20), size_bytes ≤ 104_857_600, mime}` — `begin` computes `version_id` = the existing version with that `version_number` on this project — a row with `external_id IS NULL` first, else the newest synced row with that number (ADR-0026 duplicates; ADR-0037 D5(b)) — or a fresh uuid reserved in the returned path; the version row is upserted only at commit. |
...
| Validation (commit) | ZIP magic bytes; size ≤ 100 MB (ADR-0002 #31); sha512 computed over the buffered bytes (ADR-0027); filename unique among the version's **hosted** rows (`storage_path IS NOT NULL`; a CDN-only row may share a filename — ADR-0037 D5(b)) (`conflict`), checked before the version-metadata upsert. |
| Effects | On a hosted version row (`external_id IS NULL`) upsert `project_versions` as before; on a synced version row the form's version metadata is **ignored** (the row follows Modrinth) and only the file row is written (ADR-0037 D5(b)). Sibling rules over the version's files (D5(b)): a **CDN-only sibling with the same `sha512`** (`storage_path IS NULL`) gains `storage_path = path` (it becomes the hosted row and keeps `url` — the mirror of §3.1 step 3's "hosted row gains `url`"; both upload orders converge on one row); a hosted sibling with the same filename and the same `sha512` → the existing row (U3); a hosted sibling with the same filename and different bytes → `conflict`; otherwise insert `project_files {version_id, filename, size_bytes, sha512, url:null, storage_path: path, primary, download_count:0}`. `primary` is scoped to **hosted** siblings (`storage_path IS NOT NULL`): if `primary` true → clear `primary` on hosted siblings; if the version has no hosted primary → this file becomes primary (the sync manages `primary` among CDN-only rows). Revalidate `projects`, `project:<slug>`. SC-24. |
...
| Input (`fetchMentionPreviewInput`) | `{url}` — a SCHEMA rule (`readMentionUrl`, `lib/validation/mention-url.ts` — ADR-0045 D8): parseable, ≤ 2048, scheme `http:` / `https:` only (`file:` / `javascript:` / `data:` are `validation` before any resolver runs), no credentials, `http:` upgraded to https; the parsed value is the normalised href of what was pasted, NOT the canonical form. Host rules are the guard's: the host must not resolve to a private / loopback / link-local range (checked after DNS in `adapters/oembed.assertPublicHost`, §4.4 — which repeats scheme + credentials as defence in depth); every refusal is the one `upstream_error` message below. |
| Logic | `requireRole('admin')` → the rate limit (before DNS and before any request) → the §5.4 metadata chain (ADR-0045 D8: step 1 runs for every platform and its normalised URL is what every later step uses). |
| Returns | `{ok:true, data:{platform, external_id, canonical_url, title, creator_name, creator_url, thumbnail_url, published_at, view_count, source:'oembed'\|'data_api'\|'og'}}` — `creator_name` is `string \| null` (an Open Graph page often has none; the island opens the manual fields), `source` = `data_api` when step 3 returned the video, else `oembed`, else `og` (ADR-0045 D8); unknown/unsupported → `{ok:false, error:{code:'upstream_error', message:"Couldn't read that page. You can fill the fields by hand."}}` — EVERY `AdapterError` that ends the chain (an SSRF refusal, DNS, a timeout, a 4xx / 5xx, a non-HTML page, no title) is that one message: the guard is no oracle; the reason is one server-side `warn` line carrying the step and the adapter's error code, never the URL; anything that is not an `AdapterError` → `internal`. Nothing is stored and nothing is revalidated. |
| Tests (05) | T-ACT-62 (mod = D `forbidden`); T-ADP-14, T-ADP-15, T-ADP-16, T-ADP-1 (`fetchPage`), T-ADP-20 (`oembed` has no base override — ADR-0045 D6); T-E2E-39. |
...
| Preconditions | `url` unique (canonicalised: strip `utm_*`, `si`, `feature` params; YouTube → `https://www.youtube.com/watch?v=<id>`) → `conflict`. `project_id` must exist (`validation`). As built (ADR-0045 D9): `canonicalMentionUrl` (`lib/validation/mention-url.ts`) — `utm_*` / `si` / `feature` dropped case-insensitively, every YouTube form `videoIdFromUrl` reads → the watch form, `http:` → `https:`, nothing else rewritten (idempotent); uniqueness is the `mentions_url_key` constraint — 23505 → `conflict` on `url`, no racy pre-read; a nonexistent `project_id` → `validation` on `project_id` (a 23503 mid-call answers the same). |
| Effects | Insert `mentions {…, source:'manual', created_by: <the acting admin>}` — set by the action, since `auth.uid()` is NULL under the service client (ADR-0045 D9); for `platform 'youtube'` the link's own video id is `external_id` (a caller's value only when the link carries none) and `thumbnail_url` = the first adapter-returned `i.ytimg.com` URL, else the `https://i.ytimg.com/vi/<id>/hqdefault.jpg` literal (D7). `revalidateTag('mentions')`; if `project_id` → `revalidateTag('project:<slug>')`; never `projects`. SC-24. |
| Returns | `{ok:true, data:{mention}}`. Errors: `forbidden`, `validation`, `conflict`. Non-YouTube `thumbnail_url` is stored but the UI renders the `PlatformMark` placeholder (no remote fetch — ADR-0002 #33). `createMentionInput` as built (ADR-0045 D9): `project_id` required and nullable; the optional keys absent-or-valued (never `null`); `creator_url` / `thumbnail_url` = the §1.4 shared URL rule (https, ≤ 512); a `youtube` row's `external_id` must be 11 url-safe chars (refused in words — `NOT_A_YOUTUBE_ID`); `sort_order` within `integer`; text trimmed. Schemas, input types and result shapes live in `lib/actions/mentions.schema.ts` (ADR-0013 pattern). Tests (05): T-ACT-63 (mod = D); T-RLS-104; T-E2E-39. |

#### `updateMention`
| Item | Contract |
|---|---|
| Auth | `requireRole('admin')` (ADR-0002 C7). |
| Input (`updateMentionInput`) | either `{id: uuid, patch: Partial<createMention input minus url> & {status?: draft\|published\|hidden}}` **or** `{reorder: [{id, sort_order:int}] max 200}` (drag-reorder; runs in one transaction). As built (ADR-0045 D10): in `patch` at least one key is present, the keys optional on create may be `null` = clear the stored value, a `url` key is stripped (a mention's link never changes), `suggested` is unreachable from both forms; in `reorder` 1..200 pairs, each id once; a value fitting neither form fails as ONE issue at path `''` (the `curateProject` behaviour). |
| Effects | Update; `revalidateTag('mentions')` + affected `project:<slug>` tags. Suggested-tab Approve (S2.4) = `patch.status='published'` on a `status='suggested'` row — never automatic. SC-24. As built (ADR-0045 D10): `{id, patch}` writes only the keys present; unknown `id` → `not_found`; unknown `patch.project_id` → `validation` on `project_id`; a YouTube row must keep an 11-char `external_id` — checked against the STORED half when the patch carries only one of `platform` / `external_id` → `validation` on `external_id`; revalidates `mentions` + the project it left + the project it joined, each once. `{reorder}` = RPC `reorder_mentions(p_items jsonb)` (migration `20260919120100`; `security definer`, service_role-only EXECUTE) — ONE statement over every listed row, an id that matches no row → P0002 → `not_found` with NOTHING applied; the client sends `sort_order` 1..n and the rows hold exactly what was sent (nothing is renumbered server-side); revalidates `mentions` once — `sort_order` only orders the Home strip, so no per-project tag. |
| Returns | `{ok:true, data:{mention}}` / `{ok:true, data:{reordered:n}}`. Errors: `forbidden`, `validation`, `not_found` (ADR-0045 D10). Tests (05): T-ACT-64 (mod = D); T-RLS-103, T-RLS-105, T-RLS-129 (`reorder_mentions` grants + behaviour — ADR-0045 D2). |
...
| Item | Contract |
|---|---|
| Trigger | `SyncStatus` "Sync now" buttons in `/admin/projects` (modrinth, curseforge) and on the `/admin` dashboard (since S1.2; from S1.6 its rows are modrinth, curseforge and youtube — `DASHBOARD_SYNC_SOURCES` in `lib/data/admin.ts`, 03 §2.10 `SyncStatus` Slice cell; ADR-0043 D11) (and `sync-now` skill via cron route instead); from S1.8 also on `/admin/mentions` — its one row is `mentions` (`MENTIONS_SYNC_SOURCES` in `lib/data/admin.ts`; `triggerSync({source:'mentions'})` runs `refreshMentions`; `DASHBOARD_SYNC_SOURCES` unchanged — ADR-0045 D11). |
| Auth | `requireRole('admin')` (ADR-0002 C7). |
...
| Item | Contract |
|---|---|
| Trigger | `/admin` dashboard video list (Hide / Show, and `is_short` override toggle) — per row a Hidden `Toggle`, a Short `Toggle` (a flip always sends an explicit boolean = an override) and, only on rows that carry an override, a ghost "Auto" button sending `is_short: null`; every successful submit redirects with `?saved=saved` → "Saved." (02 §1.3 `/admin` row; ADR-0043 D11). |
| Auth | `requireRole('admin')`. |
...
| Effects | Update `videos.hidden` and / or the Shorts pair (service role) — `is_short: true \| false` → `is_short_override` = `is_short` = the value; `is_short: null` → `is_short_override = null` and `is_short` recomputed by §5.3 `isShort` from the row's stored `duration_seconds` / `title` / `description` (ADR-0043 D1: the override lives in `videos.is_short_override boolean null`; `is_short` stays the effective column every reader filters on; no trigger). `syncYoutube` never writes `hidden` or `is_short_override`, and keeps an admin override by writing `is_short = is_short_override ?? isShort(v)` (§3.3 step 6). `revalidateTag('videos')`. SC-24 audit line. |
| Returns | `{ok:true, data:{youtube_id, hidden, is_short}}`. Errors: `forbidden`, `not_found`, `validation`. |
| Tests (05) | T-ACT-68; T-E2E-47 (the `/admin` leg — ADR-0043 D9). |
...
| `/api/cron/notify` | `notifyFanOut` then `notifyDeliver` (one run row — `lib/jobs/notify.ts` `runNotify` owns the row and the SC-13 lock and calls both with its `runId`; each job manages its own row only when called without one — ADR-0030 D2; summary `{ok, source:'notify', run_id, items: <delivered>, ms, fan_out:{ok, items, events, skipped, stale, errors[], ms}, deliver:{ok, items, failed, digests, skipped, claimed, deferred, errors[], ms}, error?}` — both steps run even when fan-out failed, `ok` = both ok, `error` = `'fan-out: …; deliver: …'`) | `notify` |
...
Common signature: `export async function <job>(opts: {runId?: string, trigger: 'cron'\|'manual', full?: boolean}): Promise<JobSummary>` where `JobSummary = {ok: boolean, source, run_id, items: number, ms: number, error?: string, skipped?: string \| number, [k: string]: unknown}` (`skipped` carries the SC-13/§3.2 string markers `'running'`/`'not_configured'` OR §3.1 step 5's numeric skipped-types count — the union reconciles this doc's own two uses; prose clarification, no rule change, 00 §1.5 F-2, 2026-08-27). Each job: acquire lock (SC-13) → insert `sync_runs` → work → finalize row (SC-11) → revalidate tags → on failure emit `sync.failed` per rule J-F below (**from S1.5** — `sync.failed`/`sync.stale` emission starts when `notifyFanOut` lands; jobs in S1.2 log the failure only via `log.error({job, id, msg})`, ADR-0002 A8) → return summary. The lock/insert/finalize/J-F sequence is implemented once in `lib/jobs/runner.ts` `runJob({source, opts, work})`; every job runs through it (ADR-0030 D1). When a caller supplies `opts.runId` the caller owns the `sync_runs` row: the callee runs no SC-13 lock check, inserts nothing, does not finalize and emits no J-F — the owner (`runNotify`) finalizes and emits for the combined outcome (ADR-0030 D2).

| # | Rule |
|---|---|
| J-F | `sync.failed` is **edge-triggered** (this doc owns the rule; 01 INV-71 defers to J-F/J-S; 05 T-ACT-74): emitted only when this run fails **and** the previous **finalized** `sync_runs` row for the same `source` (`ok IS NOT NULL`) has `ok = true` (or none exists); an unfinalized older row — a run killed at `maxDuration` or crashed before finalize — is skipped, not read as a failed verdict (it never had the chance to emit) — ADR-0030 D1, `lib/jobs/runner.ts`, `tests/db/jobs/runner.test.ts`. Payload `{source, run_id, error(≤300), started_at}`. 05 T-ACT-45 must seed the previous run `ok=true` (SEED-12 does). |
| J-S | `sync.stale` is emitted by `notifyFanOut` step F0 for each source in `('modrinth', 'youtube', 'curseforge'*, 'mentions'**)` with no `sync_runs` row `ok=true` in the last **6 h**, at most once per 6 h per source (dedupe: last `sync.stale` event for `subject_id = source` older than 6 h). Excluded: `stats` (daily cadence — a 6 h window is meaningless), `notify` (it is the emitter), `skins` (script only). *`curseforge` only when `CURSEFORGE_API_KEY` is set and ≥ 1 `project_links` curseforge row exists. **`mentions` only when `YOUTUBE_API_KEY` is set and ≥ 1 mention `refreshMentions` would refresh exists — `platform 'youtube'`, `external_id` not null, `status in MENTION_REFRESH_STATUSES` (`lib/jobs/constants.ts`; the §3.4 select exactly, so a hidden-only or id-less YouTube table is never stale — ADR-0045 D13). `subject_type = 'sync_source'`, `subject_id = syncSourceSubjectId(source)` (deterministic uuid — the column is uuid), payload `{source, last_ok_at, hours_since_ok}`; a source with no `sync_runs` row at all is never stale; the `mentions` condition is live from S1.8 (ADR-0045 D13); `hours_since_ok` is a one-decimal number, `null` (with `last_ok_at: null`) when the source has runs but none ok — F0 compares `finished_at` of the latest `ok=true` run (ADR-0030 D3). |
...
| J-D | Jobs never `.delete(` synced rows (01 INV-24). The only deletions in `lib/jobs/` are `snapshotStats` housekeeping: RPC `purge_project_downloads(90)`, `purge_rate_limit_hits(1)` and Storage `remove()` of orphan objects (U1) — 01 INV-24's grep exempts these. **ADR-0037 D3 — the one recorded exception:** RPC `fold_project(uuid, uuid)` (service-role only; invoked by `linkProjectListing`, never by a job) deletes exactly one duplicate `source='modrinth'` `projects` row, the `project_versions` rows merged into a canonical version (from the duplicate, or a synced row of that listing the sync had already re-parented onto the canonical after a failed fold) and CDN-only sha512-duplicate `project_files` rows, only when an admin links a listing the sync had already imported; the `lib/jobs` grep is unchanged. |

### 3.1 `syncModrinth` (S1.2) — hourly
| Item | Contract |
|---|---|
| Idempotency key | `projects (source='modrinth', external_id)`; `project_versions.external_id` (Modrinth version id); `project_files (version_id, filename)` for CDN-only rows (`storage_path IS NULL`) — unchanged by ADR-0037 D2, which first resolves **the target row X syncs into**: (i) a `project_links` row `{platform:'modrinth', external_id: X.id}` → its `project_id` (the canonical odsens row — "linked"); else (ii) the `projects (source='modrinth', external_id = X.id)` row (update) or a new one (insert). |
| Steps | 1. `adapters/modrinth.listUserProjects(MODRINTH_USER)` → full Project objects. `/v2/user/{user}/projects` returns full Project objects (gallery, body, license, links included), so the per-project `GET /v2/project/{id}` in data-model §5 is unnecessary — verified against fixture `user-projects.json`. 2. For each object with `status in ('approved','archived')` (Modrinth status enum: `approved, archived, rejected, draft, unlisted, processing, withheld, scheduled, private, unknown` — only the first two are publicly listable): map (§5.2) → upsert `projects` sync-owned columns: `slug, project_type, title, description, body_md, icon_url, gallery[{url,title,description,ordering,featured}], categories (Modrinth categories ∪ additional_categories), loaders, game_versions, license (license.id), source_url, issues_url, discord_url, downloads_modrinth, followers, published_at, external_updated_at, status:'published', synced_at`. Never touch `project_overrides`. **ADR-0034:** `slug` is `normalizeSyncedSlug(raw.slug, raw.title, raw.id)` — slugified; the title, then `p-<id>`, when the upstream slug fails the shared slug rule (a trailing dash, a reserved word) — never the raw Modrinth slug (D1); `gallery[].url` is the item's `raw_url` when present (the full-size original, not the `_350` thumbnail) (D4); `icon_url` is `resolveIconUrl(mapped.icon_url)` — the full-size original behind a resized `…_96.webp` icon — probed when the icon is new or changed (`iconBase(existing.icon_url) !== iconBase(mapped.icon_url)`) or while the stored value is itself still a resized icon (`isResizedIcon` — pre-rule rows upgrade once), otherwise the stored value is kept (D4). **ADR-0037 D2:** step 2 for a **linked** X (a `project_links` `modrinth` row names X.id) never inserts or updates a `projects` row's metadata — the odsens row's title, description, body, icon, gallery, categories, loaders, game versions, license, links and status are Oliver's; the job writes only `project_links.downloads/url/synced_at` and `projects.downloads_modrinth` (J-I compare; the canonical row's `synced_at` is untouched). Step 2 (ii) slug collision: the effective slug is the mapped one (`normalizeSyncedSlug`) unless another row that is not this listing's own holds it (the primary cross-posting case — Oliver's odsens `foo` exists when Modrinth approves `foo`), in which case the stored slug is kept on update and the ADR-0034 last-resort slug `p-<id lower>` is used on insert; no error is counted (the `/admin/projects` match note keys on the title as well). A hosted icon wins on any row: `icon_url` is left alone when the stored value is a Storage path (`project-media/…`). 3. `adapters/modrinth.listVersions(projectId)` → upsert `project_versions {external_id, version_number, name, changelog_md, game_versions, loaders, version_type, date_published, downloads}` and `project_files {filename, size_bytes, sha512 (hashes.sha512), url, primary, storage_path:null}`. Versions/files absent upstream are **not deleted** (DECIDED — ADR-0002 #66). **ADR-0037 D2:** versions key on `external_id` **globally** (not per project) — a matching row on another project is re-parented (`project_id` = the target; a version row follows its listing — this is what makes unlink and any stray state converge) and then treated as existing; no match → on the target row (linked canonical or the Modrinth-first row itself) a version with `external_id IS NULL AND version_number = V.version_number` **adopts** the Modrinth id (`external_id = V.id`) and is sync-owned from then on (`name, changelog_md, game_versions, loaders, version_type, date_published, downloads` follow Modrinth on every run; its hosted files stay); among same-numbered upstream candidates (ADR-0026 duplicates) the one carrying a file whose `sha512` equals a file of that hosted version adopts, else the first in adapter order; otherwise insert. Files: the `(version_id, filename)` match applies only to CDN-only rows (`storage_path IS NULL`); before inserting an upstream file the job checks `sha512` against every file of the version — a match means the same bytes live in both homes: no new row, and the matching row gains `url` (the CDN URL) when it had none (the ADR-0002 #42 CDN link, recorded on the row); a hosted row with the same filename but a different `sha512` is left alone and the upstream file lands as its own CDN row; `primary` is written on CDN-only rows only (`uploadProjectFile` manages it among hosted rows). 4. Any `projects (source='modrinth')` row whose `external_id` was not in the list this run **or** is named by a `project_links {platform:'modrinth', external_id}` row pointing at a different project (a duplicate the fold normally prevents — ADR-0037 D2) → `status='hidden'` (never delete); both computed from the list and the links, never from per-item success (J-P keeps a row that failed mid-item). Skipped only if step 1 failed. 5. Skipped Modrinth types (`modpack`, `shader`) counted in `summary.skipped`. |
| External calls | 1 list + N version calls (N ≈ 18) per run; ≤ 300 req/min limit is far away; sequential with 100 ms spacing. Plus ≤ 5 CDN `HEAD`s per new/changed icon (and once per pre-rule row still holding a resized icon) — `cdn.modrinth.com`, outside the API quota (ADR-0034 D4). |
| Revalidate | `projects`; `project:<slug>` for every upserted/hidden slug (none on a no-change run); a linked X revalidates `project:<canonical slug>` (never its Modrinth slug) plus the slug of any project a version was re-parented from (ADR-0037 D2). |
| Summary | `{items: upserted, hidden, skipped, versions, files, errors[]}` — a linked X counts in `items` only when a link, count, version or file row changed; a run with unchanged upstream data still changes no row but `synced_at` (ADR-0037 D2). |
...
| Steps | **Gather, then write (ADR-0043 D2 — was "RSS → upsert minimal rows" first, which would give a live / upcoming stream a permanent public stub under J-D).** 1. Read the existing rows (`youtube_id`, `is_short_override`). 2. RSS `https://www.youtube.com/feeds/videos.xml?channel_id=${YOUTUBE_CHANNEL_ID}` (keyless; Atom XML through `fetchText`, SC-09) → ids, titles, published dates. 3. If `YOUTUBE_API_KEY` set and (`full` **or** `videos` table empty): walk `playlistItems.list part=contentDetails playlistId="UU"+channelId.slice(2)` (50/page; page cap + repeated-token guard) to collect all ids. 4. If `YOUTUBE_API_KEY` set: `videos.list part=snippet,contentDetails,statistics id=<all known ids in batches of 50>` → `title, description, thumbnail_url (maxres ▸ standard ▸ high ▸ medium ▸ default), duration_seconds (ISO 8601 parse), view_count, like_count, published_at`; live / upcoming items are dropped inside the adapter (§4.3; ADR-0002 #77). 5. **Writes happen only now, and only if every list call above answered** — RSS, `playlistItems` or `videos.list` failing = zero rows written, run `ok=false`, J-F (T-ACT-45, T-ACT-74). Keyed run: write the mapped videos the run asked about, once each (a mapped id outside the asked set — existing ∪ RSS ∪ walk — is ignored); a new id is inserted only when `videos.list` returned it mapped (so a live / upcoming upload never gets a row — it arrives on a later run once it has ended), a known row is updated only when a synced column differs — otherwise only its `synced_at` moves (J-I; sent in chunks of ≤ 50 ids) — and a stored row the API did not return is left untouched, `synced_at` included (ADR-0043 D18). An insert is an `upsert` on `youtube_id` with `ignoreDuplicates` (a row that appeared since step 1 is left alone, never overwritten); a known row is a plain `update` (ADR-0043 D18). No-key run: insert RSS-minimal rows (`title, published_at, thumbnail_url = https://i.ytimg.com/vi/<id>/hqdefault.jpg` — built by the job from the id, whatever host the feed's `media:thumbnail` names; 01 INV-54) — the feed carries no live flag, so a no-key run may store such a row for a live / upcoming upload, and later keyed runs leave it untouched (J-D; Oliver can hide it — ADR-0043 D18); on a known row an RSS-only run writes only `title` / `published_at` and the effective `is_short` recomputed from the feed title over the **stored** duration and description (step 6) — it never downgrades a stored row's Data-API fields (`description`, `duration_seconds`, counts, the best thumbnail) (ADR-0043 D18). 6. `is_short = is_short_override ?? isShort(v)` (§5.3; ADR-0043 D1). The override is read at step 1, so each row update is guarded on the override **as read** (`youtube_id` + that value); when `updateVideo` changed it mid-run no row matches and the payload is resent without `is_short` — the admin's flag stands and the next run reconciles (ADR-0043 D21). A keyed `thumbnail_url` is stored only when it is an `https://i.ytimg.com/` URL, else the `hqdefault` literal (01 INV-54; D21). Videos absent from a `full` walk are **not** deleted; `hidden` and `is_short_override` are owned by `updateVideo` (§1.8, ADR-0002 #20) and are never part of a sync payload. |
| Quota | Per run: 1 RSS (0 units) + ceil(N/50) `videos.list` (1 unit each) ≈ 1 unit; `full` walk adds ceil(N/50) units. Daily ≈ 25–50 units of 10,000. If `YOUTUBE_API_KEY` missing → RSS-only, `summary.degraded='no_key'`, run `ok=true`. `summary` extras: `inserted`, `updated`, `rss`, `walked`, `full`, `units`, `errors[]` (+ `degraded` on a no-key run only); 00 S1.6.AC9's "log line with units" is the runner's own `done` / `failed` line, whose `meta` carries `units`, `inserted`, `updated`, `errors` (a count) (+ `degraded`) — `units` is reported on the failure path too, since a failed `videos.list` still cost one (ADR-0043 D18). |
| Revalidate | `videos` — only when a row was inserted or changed; none on a no-change run and none when a list call failed (the §3.1 / §3.2 rule, 05 T-ACT-51; ADR-0043 D18). |
| Tests (05) | T-ACT-53; T-ACT-74 (youtube — 00 S1.6.AC11, ADR-0043 D10); T-ADP-9, T-ADP-10, T-ADP-11, T-ADP-12, T-ADP-13; T-UNIT-29. |

### 3.4 `refreshMentions` (S1.8) — hourly
| Item | Contract |
|---|---|
| Steps | Gather, then write (ADR-0045 D12; the ADR-0043 D2 posture): 1. select `mentions where platform='youtube' and external_id is not null and status in MENTION_REFRESH_STATUSES` (= `('draft','published')`, `lib/jobs/constants.ts`), paged at 1,000; 2. `youtube.listVideoStats(<distinct ids>)` — `videos.list part=statistics`, batches of ≤ 50, one unit each; ANY batch failing fails the whole gather (nothing from the batches that answered is written); 3. decide: a row changes only when the API answered ITS id with a non-null count that differs from the stored one; 4. read the changed rows' `projects.slug` (chunks of 50); any of 1–4 failing → `ok=false`, zero writes, no tags (J-F emits); 5. write `view_count` one row at a time, each guarded on the `external_id` AS READ (a row `updateMention` re-pointed meanwhile keeps its number — no match → `unchanged`), per-row errors counted in `summary.errors[]` (≤ 20) and never rethrown, `ok=false` only when > 50 % of the attempted rows failed. Requires `YOUTUBE_API_KEY` — else the run still writes its `sync_runs` row (`ok=true`, `error 'not configured'`) and returns `{ok:true, items:0, skipped:'not_configured'}` with no adapter, no request, no read. Non-YouTube mentions are never refreshed in v1. `summary` = `{items: <rows updated>, mentions, asked, returned, updated, unchanged, units, errors[]}`; the runner's `done` / `failed` line carries `meta.units` on the failure path too, never the key (SC-15). |
| Idempotency | `mentions.id`; overwrite `view_count` only when the API returns the id with a non-null count that differs — a missing id (deleted / private), a hidden count (`view_count: null`) and the same number are all `unchanged`: a stored number, possibly typed by the admin in `createMention`, is never overwritten with NULL (ADR-0045 D12). `mentions` has no `synced_at`, so a no-change run writes nothing (J-I). The only column this job writes is `view_count`. |
| Revalidate | `mentions` once + `project:<slug>` for mentions whose `view_count` changed — only when ≥ 1 row changed (the §3.1–§3.3 rule; a general mention adds no project tag). |
| Tests (05) | T-ACT-54; T-ADP-13 (`listVideoStats`); T-ACT-71 (no key); T-ACT-74 (`refreshMentions` leg — J-F edge + delivery, 00 S1.8.AC7; ADR-0045 D12); T-ACT-33 / 70 (`/api/cron/refresh-mentions`); T-ACT-42 (`triggerSync` — D11). |
...
| F3 | Result: every event has ≥ 2 recipient rows after F2 → F1 never re-selects it (idempotency key = `(event_id, channel, coalesce(address,''))`; unique index in schema, data-model §2). As built (ADR-0030 D2): F0 is per-source J-P (an error is counted, the tick continues); F1 is the PostgREST embedded-null anti-join (`notification_recipients=is.null`); the discord address is `site_settings.discord_webhook_url` else the `DISCORD_WEBHOOK_URL` env fallback (SC-16, DB wins); F3 = one batch insert per event, a 23505 falls back to per-row inserts skipping 23505; summary `{items: <recipient rows created>, events, skipped, stale, errors[]}`. |
...
| N2 | Group **per channel** (email: per `address`; discord: one group per webhook address — one group when one webhook is set) — refines notifications.md "per channel". If a group has **> 5** eligible rows → one **digest** message (subject/title "N things from the allay", list of kind + target + excerpt, link `/admin/comments` or `/admin/settings`); else one message per row. Digest bodies: `emails/templates/Digest.tsx` (email) / one embed of ≤ 25 lines (discord) — ADR-0030 D7; the digest's kind word is the N6 event word (`DIGEST_KIND_LABELS` in `Digest.tsx` = `lib/notify/deliver/content.ts` `EVENT_LABELS`, T-UNIT-3 asserts parity). As built: `DELIVER_TIME_BUDGET_MS` is measured from the TICK's start — the owner's `sync_runs.started_at` when nested under `runNotify` (one PK read, best effort: an unreadable row falls back to the step's own start), else the run's own start — and checked before every send unit (a digest, or one row of a single-mode group), not per (channel, address) group, because one SC-09 send can take ~47 s. |
...
| N4 | Mark: sent → `status='sent', sent_at=now(), attempts+1`; failed → `attempts+1, error(≤500)`; `status='failed'` when `attempts` reaches 5 (max 5 — notifications.md). Digest marks all its rows together. As built: delivery is **at-least-once** — a row is marked only after the adapter answered, so a tick killed at `maxDuration` (or a failed status write) re-sends the unmarked unit next tick; each unit is marked right after its own send; a digest marks every row even when one write fails, then the run fails (`ok=false`, error = the write) and no further unit is sent this tick; rows not reached stay `pending` and are not counted as `deferred`. |
| N5 | Email content: React Email templates in `emails/` (`CommentNew`, `CommentHeld`, `CommentReported`, `SyncFailed`; digest uses `EmailLayout` with a list) + plain-text alternative always; From `odsens <${NOTIFY_FROM_EMAIL}>`; `Reply-To` = `NOTIFY_FROM_EMAIL` **only after** inbound forwarding exists (questions.md setup to-do; until then no Reply-To header); subject formats: `comment.new` "New comment on <target title>" · `comment.held` "Held for review: <target title>" · `comment.reported` "Reported comment on <target title>" · `sync.failed` "Sync failed: <source>" · `sync.stale` "Sync stale: <source>" · digest "<N> things from the allay" (05 T-UNIT-26). Every email footer: "The allay emails you because <switch> is on." + link `${NEXT_PUBLIC_SITE_URL}/admin/settings`. As built: the `SyncFailed` `runAt` is `relativeTime(started_at | last_ok_at)`; a source with no ok run ever (`EventContent.runAt = ''`) reads "No good run yet." in place of the stale lead line and omits the `Last run / Last good run <runAt>.` meta line — the word "never" is not used (ADR-0030 D19); the title half of every subject is clipped so the whole subject is ≤ 60 code points incl. the ellipsis (03 E-06, `SUBJECT_MAX`); comment events missing `target_title`/`target_slug` are hydrated from `projects_public` by `target_id` per tick, and a payload with no target reads title `odsens` and links `/admin/comments`. |
| N6 | Discord embed (§4.6): `username:'allay'`, `avatar_url: ${SITE_URL}/brand/allay.png` (asset pending Q44 — omit field until the file exists), embed `{title: '<Event> — <target title>', description: excerpt(200), url: link, color}` with color indigo `0x4B45D6` default · gold `0xFFC61F` for `comment.held`/`comment.reported` · alert `0xCC3A2A` for `sync.failed`/`sync.stale`. As built: embed `url` = the comment on the site for `comment.new`, `/admin/comments` for `comment.held`/`comment.reported`, `/admin` for `sync.*`; description = `@handle: "…"` (held first-time: `First comment from @handle: "…"`; reported adds `<n> reports · <reason>`; sync = the allay lines + optional `Cause: …`); digest lines `<Event> — <title>: "<excerpt>"` (≤ 25) + `…and N more in admin.`; a scrubbed author reads `a deleted account`; a sync event's single-embed title is `<Event> — <Source>` (bare source name, DESIGN.md §12.1 frame) while the mail h1 keeps `<Source> sync`; a stale source with no ok run reads `<Source> counts haven't updated yet.` (ADR-0030 D19); every comment excerpt has its URLs removed at the source (`content.ts` `stripLinks`, the §1.2 B3 pattern; leftover spaces collapse; a link-only excerpt quotes nothing) so the description matches the mail (03 E-05); the excerpt, the handle, the reported reason, the digest project title and the `Cause:` text are Discord-markdown-escaped (`escapeDiscordMarkdown` — backslash before `\ * _ ~ \` \| > [ ] ( )` and line-leading `#` / `-`) so masked links, spoilers, code and emphasis render literally; embed titles are not markdown-rendered and stay raw. |
...
| Summary | `{items: sent, failed, digests, skipped}` + `claimed`, `deferred` (rows left pending by the `DISCORD_PER_TICK` cap or `DELIVER_TIME_BUDGET_MS` — the budget is read from the tick's start, N2; rows a failed status write stopped the tick from reaching stay `pending` and are not `deferred`, N4), `errors[]`; `skipped` = N7 rows, `digests` = digest messages sent; a per-row send failure never fails the run (J-P) — only the claim read / a status write does (ADR-0030 D2). |
...
| Adapter | Base URL (overridable in tests only via `*_API_BASE`, ADR-0002 #73 — except `oembed`, which has no override of any kind, ADR-0045 D6) | Auth | Functions (export names — `_registry.md`) | Timeout / retry | Quota / limits | Fixtures |
|---|---|---|---|---|---|---|
| **4.1 `modrinth`** (`createModrinth`) | `https://api.modrinth.com/v2` | none; `User-Agent` SC-10 **required** | `listUserProjects(user)` → `GET /user/{user}/projects` · `listVersions(projectId)` → `GET /project/{id}/version` · `mapProject(raw) → ProjectRow` (§5.2) · `mapVersion(raw) → {version, files[]}` · `mapProjectType(project_type, loaders)` · `resolveIconUrl(iconUrl)` → the original behind a resized icon (HEAD `<base>.png|.jpg|.jpeg|.webp|.gif`, 5 s each, no retry; unchanged input when none answers 200 — ADR-0034 D4) · pure exports `normalizeSyncedSlug(slug, title, id)`, `iconBase(url)`, `isResizedIcon(url)` (ADR-0034 D1/D4) · `getProject(idOrSlug)` → `GET /project/{id\|slug}` (SC-09; 404 → `not_found`) · pure export `parseModrinthRef(ref) → {ref} \| null` (a `modrinth.com` project URL, a bare slug or a bare id `/^[A-Za-z0-9._-]{1,64}$/`; dots-only refs (`.`, `..`) and any other host → `null`) — ADR-0037 D1/D10; `modrinthListingUrl(id)` (= `https://modrinth.com/project/<id>`) lives in `lib/format/project.ts` and the adapter imports it | SC-09 | 300 req/min; honour `X-Ratelimit-Remaining/Reset` (sleep until reset when remaining < 5) | `tests/fixtures/modrinth/user-projects.json`, `versions.json`, `project-shader.json`, `error-429.json`, `project/sd000101.json` (alias of the `sd000101` object), `project/sd000199.json` (a listing absent from the 18-project list — ADR-0037 D10; both fixture servers serve `GET …/project/<id>` from `<path>.json`) |
...
| **4.3 `youtube`** (`createYoutube`) | RSS `https://www.youtube.com/feeds/videos.xml` · Data API `https://www.googleapis.com/youtube/v3` · oEmbed `https://www.youtube.com/oembed` (test override `OEMBED_BASE` — this endpoint ONLY, never the §4.4 page read; ADR-0045 D6) | RSS none · Data API `key=${YOUTUBE_API_KEY}` (redacted from errors/logs) | `fetchRss(channelId)` → `[{youtube_id, title, published_at, thumbnail_url, description}]` (`thumbnail_url` = the feed's `media:thumbnail@url` verbatim, `null` when an entry has none — the job never stores it, §3.3 step 5; ADR-0043 D17) (Atom XML read through `fetchText` and parsed by a hand-rolled parser in this file, exported as `parseRss(xml)` — no XML dependency; malformed → typed `parse_error`; ADR-0043 D3) · `listVideos(ids[])` (batches ≤ 50, `part=snippet,contentDetails,statistics`) → `Promise<MappedVideo[]>`, a plain list: live / upcoming items (`snippet.liveBroadcastContent !== 'none'`) are dropped inside, so for the job "not returned" = "not inserted" (ADR-0043 D2) · `listUploads(channelId)` (playlistItems paging; page cap + repeated-token guard) · `channelStats(channelId)` → `{views, subs}` (`subs` is `null` when the channel hides its subscriber count — never a made-up 0; ADR-0043 D17) · `parseDuration(iso8601) → seconds` · `pickThumbnail(thumbnails)` · `isShort(v)` (§5.3) · `mapVideo(item)` · `oembed(url)` → `GET https://www.youtube.com/oembed?url=<enc>&format=json` (no key) · `videoIdFromUrl(url)` (handles `watch?v=`, `youtu.be/`, `/shorts/`, `/live/`, `/embed/`) · `listVideoStats(ids[])` → `Promise<VideoStats[]>` (`{youtube_id, view_count: number \| null}`; `part=statistics` only, batches of ≤ 50 strictly sequential, one unit each; ids that are not 11 url-safe chars are never sent, duplicates asked once, live / upcoming items KEPT, a hidden count → `null`, an unanswered id absent — what `refreshMentions` calls, §3.4; ADR-0045 D7) · `getVideoMeta(id)` → `Promise<VideoMeta \| null>` (`part=snippet,statistics` for ONE id, 1 unit; answers only for the item whose `id` IS `id` — never `items[0]`; a malformed id → `null` with no request; a matched item without a title → `parse_error` — §5.4 step 3; ADR-0045 D7) · exposes `unitsUsed` and `hasKey` (without `YOUTUBE_API_KEY` the keyed calls throw `AdapterError` `unsupported` with no request and no unit — SC-16) | SC-09 | 10,000 units/day; each `list` call 1 unit | `youtube/rss.xml`, `rss-malformed.xml`, `videos-list.json`, `videos-mentions.json`, `playlist-items.json`, `oembed.json`, `channels.json` (+ the e2e fixture-server aliases `videos.json`, `playlistItems.json` — `tests/fixtures/README.md`; hand-made, ADR-0043 D8), `videos/seedvid0009.json` (the single-`id` file the e2e mention preview pastes — served by the fixture server's single-`id` rule, ADR-0045 D28) |
| **4.4 `oembed`** (`createOembed({fetch, env, lookup})` — env needs only `MODRINTH_USER_AGENT`; `lookup` = the injectable resolver, default `node:dns/promises` `lookup` with `all: true`) | any — **no base override of any kind** (ADR-0045 D6: the one request in the app whose URL somebody else chose; unit tests inject `fetch` + `lookup`; no env value, `E2E` or `NODE_ENV` ever relaxes the guard) | none | `assertPublicHost(url)` → the NORMALISED `URL` to request; rejects in order: > 2048 chars / unparseable; scheme not `http(s)`; userinfo; `http:` upgraded to `https:` and any explicit port that survives (every hop is https on 443); trailing dots stripped, fragment dropped, empty host; an IP literal → `isPublicAddress` (no DNS); a name on the deny-list (`localhost`, `*.localhost`, `*.local`, `*.internal`, `*.home.arpa`, `*.lan`, `*.intranet`, `*.corp`, `*.home`, any single-label host); DNS under a 3 s cap (`DNS_TIMEOUT_MS`) — resolver failure → `network_error`, no address or ANY non-public address → `rejected`. `isPublicAddress(ip)` (exported): IPv4 deny-list (this-network, RFC1918, CGNAT, loopback, link-local incl. `169.254.169.254`, protocol assignments, the three TEST-NETs, 6to4 relay, benchmarking, multicast, reserved + broadcast), IPv6 ALLOW-list (`2000::/3` minus `2001::/23`, `2001:db8::/32`, `2002::/16`, `3fff::/20` — so `::1`, IPv4-mapped, NAT64, ULA, link-/site-local and multicast are refused) · `fetchOpenGraph(url)` → guard → `http.ts` `fetchPage` with `redirect: 'manual'`, `retries: 0`, `maxBytes` 1 MB (`OG_MAX_BYTES`), `contentTypes` html / xhtml, `Accept: text/html`; follows ≤ 3 redirects (`OG_MAX_REDIRECTS`), each `Location` resolved against the current hop and sent through the guard again; too many / looping / location-less redirects, an oversized body or a non-HTML answer → `unsupported`; parses `og:title, og:image, og:site_name, og:url, og:type, article:published_time, <title>` → `{title, image, site_name, canonical, published_at, og_type}` (PARSED FIELDS ONLY — never the body; `image` / `canonical` kept only as clean `https:` URLs under the same policy; no title at all → `parse_error`); every error names the page by ORIGIN only with an empty `body` · `parseOpenGraph(html, url)` (exported; hand-rolled, LINEAR — one forward scan, 4,096-char tag cap, comments and raw-text elements skipped, first non-empty value per key, `property` or `name`, UTF-8 only — no HTML dependency, 01 INV-78) · `detectPlatform(url)` → hostname map on a dot boundary (a domain and its subdomains; `evilyoutube.com` → `article`): `youtube.com\|youtu.be → youtube`, `tiktok.com → tiktok`, `twitch.tv\|clips.twitch.tv → twitch`, `reddit.com\|redd.it → reddit`, else `article` (05 T-ADP-16; the admin can change the platform in the form; `other` is reachable by hand) · also exported: `OG_MAX_BYTES`, `OG_MAX_REDIRECTS`, `OG_TIMEOUT_MS`, `DNS_TIMEOUT_MS`, types `DetectedPlatform`, `LookupAddress`, `Lookup`, `OpenGraph`, `OembedEnv`, `Oembed`. DNS rebinding (check-time vs connect-time lookup) is an accepted residual risk — no pinning without a new dependency or a second HTTP path; neutralised in practice by https-only, all-addresses checking, no retry, parsed-fields-only output and the admin-only, rate-limited caller (ADR-0045 D6) | ONE 10 s budget (`OG_TIMEOUT_MS`) for the whole redirect chain, **no retry** (interactive); DNS 3 s | — | `oembed/og-page.html`, `no-og.html`, `tiktok.html` (unit / db inputs only — the app never requests anything under `oembed/` from the fixture server) |
| **4.5 `resend`** (`createResend`) | `https://api.resend.com` — a plain `POST /emails` through `fetchJson` (no `resend` SDK; the base URL must stay injectable — ADR-0030 D6; the deliverer passes the rendered `html` + `text`, no `react` param; a blank `html`/`text` is `rejected` before any request; 422 → `rejected`, no retry; 2xx without `id` → `parse_error`) | `RESEND_API_KEY` | `sendEmail({to, subject, react, text, from, replyTo?, headers:{'X-Entity-Ref-ID': recipient_row_id}})` → `{id}`; `from` defaults `odsens <${NOTIFY_FROM_EMAIL}>` (05 T-ADP-17; the template → `react`/`text` mapping lives in `deliver/email.ts`) | 10 s; retry per SC-09 (Resend 429) | free tier 3k/mo; ≤ 2 req/s → deliverer sends sequentially | `resend/send-ok.json` (mocked SDK) |
| **4.6 `discord`** (`createDiscord`) | webhook URL passed per call (the row's `address` / the tested URL) | URL is the secret; never logged; regex in §1.3 | `postEmbed(url, {title, description, url?, color, fields?})` → `POST {url}?wait=true` body `{username:'allay', avatar_url?, embeds:[…]}` → `{status}`; 429 → respect `retry_after` (ms) once (05 T-ADP-18; Discord's live payload uses seconds — follow-up, ADR-0030 D9; wait order body `retry_after` → `Retry-After` header (s) → 1 s, capped 30 s; a URL outside the §1.3 regex → `AdapterError 'unsupported'` with no request; the module also exports `DISCORD_COLORS`, `discordWebhookUrlPattern`, `DISCORD_USERNAME`, `maskWebhookUrl` (an alias of `lib/format/secret.ts` `maskSecret` — one implementation, ADR-0030 D12), `rewriteDiscordBase`) | 10 s; SC-09 | Discord webhook ~30 req/min per webhook — deliverer sends ≤ `DISCORD_PER_TICK` (§5.8, default 20) per tick | `discord/webhook-ok.json` |
...
Rule (matches data-model §2.3): `is_short = duration_seconds <= 60 || /\B#shorts\b/i.test(title + ' ' + description)`. Rows: `PT45S` no tag → true · `PT61S` no tag → false · `PT2M` with `#Shorts` in description → true · `PT10M` → false · `duration_seconds` null → false (the rows win over the literal formula: `duration_seconds == null` is guarded first, since `null <= 60` is `true` in JS — prose clarification 2026-09-18, no rule change; so an RSS-only row is never a Short). Refinement (e.g. `HEAD https://www.youtube.com/shorts/<id>` 200-vs-redirect probe, or duration ≤ 180 s) requires an ADR with slug `shorts-detection` (number assigned per 06 README). Admin override of `is_short` = `updateVideo` (§1.8), stored in `videos.is_short_override` (ADR-0043 D1). Tests (05): T-ADP-11.
...
| 3 | youtube and `YOUTUBE_API_KEY` set and a readable video id | `youtube.getVideoMeta(id)` (one id, strict id match, 1 unit — ADR-0045 D7; was `listVideos([id])`) | `view_count = statistics.viewCount, published_at = snippet.publishedAt`; fills any field missing from 2 (title, channelTitle → creator_name, `https://www.youtube.com/channel/<channelId>` → creator_url, best thumbnail) — step 2 wins the title (D8) | continue to 4 with what we have |
...
| 5 | always | normalise: trim, title ≤ 200, creator ≤ 80, `creator_url` / `thumbnail_url` https and ≤ 512 else `null`, `canonical_url` = `canonicalMentionUrl`, a YouTube thumbnail = an `i.ytimg.com` URL else the `hqdefault` literal (ADR-0045 D7 / D8) | return `data.source` = `data_api` when step 3 returned the video, else `oembed`, else `og` (ADR-0045 D8) | — |
...
| `linkProjectListing` / `unlinkProjectListing` (was `setProjectLink` — ADR-0037 D1) | `project_link` | 30 / h per user (one shared scope) | `rate_limit_hits` |
...
| `tip_click` | `{amount?: 1\|3\|5\|'other', from: 'support'\|'tip-panel'\|'floating'}` (binding shape — ADR-0002 A16; `amount` = the chosen preset, or the literal `'other'` when the "Other" input is used — the typed value is never sent; absent when no amount is chosen, e.g. `FloatingSupportButton`, `TipPanel`). **Since ADR-0042 no emitter sends `amount`** — the picker is gone and `KofiCard` sends `{from:'support'}`; the key stays in the allowlist shape (T-UNIT-38) for the day an amount exists on our side again. The nav Support button is a plain `<a href="/support">` and emits nothing (03 N-04) | `KofiCard` (`support`), `TipPanel` (`tip-panel`), `FloatingSupportButton` (`floating`) |
| `video_play` | `{youtube_id: <id>, kind: 'video'\|'short'\|'mention'}` (`kind` = what is playing, not the placement — the big player an up-next row selects plays a `video`; the row itself never plays and there is no `upnext` kind — ADR-0043 D5) | `VideoFacade` on play (every variant that plays — `upnext` is presentational and never plays; ADR-0043 D5) |
...
| TIP ON KO-FI (was CONTINUE ON KO-FI — ADR-0042) | Replaces `KofiCard`'s gold slab with the `KofiPanelSlot` iframe **in place** (no new tab; "← Back" returns the slab): `https://ko-fi.com/<kofi_page>/?hidefeed=true&widget=true&embed=true`, 712/620 px, click-to-load, `/support` only (a code rule — 01 INV-58; the CSP `frame-src` entry for `https://ko-fi.com` is part of the site-wide baseline, 01 §20 — ADR-0041 D1). URLs come from `lib/support.ts` `kofiEmbedUrl(page)` / `kofiPageUrl(page)` (page name `encodeURIComponent`-ed; 05 T-UNIT-54). **There is no amount on our side** (ADR-0042 D1; closes ADR-0002 #50's "verify when the account exists"): checked 2026-09-18 against the live panel — it opens at Ko-fi's own default whatever the URL says (`amount`, `donationAmount`, `value`, `coffees`, `qty`, `a`, `price` all ignored), its page reads neither `location.search` nor `message` events, and Ko-fi's only developer surface is the payment webhook (§9.1). The default and suggested amounts are set by the creator in Ko-fi's own settings. |
| "on Ko-fi ↗" ghost link | Opens `https://ko-fi.com/<kofi_page>` in a new tab (`rel="noopener noreferrer"`). |
| Empty `kofi_page` | (`lib/support.ts` `normalizeKofiPage` — null / empty / whitespace) the TIP ON KO-FI button disabled, the line "Tips open soon." (`--gold-ink` on gold — ADR-0041 D5), no "on Ko-fi ↗" link, no panel (00 S1.9.AC4; DESIGN.md §12.7 build clarifications — not the C20 placeholder copy). |
...
| `banned` | 403 | `profiles.is_banned` — comments UI "You can't comment here."; account actions (`lib/auth.ts` `require*`) "This account is banned.", and the browser is kept on `/banned` (ADR-0019) |
...
| §1.4 projects, uploads (§1.4.5 U1–U3) | T-ACT-34…41, 73; T-RLS-17, 19, 41, 42, 117, 119; T-UNIT-17, 18, 20, 22, 36; T-E2E-34, 35; ADR-0037 (S1.5a): T-ACT-79…81 (`linkProjectListing` / `unlinkProjectListing` / the fold), 83 (uploads on synced rows), 84 (`curateProject` stored entries); T-RLS-135…137; T-ADP-22; T-UNIT-49, 50; T-E2E-51…53 |
| §1.5 skins, art | T-ACT-56…61, 73; T-UNIT-19, 45 (`render-skins.mjs`); T-RLS-54; T-E2E-38 |
| §1.6 mentions | T-ACT-62…64; T-ADP-14…16, T-ADP-1 (`fetchPage`), T-ADP-20 (`oembed` has no override); T-RLS-103…105, T-RLS-129 (`reorder_mentions` — ADR-0045 D2); T-E2E-39 |
| §3.4 refreshMentions | T-ACT-54, 71, 74 (`refreshMentions` leg), 33 / 70 (the route), 42 (`triggerSync` `mentions`); T-ADP-13 (`listVideoStats`) — ADR-0045 D11 / D12 |
| §1.7 triggerSync · §1.8 updateVideo | T-ACT-42, 70; T-E2E-41 · T-ACT-68; T-E2E-47 (the `/admin` Hidden-toggle leg — ADR-0043 D9) |
...
| §3 jobs | T-ACT-45…56, 70…75; T-ADP-1…13, 20; T-ACT-82 (`syncModrinth` adoption — ADR-0037 D2) |
...
| OPEN-7 | Admin video hide / `is_short` override | **DECIDED (ADR-0002 #20)** — `updateVideo` (§1.8) from the `/admin` dashboard list; no `/admin/videos`. The override is stored in `videos.is_short_override` (ADR-0043 D1); the list's controls are ADR-0043 D11 |
