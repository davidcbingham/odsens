...
Not yet applied (tracked in §12 "still disagree"): 01 INV-40 / 02 §5 / 05 T-ACT-27 `updateSettings` tags; 05 T-E2E-49 `from:'fsb'`; `.env.example` `CURSEFORGE_MEMBER` (S0) + `HASH_SECRET=` (S1.3); `_registry.md` Comments row + `app/auth/` route files; 00 S0/S1.4 wording.
...
- `.env.example`: drop `CURSEFORGE_MEMBER=` at S0; add `HASH_SECRET=` at S1.3 (SC-16/17; T-UNIT-35 env parity).
