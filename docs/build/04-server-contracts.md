...
| Input (`linkProjectListingInput`) | `{project_id: uuid, platform: z.enum(['modrinth','curseforge']), ref: string 1..300}`. `curseforge`: `ref` is either digits (`/^\d{1,10}$/`) or a CurseForge URL `^https://(www\.)?curseforge\.com/minecraft/(mc-mods|texture-packs|data-packs|bukkit-plugins|modpacks|shaders)/([a-z0-9-]+)` (`adapters/curseforge.parseRef`). `modrinth`: `ref` is a Modrinth URL (`https://(www.)?modrinth.com/(mod\|plugin\|datapack\|resourcepack\|shader\|modpack\|project)/<slug-or-id>[/…]`), a bare slug or a bare id (`/^[A-Za-z0-9._-]{1,64}$/`), parsed by the pure adapter export `parseModrinthRef(ref) → {ref} \| null` — any other host → `validation`. Removing a link is `unlinkProjectListing` (the former `ref: null` clause). |
