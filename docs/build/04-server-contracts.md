...
| Item | Contract |
|---|---|
| Trigger | `SyncStatus` "Sync now" buttons in `/admin/projects` (modrinth, curseforge) and on the `/admin` dashboard (since S1.2; from S1.6 its rows are modrinth, curseforge and youtube — `DASHBOARD_SYNC_SOURCES` in `lib/data/admin.ts`, 03 §2.10 `SyncStatus` Slice cell; ADR-0043 D11) (and `sync-now` skill via cron route instead). |
| Auth | `requireRole('admin')` (ADR-0002 C7). |
