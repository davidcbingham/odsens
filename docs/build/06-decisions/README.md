...
| ADR-0028 | S1.4 contract clarifications (deleted-slot rule, report preconditions, counter triggers vs `profiles_guard`, schema helper names, `commentBodySchema` home, `ModActionRow surface`, banned reach via the proxy, moderator merge `public_profiles` read, rename-handle schema, `stripHtml` script blocks, optimistic insert only under `auto`) | Accepted | 2026-09-03 | S1.4 | 00 §S1.4.AC8, AC10, §S1.4 Scope-IN + Risks, §6 + Status line; 01 INV-66 + §1 folder list + module-ownership prose + Status line; 04 §1.2 `reportComment` Preconditions + Body rules + `renameUserHandle` Input + `postComment` Effects + Status line; 03 §1.4 C-17 (5), §2.4 `CommentThread` + `ModActionRow` + Status line; 02 §2.3 + Status line; 05 §5 T-UNIT-4, §7.5 T-E2E-28/30 + Status line; `docs/data-model.md` §2.5, §2.11, §4; `_registry.md` SQL + Modules lines; `docs/questions.md` S1.4 notes |
| ADR-0029 | Builds wait for the database schema (`scripts/wait-for-schema.mjs`, RPC `migration_versions()`) | Accepted (2026-09-11, v0.6) | 2026-09-03 | S1.5 | 05 §4 CI-4, T-RLS-129, T-UNIT-46, §8 S1.5 + Status line; `_registry.md` scripts + SQL lines; 00 §6 + Status line; `ship` step 10; `vercel-ops` troubleshooting; `docs/dev-tooling.md`; `docs/questions.md` S1.5 notes |
| ADR-0030 | S1.5 contract clarifications (notify orchestration, shared runner, `sync.stale` subject, payload retention, settings composition, adapters over `fetchJson`, `Digest`, fixture-server POST); D14–D20 build-pass and gate-round letter (settings-action/email/schema letter, moderator 404 interim, COV-4 combined run, settings page as built, D20 gate round 1: 01 §1 tree + INV-78, 02 §1.3 entry button, 05 T-E2E-35, `DELIVER_TIME_BUDGET_MS` 12000) | Accepted (2026-09-11, v0.6) | 2026-09-03 | S1.5 | 04 SC-09, SC-22, §1.1 `deleteAccount`, §1.3 `setUserRole`, §2.4, §3 preamble, J-F, J-S, §3.7 N2/N4/N5/N6, §4.5, §4.6, §5.8 + Status line; 01 INV-71; 03 C-03, §2.2 `Toggle`/`Chip`, §2.10 `NotificationMatrix`, §2.11 rows, E-01/E-03/E-05/E-06/E-07 + Status line; 02 §2.8 + Status line; 05 F-3, SEED-12, H-4, COV-4, T-RLS-129, T-ACT-30/32/52/65/66/72/74, T-ADP-18/19, T-UNIT-3/26, T-E2E-37 + Status line; `docs/data-model.md` §2.6, §2.11; `docs/notifications.md`; `_registry.md` Jobs, Modules, Adapters, Notify, Email lines; 00 §6 + Status line; `docs/questions.md` S1.5 notes |
| ADR-0031 | Sign-out's CSRF check compares `Origin` against the request's own host (production logout returned 403) | Accepted (2026-09-11, v0.6) | 2026-09-06 | cross-cutting | 04 §2.2 + §7 env matrix + Status line; 02 §4 Sign-out + §1.2 route table row + Status line; 05 §7.2 T-ACT-9 + Status line; `docs/questions.md` 2026-09-06 |
| ADR-0032 | The crown alone is the site mark; the full OddSense character is Oliver's account picture | Accepted (2026-09-11, v0.6) | 2026-09-06 | cross-cutting | `DESIGN.md` header changelog (v1.6), §2 Palette, §10 Assets |
| ADR-0033 | The admin header carries the odsens mark as the way back to the public site | Accepted (2026-09-11, v0.6) | 2026-09-06 | cross-cutting | 03 §2.10 `AdminShell` row + Status line; `DESIGN.md` §9 (9) + changelog (v1.7); 05 §7.3 T-E2E-33 + Status line; `docs/questions.md` 2026-09-06 |
| ADR-0034 | Catalogue fixes from Oliver's first use: synced slugs normalised, loader display names, versions-table ranges, full-size Modrinth images | Accepted (2026-09-11, fix pass 1) | 2026-09-11 | cross-cutting | 04 §3.1 Steps + External calls + §4.1 exports + Status line; 03 §2.3 `VersionsTable`/`GetItPanel` rows + Status line; 05 T-ACT-78, T-ADP-4, T-ADP-21, T-UNIT-47, T-UNIT-48, §8 S1.2 + Status line; `docs/data-model.md` §2 `slug`/`icon_url`/`gallery`; `_registry.md` Modules/Adapters; 00 §6 |
| ADR-0035 | Look-and-feel fixes from Oliver's first use: themed `Select` listbox (client island), 2 px avatar outline, route-change fade, `CheckGrid` loaders, tip line | Accepted (2026-09-11, fix pass 2) | 2026-09-11 | cross-cutting | 03 §2.2 `Select`/`Avatar`/`AvatarUpload`/`CheckGrid` + §1.4 island list + §2.3 `TipPanel` + §4 N-02 + C-08 + Status; 02 §1.3 admin project rows + Status; 01 §1 tree; `DESIGN.md` §3/§5/§8/§11.1 + changelog v1.8; 05 T-E2E-2, T-E2E-35, T-E2E-50, §8 + Status; `_registry.md`; 00 §6 |
| ADR-0036 | Build order after Oliver's first use: S1.5a Cross-posted projects + S1.5b Support page inserted before S1.6; S1.9 narrowed to Stats | Accepted (2026-09-11, build order) | 2026-09-11 | cross-cutting | 00 §1.4, §1.7, §2 (S1.5a, S1.5b, S1.9), §4.1–4.3, §6 + Status; 01 §7 + Status; 02 §1.1/§2/§6/§8 + Status; 03 §2 slice cells + Status; 04 §5.7 + Status; 05 §8 + Status; `_registry.md` ID conventions + Slices; `START-BUILD.md`; `docs/questions.md` |
