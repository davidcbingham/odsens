...
3. **S1.5b — Support page.** The public `/support` page (`AmountPicker`, CONTINUE ON KO-FI mounting `KofiPanelSlot`, "on Ko-fi ↗", the leaderboard empty state), `FloatingSupportButton` on every public route except `/support`, the `tip_click` event, and the Ko-fi `frame-src` CSP entry move out of S1.9 into S1.5b, with their acceptance criteria S1.9.AC4–AC7, AC11 and the S1.9 tests T-E2E-11, 49 (+ the `tip_click` custom-events smoke; T-E2E-40 is the `/admin/stats` test and stays). **S1.9 keeps** `stats_daily`, `snapshotStats`, `/admin/stats`, the `sign_in` event and S1.9.AC1–AC3, AC8 (minus `tip_click`), AC9, AC10 (minus `/support`).
...
- Follow-ups: S1.5a Session A writes its contract ADR (link actions, sync adoption rules, fold semantics vs 01 INV-24 / 04 J-D "never delete", 04 §1.4 uploads on synced rows, `Downloadable` for synced rows, 02 §2.3 + 03 `GetItPanel`/`ExclusiveBadge` primary + badge rules, ADR-0002 #42 as the fallback) → owner `build-phase`; the S1.6 Session-A prompt is redrafted after S1.5b merges → owner `build-phase`; the `ship` skill's tag line learns the inserted form `v0.<n>.<k>` → owner `keep-docs` (done in this PR).

## Docs amended
| Doc | Section | Change |
|---|---|---|
| `docs/build/00-build-plan.md` | §1.4 Tagging; §1.7 Gate matrix; §2 new S1.5a + S1.5b sections; §S1.9 heading, Scope IN, Eng docs, AC, Tests, Gates, Demo, Risks; §4.1 (none rows), §4.2, §4.3; §6 Changelog; Status line | the inserted slices, the S1.9 narrowing (contains ADR-0036) |
| `docs/build/01-architecture.md` | §7 `KOFI_PAGE` row; Status line | S1.5b (contains ADR-0036) |
