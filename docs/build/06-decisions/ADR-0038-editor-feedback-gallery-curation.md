...
- Negative: `gallery_overrides` entries for urls Modrinth later removes linger harmlessly (max 40); a Delete removes the Storage object at once — no undo (the row buttons are ghost, not confirmed; Oliver can re-upload).
...
| `docs/build/02-routes-and-pages.md` | §1.3 `/admin/projects/[id]` row; Status line | D1, D3 |
| `docs/build/03-components.md` | §1.4 client-island list (`SavedToast`); §2 admin table (`SavedToast`), `Avatar` row (`frame`); §2.10; Status line | D1, D2, D3 |
| `docs/build/04-server-contracts.md` | §1.4 `curateProject` Input/Effects, `updateExclusiveProject` Input/Effects; Status line | D3 |
