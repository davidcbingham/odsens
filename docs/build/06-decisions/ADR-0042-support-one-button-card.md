...
- Related: David's decision, recorded in `docs/questions.md` "Support page after first use (2026-09-18)" — option "A + (a)": one button that swaps the card for Ko-fi's panel; the floating button stays a link to `/support` (opening a pop-up of our own in place is a possible later step, to be discussed with Oliver). Closes ADR-0002 #50 and, per ADR-R14, the two 03 §7 OPEN rows it contradicts: **O-8** ("Preselected tip amount — $3") is closed — nothing is preselected on our side — and **O-9** ("…whether it can preselect an amount") is answered: it cannot. Amends ADR-0041 D2 (the props) and D7 (two copy lines). Supersedes nothing.
...
| `docs/build/00-build-plan.md` | §S1.5b Scope IN (two bullets), AC1 gloss, AC6, Demo script, Risks; §S1.9 Scope marker, AC4 (= S1.5b.AC1), AC8 `tip_click` clause, Demo step 3 + Risks markers; §5 00-O-19; §6 Changelog; Status line | D1, D2, D3, D4 (contains ADR-0042) |
| `docs/build/01-architecture.md` | §13 INV-58 Statement + Check cell; Status line | D2, D4 (contains ADR-0042) |
| `docs/build/02-routes-and-pages.md` | §1.1 `/support` row; §2.7 Sections + Data; §6 `/support` loading row; §9 O-19; Status line | D1, D2, D6 (contains ADR-0042) |
| `docs/build/03-components.md` | C-13; §1.4 island row; §2.2 `Button` + `TrackedLink` emitter lists; §2.9 `KofiCard` (was `AmountPicker`) + `KofiPanelSlot` (incl. its DESIGN.md-source cell); §3 ARIA row; §7 O-8 (closed) + O-9 (answered); Status line | D1–D5 (contains ADR-0042) |
