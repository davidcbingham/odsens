...
4. **Full-size images from Modrinth (D4).** (a) Gallery: `mapProject` stores `raw_url ?? url` in `gallery[].url` — the original, not the `_350` thumbnail; the gallery/lightbox/hero already read `url`. (b) Icon: `createModrinth` gains `resolveIconUrl(iconUrl)` — for a resized `…/<hash>_<n>.webp` URL it HEAD-probes `<base>.png|.jpg|.jpeg|.webp|.gif` (10 s each, no retry; the CDN, not the rate-limited API) and stores the first 200, else the input unchanged. `syncModrinth` calls it only when the icon is new or changed (`iconBase(existing.icon_url) !== iconBase(mapped.icon_url)`), so an unchanged catalogue costs zero extra requests per run. Fixture icons are plain `.png` URLs, so the db/e2e suites never probe (05 H-5 holds). (c) `next.config.ts` `images.qualities = [75, 90]` and `quality={90}` on the four big slots — detail icon, hero screenshot, gallery well, lightbox. Cards keep the default 75 at 64 px.
5. **Tests.** T-UNIT-47 (`loaderLabel`), T-UNIT-48 (`formatVersionList`), T-ADP-21 (`normalizeSyncedSlug`, `raw_url` preference, `iconBase`, `resolveIconUrl` probe order / failure paths); the `projectChips` expectations in `tests/unit/data-projects.test.ts` now read names.
...
- Negative: a synced slug can now differ from Modrinth's (only when Modrinth's is invalid here); `next/image` optimises larger originals (more bytes per first render of a gallery; cached thereafter); up to five HEAD requests per new or changed icon per sync.
...
| Doc | Section | Change |
|---|---|---|
| `docs/build/04-server-contracts.md` | §3.1 `syncModrinth` Steps; §4.1 `modrinth` export list; Status line | slug normalised, `icon_url` original probe, gallery `raw_url`; `resolveIconUrl` export (contains ADR-0034) |
...
| `docs/build/05-test-plan.md` | §7.3 T-ADP-4, T-ADP-21 (new); §7.4 T-UNIT-47, T-UNIT-48 (new); Status line | the new assertions (contains ADR-0034) |
| `docs/data-model.md` | §2 `projects.icon_url` / `gallery` notes | "full-size original" (contains ADR-0034) |
| `docs/build/_registry.md` | Modules; Adapters | `format/loader.ts`; `versions.ts formatVersionList`; `resolveIconUrl`, `normalizeSyncedSlug`, `iconBase` |
...
| backend-reviewer | the icon probe is HEAD-only, bounded (5 × 10 s), never on an unchanged icon, never in fixture mode; slug rename revalidates old + new paths |
