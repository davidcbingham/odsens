...
- Found *(addendum 2026-08-20, gate round 1 — `frontend-reviewer` #9)*: `/profile` did not hydrate in place. `ViewerProvider` published viewer state through a React context; the provider sits above the route segments and `app/(public)/profile/loading.tsx` makes the page segment a Suspense boundary React hydrates lazily after the root. Any context update that reaches a still-dehydrated boundary marks it changed, and React then discards the server HTML and client-renders the segment (error #421 for a sync update; a silent replacement for a transition) — the server `HandleField` was replaced ~0.1 s after load in 6/6 runs and keystrokes typed until then were lost. 03 §2.5 / C-01 described the mechanism as "context"; C-17a fixes only the `useViewer()` shape.
...
9. *(addendum 2026-08-20)* **`ViewerProvider` publishes through a module-level external store, and `useViewer()` reads it with `useSyncExternalStore`** — no React context. Only the leaves that call `useViewer()` re-render, so nothing propagates into a dehydrated boundary and `/profile` hydrates in place. `getServerSnapshot` is the `loading` state (server HTML and hydration agree; C-17a leaves render the signed-out shape while loading). The auth subscription (`lib/supabase/client.ts`, lazy — ADR-0008) is reference-counted across mounted providers (StrictMode remounts, the `/dev/components` specimen): the first mount starts it, the last unmount stops it; the last published state survives a remount, so a client-side trip through `/admin` and back does not flash "Sign in". The `useViewer()` shape, the five-column own-row read and the `odsens:viewer-refresh` event are unchanged; the retry workaround in `tests/e2e/flows/profile.spec.ts` `typeHandle` is removed. 03 §2.5 `ViewerProvider` row and C-01 say "store", not "context".
...
- Positive *(addendum 2026-08-20)*: `/profile` keeps its server HTML (no skeleton flash, no double render, SSR ids kept); a future session-aware leaf inside a `loading.tsx` segment subscribes without touching the boundary.
- Negative *(addendum 2026-08-20)*: the store is a module singleton — a second `ViewerProvider` on the same page shares it (by design); tests that need a fresh store reload the page.
...
| `docs/build/03-components.md` | §2.5 `ViewerProvider` row | "publishes through a module-level external store that `useViewer()` reads with `useSyncExternalStore` (never a React context …)" (contains the string ADR-0014; addendum 2026-08-20) |
| `docs/build/03-components.md` | §1.1 C-01 | `.module.css` exemption wording: `ViewerProvider` (store only, no markup — ADR-0014) (addendum 2026-08-20) |
| `docs/build/00-build-plan.md` | §6 Changelog ADR-0014 row | addendum sentence appended (addendum 2026-08-20) |
| `docs/build/06-decisions/README.md` | §7 Index | ADR-0014 Amends cell extended (addendum 2026-08-20) |
| `docs/questions.md` | S1.1 build notes | addendum named in the ADR list line (addendum 2026-08-20) |
...
| frontend-reviewer | `node scripts/check-client-islands.mjs` exits 0 (the `ProfilePanel` row is present); `limitedUntil` is a string prop — no `Date.now()` / `new Date()` in `ProfilePanel` render; `deleteAccount` runs inside `startTransition`; `data-variant` is the only look switch in `GoogleSignInButton`; (addendum, closes gate-round-1 finding #9) `components/accounts/ViewerProvider.tsx` has no `createContext` / `useContext` — `useViewer()` is `useSyncExternalStore(subscribe, getSnapshot, getServerSnapshot)`; on `/profile` the server-rendered `input[name=handle]` stays attached through hydration (same element before and after the viewer read); `tests/e2e/flows/profile.spec.ts` `typeHandle` has no `toPass` retry |
