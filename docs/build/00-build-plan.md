...
- CI (GitHub Actions): lint, typecheck, `test:unit`, `test:db` (Supabase CLI service), `build` (uploads `build-output.txt` — the `next build` route table — as a CI artifact; `frontend-reviewer` compares a PR's route table against `main`'s latest artifact — 01 INV-10, 05 CI-4, ADR-0002 A13), `test:e2e`; client-bundle grep for the CI-4 list (05 §4): `SERVICE_ROLE`, `sb_secret`, `CURSEFORGE_API_KEY`, `YOUTUBE_API_KEY`, `RESEND_API_KEY`, `DISCORD_WEBHOOK`, `KOFI_`, `CRON_SECRET`, `GOOGLE_OAUTH`, `HASH_SECRET` → must be absent (`scripts/check-bundle-secrets.mjs`); `scripts/check-client-islands.mjs`, `check-fixtures.mjs`, `check-test-ids.mjs` wired.
...
6. S0.AC6 — CI workflow runs the five required checks in DoD-2 (`lint`, `unit`, `db`, `build`, `e2e`) and blocks merge on failure; `scripts/check-bundle-secrets.mjs` inside `build` passes; the `build` job publishes `build-output.txt` as an artifact (ADR-0002 A13).
...
**Acceptance criteria**
1. S1.1.AC1 — Clicking "Sign in" starts the Google OAuth flow; after consent the user lands on `/welcome` if `profiles.handle` is null; visiting any page other than `/welcome`, `/privacy`, `/how-comments-work`, `/auth/*`, `/api/*` (and static assets) while handle is null redirects to `/welcome` (02 M5; T-ACT-10, T-E2E-21; a banned account is sent to `/banned` first — 02 M4b, ADR-0019).
...
13. S1.1.AC13 — View `site_settings_public` is selectable by anon and exposes exactly `comments_closed_default`, `kofi_page`, `owner_profile_id`, `moderation_mode` (ADR-0002 C6 + A3; T-RLS-132); `site_settings` itself stays admin-only (T-RLS-12..14); `rate_limit_hits` is service-role only for every role (T-RLS-130) and `rate_limit_ok`/`purge_rate_limit_hits`/`check_handle` grants are per T-RLS-129; a burst of `checkHandle` calls past the 04 §5.5 scope returns `rate_limited`.

**Tests required:** 05 §8 row S1.1 — T-RLS-1..15, 115..116, 125, 127, 129 (`check_handle` grant + `rate_limit_ok`), 130 (`rate_limit_hits`), 132 (`site_settings_public`) (T-RLS-126 belongs to S1.4 — ADR-0002 A5); T-ACT-0..10, 65 (`deleteAccount`) (T-ACT-69 belongs to S1.4 — ADR-0002 A5); T-UNIT-1, 2, 17 (png/jpg/webp), 18 (`avatar`), 23 (`emailHash` part), 37 (scopes present so far); T-E2E-12, 13, 16, 21, 22, 23, 32, 33 (gate + 404 for wrong role; sidebar can be a stub), 46; seed SEED-3.
...
**Risks / unknowns:** Google OAuth redirect URLs must include the Vercel preview pattern (`https://odsens-git-*-studiobing.vercel.app/**` in both the `[remotes.production.auth]` list and the base `[auth]` block that Supabase preview branches inherit — ADR-0011; the bare `*.vercel.app` wildcard is gone); Q36 under-13 line copy needs David's confirmation before S1.10 (build the DESIGN.md wording now); `deleteAccount` uses the admin client (`auth.admin.deleteUser`) — behind an inline confirm, rate 1 / day (04 §1.0); `docs/data-model.md` §4 profiles update/delete rules are already amended by ADR-0002 (#27 handle rename, #28 account deletion) — the slice needs no further ADR unless it deviates.
...
- Actions (all **admin** — ADR-0002 C7; moderators see the admin pages read-only): `triggerSync` (`lib/actions/admin.ts`; runs a job now), `curateProject` (`lib/actions/projects.ts`; upsert `project_overrides`: featured/featured_order/hidden/notes_md/comments_enabled/title_override/description_override; batch shape `reorder: [{project_id, featured_order}]` — one call, one revalidate — ADR-0002 A11; `extra_gallery` paths accepted per 04 §1.4 but the upload UI + `project-media` bucket arrive in S1.3), `linkProjectListing` / `unlinkProjectListing` (manual CurseForge id/URL per Q39 — the `curseforge` case; was `setProjectLink`, ADR-0037 D1).
- Public routes: `/projects` (client island `ProjectGrid` (`components/projects/ProjectGrid.tsx`) owns filter/search/sort state over the ISR-provided list, renders the `ProjectCard`s, "Showing n of N" and the empty state; `FilterBar`/`SearchBox`/`ActiveFilterChips` are its children — ADR-0002 A7; `FilterBar` type counts + version + sort `downloads|updated|newest|title`, count line "<N> things. Some useful, some not." — ADR-0002 #39; search = client-side substring on title + description over the ISR list — 02 §2.2 RP-02, `search` tsvector unused on the page in v1; `SearchBox` 250 ms debounce — ADR-0002 #59; `ActiveFilterChips`, 3-up `ProjectCard` grid (2 chips per card, 4 elsewhere — ADR-0002 #54), empty state §11.7), `/projects/[slug]` (breadcrumb, icon 104px, `TypeBadge`, `Chip`s, count row, `Gallery` + `Lightbox`, ABOUT `Markdown` incl. `notes_md` appended, `VersionsTable` + `ChangelogExpander` §12.5 (synced file Download cell → the Modrinth CDN URL `project_files.url` — ADR-0002 #42), `GetItPanel` sticky rail with Modrinth/CurseForge rows + combined-count line, `DetailsList`, `TipPanel` **placeholder slab pointing at `/support`** until S1.9; public comments slot reserved); `/` hero `FeaturedHero` (featured project takeover; NEW badge when `published_at` < 30 days — ADR-0002 #41; fallback = highest `downloads_total` per 02 §2.1) + Featured 4-up; `app/sitemap.ts` (`/sitemap.xml`, 02 RP-07; T-E2E-45b — ADR-0002 A9); `generateMetadata` per page; `ProjectCardSkeleton`, `ProjectDetailSkeleton` `loading.tsx`.
- Admin: `/admin/projects` (`Table` of all projects incl. hidden, empty copy per 02 §1.3 — ADR-0002 #40; **feature/hide/reorder controls live here** — feature/hide `Toggle`s (first use of `Toggle`, 03 §2.2) + `ReorderableList` reorder → `curateProject` batch `reorder` (ADR-0002 A11); `SyncStatus` = `Table`+`StatusPill`+`Button`+`SourceSwatch` from `sync_runs` — ADR-0002 #56; `StatusPill` fills per ADR-0002 #47; "Sync now" → `triggerSync`), `/admin/projects/[id]` **curate view for synced projects** (02 §1.3: per-project extras only — overrides form built from `Field` (first use, 03 §2.2) + `Toggle` + `Select` — title + description override/notes/comments toggle, extra gallery (upload UI arrives S1.3), CurseForge id field → `linkProjectListing` / `unlinkProjectListing` (was `setProjectLink` — ADR-0037 D1); feature/hide/reorder are on the list page, not here — ADR-0002 A11); S1.3 extends this route with the exclusive edit form and gallery upload.
...
- `TrackedLink` (03) + `lib/analytics.ts` `trackEvent` + Vercel Analytics custom events **start here** with `download {project, source, from}` on the Modrinth/CurseForge download links (ADR-0002 A10 / C12); `video_play`, `tip_click`, `sign_in` are wired in their own slices (S1.6, S1.9); the dashboard toggle is S1.10.
...
8. S1.2.AC8 — Admin: as role `admin`, on `/admin/projects` toggle feature/hide + reorder, and on `/admin/projects/[id]` add notes, title/description overrides, and a CurseForge id → after `syncCurseforge` the CF row and count appear on the detail page; `SyncStatus` shows last run time/ok/items; "Sync now" runs and refreshes. As role `moderator` the same pages render read-only and `curateProject`/`linkProjectListing` (was `setProjectLink` — ADR-0037)/`triggerSync` return `forbidden` (ADR-0002 C7; T-ACT-40..42 with mod = denied).
...
- Table: `project_downloads` (`project_id, file_id, ip_hash, ua_hash, created_at`; `lib/hash.ts` per ADR-0002 C13: `ipHash = HMAC-SHA256(HASH_SECRET, ip|utcDay)`; `HASH_SECRET` is already present since S1.1 — ADR-0002 A14); purge >90 days runs in the stats job (S1.9) — S1.3 ships the table + insert; RPC `record_download` (04 D4).
...
**Demo script**
1. `/admin/projects/new` → fill Modrinth-shaped form → save draft → `/admin/projects/[id]` → upload icon + a `.jar` (uploads need the project id for the SC-21 paths — ADR-0027; data-model §6, 05 T-E2E-35).
...
**Risks / unknowns:** Vercel 4.5 MB request-body cap is why 04 §1.4.5 exists (baseline, no ADR — ADR-0002 C11); `.mrpack` is a zip container (04 SC-19 treats it as the ZIP signature); `HASH_SECRET` (≥32 bytes) has existed in Vercel preview + prod since S1.1 (ADR-0002 C13/A14).
...
- Public UI on `/projects/[slug]` COMMENTS section (client seam per ADR-0002 C1 — no PPR): `CommentThread` (**one client file** `components/comments/CommentThread.tsx` — no `CommentList`/`CommentThread.List.tsx`, ADR-0002 A1; public comments arrive as props from the server page via `lib/data/comments.ts` over `comments_public`, tag `project:<slug>`; `useOptimistic`; reads the viewer's own held/hidden rows + own likes via `lib/supabase/client.ts` under RLS; mods read held/reported rows via RPC `moderator_thread` — ADR-0002 A2; optimistic insert only under `moderation_mode = 'auto'`, none under `hold_first_time` — ADR-0028 D13), `Comment` (not split — ADR-0002 #57), `Reply` (one level; deeper replies flat with `@handle`), `Composer` (`useActionState`), `LikeButton`, `ReportPicker`, `HeldNotice`, `SignInPrompt`, `ModActionRow` (+ Moderate ON/OFF `Toggle` in thread header for mods), `CommentThreadSkeleton`; all §11.2 states: own Edit/Delete, edited marker, inline delete confirm, report confirmation line, count `14 TOTAL` (= slots the viewer sees — ADR-0002 #76), empty thread, hidden slot, deleted-with-replies slot, banned composer, comments closed, composer error, held (author view + mod view with `FIRST COMMENT` tag), CREATOR (from `site_settings.owner_profile_id`, ADR-0002 #55) / MOD tags. Composer error strings from `lib/validation/comment.ts` (`commentErrorLine`; ADR-0002 C16).
...
8. S1.4.AC8 — Delete asks once inline; deleted comment with replies shows "Deleted." slot with replies intact; a deleted root without visible replies (and any deleted reply) leaves the thread and the count (05 T-E2E-3 / 03 §2.2 / DESIGN.md §11.2 — ADR-0028 D1) — status `deleted`.
...
10. S1.4.AC10 — Banned user: composer replaced by "You can't comment here." (reachable only when a session is banned mid-page — the proxy sends a banned account to `/banned` first, ADR-0019 / ADR-0028 D8); RLS blocks insert into `comments`, `comment_likes`, `comment_reports` for banned (T-RLS).
...
**Tests required:** 05 §8 row S1.4 — T-RLS-63..89, 90..93 (`notification_events`, created here), 126 (`comment_count` trigger — ADR-0002 A5), 128 (`comments_public`), 131 (status trigger), 133 (`can_comment`); T-ACT-11..24 (all comment actions auth matrix + validation — length, links, HTML strip, edit window, rate limits, auto-hold at 3 with both events, moderation-mode branch, comments-enabled rule, mod delete), 67 (`renameUserHandle`), 69 (audit line — ADR-0002 A5), 65 (`deleteAccount` cascade re-run against comments); T-UNIT-4..8, 40 (comment error lines); T-E2E-3 (comments part), 24..30, 36; seed SEED-9; COV-2.
...
**Risks / unknowns:** Optimistic UI + held status (must not flash "published" for held comments — `postComment` returns the row as stored and `CommentThread` performs no optimistic insert under `hold_first_time`, ADR-0002 #72 / ADR-0028 D13); rate limits are SQL counts via `rate_limit_ok` over `rate_limit_hits` only — every limited action records a hit (ADR-0002 A4; no edge limiter); `comment_count` counts ever-published comments (04 §1.2; 00-O-13 DECIDED).
...
**Scope IN**
- **Linking.** `project_links` gains its `modrinth` rows (the enum already has the value; PK `(project_id, platform)` unchanged): a "Modrinth listing" field in the `/admin/projects/[id]` editor beside the CurseForge one (paste a Modrinth URL, slug or id → `adapters/modrinth` resolves the project → upsert the link with `url`, `downloads`); a "Remove" control. Actions `linkProjectListing` / `unlinkProjectListing` (admin only; names pinned by ADR-0037 D1 — `setProjectLink` is retired and becomes the `curseforge` case of `linkProjectListing`; `platform: z.enum(['modrinth','curseforge'])`, `ref` = Modrinth URL / slug / id via `parseModrinthRef` + `adapters/modrinth.getProject`; replacing a Modrinth listing goes through unlink first → `conflict`; a listing links to one project — unique `(platform, external_id)`).
...
- **Fold on link.** Linking a listing that the sync already imported as its own `projects` row folds that row into the canonical one: `project_versions`, `project_links`, `comments` (and their likes/reports), `project_downloads` re-parented; `project_overrides` merged (canonical wins); the duplicate row removed — the one exception to J-D "never delete synced rows", recorded here and in ADR-0037 D3 (RPC `fold_project(uuid, uuid)`, service-role only, one transaction, invoked by `linkProjectListing` **after** the link row is written so a failed fold converges on the next sync); the old slug keeps resolving through a `project_redirects` row (`old_slug citext PK → project_id`) — `resolveProjectPage` + `permanentRedirect` from both `generateMetadata` and the page (ADR-0037 D4).
...
- **Uploads for every project.** `uploadProjectMedia` / `uploadProjectFile` / `publishProject` accept `source='modrinth'` rows (04 §1.4 "modrinth rows → forbidden" is lifted **by ADR-0037 D5**, which also amends 01 INV-24 / 04 J-D for the fold, 02 §2.3 + 03 `GetItPanel` / `ExclusiveBadge` for the primary rule and badge, and records ADR-0002 #42's CDN rule as the fallback; the two-phase flow, buckets and limits unchanged); the exclusive editor's upload well renders on synced projects; `lib/files.ts resolveDownloadable` serves hosted files regardless of `source` (the `Downloadable` shape is unchanged — ADR-0027 D5 stays deferred to S1.7, ADR-0037 D5(d)).
- **Download panel.** `GetItPanel` primary = the hosted primary file of the newest version that has a hosted file (`/api/download/[fileId]`, counted — `selectPrimaryFile`, ADR-0037 D6) when one exists, else the project's Modrinth page (today's "Download on Modrinth" rule); rows "Also on Modrinth ↗ <count>" / "Also on CurseForge ↗ <count>" from `project_links` **or** `source`; the combined-count line unchanged. `VersionsTable` shows every version once; each file's Download goes wherever that file lives. `FeaturedHero` / `ProjectCard` download targets follow the same primary rule.
...
- **Data model.** `docs/data-model.md` §2: `project_links` `modrinth` rows documented; `project_files` may carry `storage_path` on any source; the fold rules; the `project_redirects` table (ADR-0037 D4).
...
**Scope OUT**
- Autosave or drafts of unsaved text; a WYSIWYG editor; image upload from the toolbar (the Gallery section uploads; the image button inserts a URL); YouTube embeds in Markdown (S1.6 decided: **no** — the toolbar's YouTube button keeps inserting a plain link; reopened only if Oliver asks — ADR-0043 D13); the Markdown editor on `/admin/projects/new` (later); any server-contract or schema change.
...
- Public `/support` (§6.7 + §11.4 + 02 §2.7; ISR 600 with tag `settings`, reads `site_settings_public` — ADR-0002 C19; replaces the S0 placeholder): gold hatched `KofiCard` (was `AmountPicker` with $1 / $3 / $5 / Other — removed by ADR-0042: Ko-fi's panel takes no preset amount) with a single TIP ON KO-FI button that swaps the slab for the `KofiPanelSlot` iframe in place (712/620 px — ADR-0002 #50; no new tab; "← Back" returns the slab), plus an "on Ko-fi ↗" ghost link; the iframe renders for `site_settings.kofi_page`; `kofi_page` empty → button disabled with "Tips open soon."; leaderboard block in its empty state (§12.4).
...
- Ko-fi framed on `/support` only — a **code** rule (01 INV-58: the iframe exists in `KofiPanelSlot` alone, mounted by `/support`'s `KofiCard` after the click — ADR-0042); the Ko-fi `frame-src` entry itself stays in the site-wide CSP baseline of 01 §20, unchanged since S0, because a per-route policy would block the panel after a client-side navigation into `/support` (ADR-0041 D1).
...
6. S1.5b.AC6 — `tip_click` fires with `{amount?, from}` only (no emitter sends `amount` since ADR-0042) (the S1.9.AC8 clause for this event); axe zero serious/critical on `/support` at 1280 + 390 (the S1.9.AC10 clause for this route).

**Tests required:** T-E2E-11, 49 (moved from S1.9; T-E2E-40 is the `/admin/stats` test and stays in S1.9); the `tip_click` custom-events smoke; T-UNIT-38's `tip_click` case (the allowlist test itself stays in S1.9's row); T-UNIT-54 (`lib/support.ts` — ADR-0041 D8).
...
**Scope IN**
- Table: `videos` (per `docs/data-model.md` §2.3; `is_short` = duration ≤ 60 s or `#shorts` in title/description — ADR-0002 #67, refine only via an ADR with slug `shorts-detection`; live videos excluded — ADR-0002 #77: a keyed sync never gives them a row, ADR-0043 D2 (a no-key run cannot see the live flag — D18); Oliver's Shorts override lives in `is_short_override boolean null` while `is_short` stays the effective flag every reader filters on — ADR-0043 D1); RLS: published/not hidden to all; admin all.
- Adapter `lib/adapters/youtube.ts`: RSS (`YOUTUBE_CHANNEL_ID`) for cheap new-video detection (Atom XML through `fetchText` in `lib/adapters/http.ts` + a hand-rolled parser — no XML dependency; ADR-0043 D3); Data API `playlistItems` (uploads playlist) + `videos` for duration/stats; quota budget logged; 10 s timeout, retry ≤3.
- Job `syncYoutube` (04 §3.3: gather, then write — any list call failing writes nothing, ADR-0043 D2; upsert by `youtube_id`, never delete, `sync_runs`, `revalidateTag('videos')`, `sync.failed` via the S1.5 runner; no `YOUTUBE_API_KEY` → RSS-only degraded run, T-ACT-71); route `/api/cron/sync-youtube` (04 §6 `27 * * * *`, `maxDuration` 300) in `vercel.json`.
- Public: `/videos` (big `VideoFacade` player — nothing from YouTube loads until click, `youtube-nocookie.com` embed; play-block sizes 88 hero (and `mention`), 44 upnext, 56 short **and card** — DESIGN.md §12.7, ADR-0002 #58 as mapped by ADR-0043 D7 (was "88 hero/card"); Bungee title, view/date meta, blurb; the client island `VideoStage` owns the `?v=` selection (ADR-0043 D4); `UpNextList` rows = `VideoFacade variant=upnext` (ADR-0002 #49 — presentational inside the row link, ADR-0043 D5), selected = `--indigo-lift` outline; `VideoCard` "MORE VIDEOS" grid below the stage = the long videos after the first four, 3 / 2 / 1-up (was "long-form grid below on phone"; page composition head → stage → grid → Shorts — ADR-0043 D12); `ShortsRow` 9:16 104 px gold duration chip, horizontal scroll on phone; empty state §11.7; replaces the S0 placeholder); Home "Latest videos" 2-up beside "Find me" list (Modrinth / CurseForge / YouTube) per §6.1, the compact `TipPanel` under that list — one row, two columns, single column below 900px (ADR-0041 D6; ADR-0043 D15).
- Admin: hide/unhide a video via action `updateVideo` (`lib/actions/videos.ts`, admin — ADR-0002 C7; input `{youtube_id, hidden?, is_short?}`) triggered from a videos list on the `/admin` dashboard — **no `/admin/videos` route** (ADR-0002 #20; 00-O-5 DECIDED). The list carries a Hidden and a Short `Toggle` per row, an "Auto" button on rows with an override, and answers every save with "Saved."; moderators see the controls disabled (ADR-0043 D11).

**Scope OUT**
- No comments on videos (ADR-0002 C21). No mentions (S1.8). No stats snapshot (S1.9). (`video_play {youtube_id, kind}` **is** wired here on the facade click via `TrackedLink`/`trackEvent` — ADR-0002 A10.)
...
3. S1.6.AC3 — Facade matches §11.1: thumbnail + scrim, 88 px indigo play block (the big player; cards and Shorts 56 px, Up next 44 px — DESIGN.md §12.7, ADR-0043 D7) with white triangle, duration chip bottom-right (Silkscreen ≥11 px), `CLICK TO LOAD YOUTUBE` chip bottom-left.
...
6. S1.6.AC6 — Home shows the two newest non-hidden, non-short videos as facades beside the Find me list (the compact `TipPanel` sits under that list — ADR-0043 D15).
7. S1.6.AC7 — Hidden video (`hidden=true` via `updateVideo` from the `/admin` dashboard) never renders publicly; `updateVideo` as moderator/user → `forbidden` (T-ACT-68); the dashboard path itself is the admin leg of T-E2E-47 (ADR-0043 D9).
8. S1.6.AC8 — Empty state "NO VIDEOS YET / They'll show up here when they exist." + channel link when there are no **visible** videos (was "when the table is empty (T-E2E with empty seed)": T-E2E-47 hides every video through the `/admin` toggles — a service-side truncate never revalidates the ISR page; the literally empty list is the unit arm `splitVideos([])` — ADR-0043 D9).
...
11. S1.6.AC11 — A forced `syncYoutube` list failure writes one `sync.failed` event (04 J-F) and the S1.5 pipeline delivers it (T-ACT-74 youtube — ADR-0043 D10).

**Tests required:** 05 §8 row S1.6 — T-RLS-48..52; T-ACT-33 (youtube route), 53, 68 (`updateVideo`), 71 (youtube no-key), 74 (youtube — AC11; ADR-0043 D10); T-ADP-9..13 (RSS / Data API / shorts / mapping / quota); T-UNIT-12, 29; T-E2E-1 (Latest videos), 6, 47 (empty state + the `/admin` hide leg — ADR-0043 D9); amended in place: T-ADP-1 (`fetchText`), T-E2E-42 (`/admin` with the videos list); seed SEED-11 (7 rows — ADR-0043 D8).
...
3. Click an Up next item → swaps (the outline moves, the page does not scroll). Scroll to MORE VIDEOS, then the Shorts row (ADR-0043 D12).
4. Home → Latest videos 2-up beside Find me and the tip panel (ADR-0043 D15).
5. `/admin` → hide a video → it leaves `/videos` and Home; un-hide it (ADR-0043 D11).

**Risks / unknowns:** Shorts detection heuristic accuracy (ADR-0002 #67; ADR slug `shorts-detection`, number per 06 ADR-N3, only if refined); YouTube quota if `search` were used — use `playlistItems` (uploads) instead; thumbnails hosts allow-list for `next/image`; a live / upcoming upload is skipped until it has ended (ADR-0043 D2) — on a keyed run; a no-key run cannot see the live flag and may store an RSS-minimal row for it (ADR-0043 D18); a no-key (RSS-only) run has no durations, so it can class nothing as a Short (04 §5.3 null guard) until a keyed run fills them.
...
**Scope IN**
- Table: `mentions` (per `docs/data-model.md` §2.3b; `status draft|suggested|published|hidden`, `source manual|auto`, `featured`, `sort_order`, `view_count`); RLS published to all, drafts/suggested admin only. As built (ADR-0045 D1–D3): `updated_at` + trigger, both FKs `on delete set null` (a mention is never deleted with its project; `fold_project` re-parents a folded duplicate's mentions), CHECKs mirroring the input schema, RPC `reorder_mentions` for the one-transaction reorder.
- Adapters: YouTube oEmbed + `videoIdFromUrl` live in `lib/adapters/youtube.ts` (`oembed`, Data API `videos` for id + views); `lib/adapters/oembed.ts` = `detectPlatform`, `fetchOpenGraph` (generic Open Graph fallback for tiktok/twitch/reddit/article/other), `assertPublicHost` (http(s) only, no private IPs, 10 s timeout) — names per registry / 04 §4. As built (ADR-0045 D5–D7): `lib/adapters/http.ts` gains `fetchPage` (the same loop with the opt-in `redirect: 'manual'` / `contentTypes` / `maxBytes` guards); `oembed` has **no base override of any kind** (`OEMBED_BASE` re-aims only the youtube adapter's oEmbed endpoint), takes an injectable DNS `lookup`, upgrades `http:` and refuses explicit ports, checks EVERY resolved address and every redirect hop under one 10 s budget, and returns parsed fields only; `youtube` gains `listVideoStats` (stats-only batches for the job) and `getVideoMeta` (one id, strict match, for the preview).
- Actions (**admin** — ADR-0002 C7; moderators see `/admin/mentions` read-only; `lib/actions/mentions.ts`): `fetchMentionPreview` (paste URL → metadata → `MentionPreview`), `createMention` (assign `project_id` or null = "About OddSense generally"; publish), `updateMention` (feature/hide/reorder/reassign). As built (ADR-0045 D8–D10): the URL rule is a schema rule (`file:` / `javascript:` → `validation` before any resolver runs); the preview's step 3 is `getVideoMeta`, `creator_name` may be null, and every failure of the chain is the ONE "Couldn't read that page…" message (the SSRF guard is no oracle); `createMention` stores the canonical link, `created_by` = the acting admin, the link's own video id and an `i.ytimg.com` thumbnail; `updateMention` answers `not_found`, checks a half-patched YouTube id against the stored row and reorders through RPC `reorder_mentions` (nothing applied on an unknown id).
- Job `refreshMentions` (04 §3.4: hourly; YouTube `videos?id=…` batched 50/req → `view_count`; `sync_runs` source `mentions`; on failure emits `sync.failed` through the S1.5 runner — 04 J-F; `sync.stale` for `mentions` is emitted by `notifyFanOut` F0 only when `YOUTUBE_API_KEY` is set and ≥1 YouTube mention exists — 04 J-S); route `/api/cron/refresh-mentions` (04 §6 `37 * * * *`, `maxDuration` 300) in `vercel.json`. Daily reach snapshot lands in S1.9's `snapshotStats` (`metric='reach'`). As built (ADR-0045 D11–D13): gather then write — any failed batch writes nothing; a missing id, a hidden count and an unchanged number leave the row alone (a typed number is never overwritten with NULL); writes are guarded on the `external_id` as read; summary `{items, mentions, asked, returned, updated, unchanged, units, errors[]}`; the no-key run still writes its `sync_runs` row; `triggerSync({source:'mentions'})` runs it from a "Sync now" on `/admin/mentions`; the J-S `mentions` condition counts only the rows the job would refresh.
- Public: `SeenOnRow` on `/projects/[slug]` between VERSIONS & FILES and COMMENTS (title + count, 2-up `MentionCard`; **renders nothing when the project has no mentions**); Home `InTheWildStrip` after Featured (3–4 featured mentions + `ReachLine` + "All mentions →"; hidden when none); `/seen-on` (three `StatTile`s for reach totals, `FilterBar` ALL + platform counts + project `Select`, 3-up grid newest first with footer strip type badge + project link, general mentions tagged with the ODSENS wordmark chip; 1-up phone; filter yielding none → `EmptyState` "NOTHING HERE / Try another filter." — ADR-0002 #62; replaces the S0 placeholder); `Footer` second dry line "Creators featuring the mods aren't affiliated with odsens." added site-wide (registry S1.8; 02 RP-13; DESIGN §12.2). As built (ADR-0045 D14–D22): ONE cached reader (`listPublishedMentions`, tags `mentions` + `projects`) serves all three surfaces and drops a mention whose project the public cannot see; the pure shapes and orders live in `lib/mentions.ts`; `/seen-on`'s filter state is the new client island `SeenOnGrid` (`useSearchParams` under `<Suspense>`, `SeenOnGridView` as the fallback, `FilterBarView`); the three tiles show at every width with no subline and there is no `ReachLine` on the page; the SEEN ON count reads `1 MENTION` for one; the Home strip is head → cards → reach line, 4 / 3 columns; the reach line is one `PixelLabel` per segment; the ODSENS chip is Bungee 16px on the mark's slab.
- `MentionCard`: YouTube → inline facade→player + `--indigo-lift` outline + "on YouTube ↗" ghost; other platforms link out with ↗ chip worded per 03 V-04 (`WATCH ON <PLATFORM>` for video platforms, `READ ON <SITE>` for article, `SEE ON REDDIT`, `OPEN ↗` fallback — ADR-0002 #21; 00-O-6 DECIDED); non-YouTube thumbnails are never fetched remotely — `PlatformMark` placeholder instead (ADR-0002 #33); official platform marks (24 px) — neutral placeholders until Oliver's asset PR (ADR-0002 #25). As built (ADR-0045 D16 / D17): the mention facade's play block is **56px** (a grid card — DESIGN.md §12.7; refines ADR-0043 D7); a YouTube mention saved without a usable video id links out with `WATCH ON YOUTUBE`; the link-out card has no card-wide anchor (thumb slot, creator name and project title are three separate links); every card shows its title as a quiet line under the creator line (the pass-3 artboard).
- Admin `/admin/mentions`: paste URL → preview card → assign → PUBLISH; `Table` with FEATURED / LIVE / HIDDEN worded tags, ⠿ drag-reorder (featured order feeds Home), Feature/Hide; **Suggested tab UI stub** (empty state, no auto-discovery job). As built (ADR-0045 D23–D27): two URL-driven views (`?tab=suggested`), sections ADD A MENTION (the `MentionPreview` island owns Fetch + PUBLISH) → FEATURED ORDER (`ReorderableList` with visible Move up / Move down buttons — the risk note below) → ALL MENTIONS (newest added first; row buttons Feature / Unfeature / Hide / Show / Publish, "Feature" joins the END of the strip; "Saved." after every successful action — extends ADR-0038 D1) → SYNC; a moderator sees the RLS-filtered published rows with every control disabled; the Suggested stub reads "NOTHING SUGGESTED / Auto-found mentions will wait here for a yes or no. Not yet."
...
**Acceptance criteria**
1. S1.8.AC1 — Admin pastes a YouTube URL → preview shows thumb, title, creator, views, date; pastes a Reddit/article URL → OG preview; a private-IP or non-http URL is rejected server-side (T-ACT-62: a non-http(s) link is `validation` at the schema, a private host is the one `upstream_error` message — ADR-0045 D8).
...
4. S1.8.AC4 — Feature 3 mentions → Home strip shows them in `sort_order`, `ReachLine` reads e.g. "1.2M VIEWS · 6 VIDEOS · 4 CREATORS" (numbers computed from published mentions), "All mentions →" links `/seen-on` (the strip's grid is 4 columns with four cards, else 3; the seed has one featured mention, so T-E2E-1 proves one card + the line and the unit tests prove the order — ADR-0045 D19).
5. S1.8.AC5 — YouTube `MentionCard` click loads the inline player (nocookie), takes the `--indigo-lift` outline, shows "on YouTube ↗"; TikTok/Twitch/Reddit/article cards link out with ↗ + the 03 V-04 chip wording (`WATCH ON <PLATFORM>` / `READ ON <SITE>` / `SEE ON REDDIT` / `OPEN ↗` — ADR-0002 #21) and show a `PlatformMark` placeholder instead of a remote thumbnail (ADR-0002 #33); the mention facade's play block is 56px and a YouTube mention without a usable id links out as `WATCH ON YOUTUBE` (ADR-0045 D16 / D17).
6. S1.8.AC6 — `/seen-on`: three stat tiles (views, videos/mentions, creators), filter by platform with counts, project select; general mentions carry the ODSENS wordmark chip; newest first (tiles at every width, no subline; the filter is the `SeenOnGrid` island's URL state — ADR-0045 D15 / D20).
7. S1.8.AC7 — `/api/cron/refresh-mentions` updates `view_count` for YouTube mentions in batches of ≤50 ids per request; idempotent; `sync_runs` row (source `mentions`); 401 without secret; a forced Data API list failure writes one `sync.failed` event (04 J-F) delivered by the S1.5 pipeline (T-ACT-74 `refreshMentions` leg — ADR-0045 D12; a hidden count never overwrites a stored number).
8. S1.8.AC8 — Hidden mentions vanish from all three public surfaces; drag-reorder persists `sort_order` (one `updateMention({reorder})` through RPC `reorder_mentions`; by drag, by the handle's arrow keys or by the Move up / Move down buttons — ADR-0045 D2 / D26).
...
12. S1.8.AC12 — axe zero serious/critical on `/seen-on`, a project with mentions, `/admin/mentions` at 1280 + 390 (T-E2E-42's `/admin/mentions` leg shoots both widths — ADR-0045 D27).

**Tests required:** 05 §8 row S1.8 — T-RLS-102..106; T-ACT-33 (refresh-mentions route), 54 (batching), 62..64 (SSRF; admin only per ADR-0002 C7), 71 (mentions no-key); T-ADP-14..16; T-UNIT-9; T-E2E-1 (IN THE WILD + footer line), 3/5 (SEEN ON row), 10, 39; seed SEED-10; fixture server :4010 (ADR-0002 #73). Added at Session A (ADR-0045 D30, no new ids): T-ACT-74 (`refreshMentions` leg — AC7), T-RLS-129 (`reorder_mentions`), T-ACT-81 (the fold's mentions step), T-ACT-32 (`mentions` stale condition), T-ACT-42 (`mentions` runs the job), T-ACT-70 (the route), T-ADP-1 (`fetchPage`), T-ADP-13 (`listVideoStats`), T-ADP-20 (`oembed` has no override), T-UNIT-10 (`B` tier), T-E2E-42 (`/admin/mentions` at 1280 + 390), T-E2E-46 (the route), T-E2E-48 (the gallery area); amended in place: SEED-10 (pinned columns), T-ACT-54 / 62 / 63 / 64, T-ADP-15 / 16, T-UNIT-9, T-E2E-3 / 4 / 5 / 10 / 39.
...
6. `/admin/mentions` → SYNC → "Sync now" → the view count on the YouTube mention refreshes (ADR-0045 D11).

**Risks / unknowns:** OG fetch against bot-blocking hosts (Reddit/TikTok may 403) — preview falls back to manual fields (title/creator editable in admin; note in 04); official platform marks (Q44) — neutral placeholder until supplied (ADR-0002 #25); drag-reorder a11y (provide move up/down buttons too — `frontend-reviewer`; built — ADR-0045 D26); DNS rebinding between the guard's lookup and the transport's is an accepted residual risk (https-only, every address checked, no retry, parsed fields only — ADR-0045 D6); T-E2E-39 needs working DNS for `www.youtube.com` (the guard resolves for real — ADR-0045 D28).
...
*(Session A build pass 2026-09-25, branch `feat/S1.7-skins-art` — ADR-0047 (the bust renderer is a software rasterizer; `gl` is not added) and ADR-0048 (the Session A findings, D1–D28; D27 / D28 = the admin forms and their e2e as built). The "As built" glosses below carry the decision numbers; no AC is removed, weakened or moved.)*
...
**Scope IN**
- Tables: `skins`, `art` (per `docs/data-model.md` §2.4); buckets `skins` (public-read; 64×64 PNG ≤64 KB textures; cached bust renders ≤512 KB), `art` (public-read; ≤10 MB); RLS published to all, admin all; storage policies service-role only (01 INV-33) — skin textures travel inline in the action (≤64 KB, 04 SC-18), art images use the 04 §1.4.5 two-phase signed-upload flow. As built (ADR-0048 D2 / D4 / D5): migration `20260925120000_skins_art.sql` carries the four enums, `updated_at` + trigger, the owner-scoped path CHECKs `skins_texture_path_own` / `skins_render_bust_path_own` / `art_image_path_own` (the S1.2 `*_path` rule, closed) plus the slug / length / range CHECKs, `is_admin()` write policies, the two buckets with select-only policies; `20260925120100_skins_art_rpcs.sql` carries `record_skin_download` (fail-closed) and the reorder RPCs `reorder_skins` / `reorder_art` (the `reorder_mentions` copies).
- Actions (admin — ADR-0002 C7; 04 §1.5; `lib/actions/{skins,art}.ts`): `createSkin`, `updateSkin` (name, description_md, texture upload with 64×64 PNG validation, `model classic|slim`, `is_exclusive`, status, sort_order), `createArt`, `updateArt` (title, kind `avatar|thumbnail|icon|render|other`, image via two-phase, width/height computed server-side, year, credit, downloadable, status, sort_order). As built (ADR-0048 D6–D9): the skin inputs are form-shaped (`FormData` from the admin island), `updateSkin` / `updateArt` are unions with a `{reorder}` arm through the RPCs, `createSkin` inserts the row BEFORE uploading (a slug conflict writes no object), the verbatim "Skins need to be 64×64." fires for a PNG whose IHDR is not 64×64 and `validateUpload`'s copy for everything else, `createArt` mints the art id at `begin` inside the pending path and its commit accepts the pending or the final path of that id; `upload:skins` counts every call, `upload:art` every `begin` and every metadata-only / reorder call.
- Job: `renderSkinBust` (`lib/jobs/renderSkinBust.ts`, awaited by the skin actions → `skins.render_bust_path`; the renderer `lib/skins/render.ts` is a **pure-TypeScript software rasterizer over the vanilla player model — no `gl`, no native dependency beyond `sharp`; ADR-0047 replaces the ADR-0002 C22 / 00-O-14 default** (headless-gl has no Node 24 prebuilt and needs X11 + Mesa, absent on Vercel); fallback: the `SkinCard` renders a live `SkinViewer3D variant="bust"` while `render_bust_path` is null — ADR-0048 D15); `scripts/render-skins.mjs` for bulk (`add-content` use; idempotent — T-UNIT-45; as built with the plain-Node loader `scripts/lib/ts-loader.mjs`, one `sync_runs` row `source='skins'` per run, revalidates nothing — ADR-0048 D10 / D11).
- Public: `/skins` (§6.5: big live `SkinViewer3D` panel (client, lazy-loaded skinview3d) with spin / walk / front-back controls; 4-up `SkinCard` grid of rendered busts in 3:4 slots with the 64×64 source pinned small at integer scale `image-rendering: pixelated`; name + description + DOWNLOAD PNG + Slim toggle under the viewer; selected card `--indigo-lift` outline; `ExclusiveBadge` when `is_exclusive`; empty state §11.7); `/art` (§6.6: filter row all / avatars / thumbnails / icons; `ArtMasonry` column-flow, natural aspect, 18 px gutter, 4 cols desktop / 2 phone / 1 under 480; `ArtCard`; `Lightbox` with title, year, optional download; empty state). As built (ADR-0048 D12–D17): the `?skin=` selection and the `?kind=` filter are two new client islands `SkinsStage` / `ArtGallery` (`useSearchParams` under `<Suspense>`, the `VideoStage` / `SeenOnGrid` pattern) and `SkinViewer3D.Canvas.tsx` is the one `skinview3d` importer behind the one `next/dynamic` site; on `/skins` the details slab sits BESIDE the 4:3 stage at ≥ 900px and under it below (DESIGN.md §6 #5 read at laptop width — queued for its next revision), no download count is shown, and a card without a cached bust renders the live bust variant; on `/art` the fixed buttons ALL / AVATARS / THUMBNAILS / ICONS show at any count (RENDERS / OTHER only when present), an unoffered `?kind=` falls away to ALL, and each `ArtCard` carries the pass-3 artboard's caption strip.
- Admin: `/admin/skins`, `/admin/art` (add/edit forms with `UploadWell`, `Table` with status pills, reorder; moderators read-only per 02 §1.3). As built (ADR-0048 D18 / D19): ONE route each with `?edit=<id>` switching the `SkinForm` / `ArtForm` island to edit mode; sections ADD → ORDER (`ReorderableList` → `updateSkin({reorder})` / `updateArt({reorder})`) → ALL (`Table`, newest created first, Publish / Unpublish → PRG `?saved=saved` → "Saved." — extends ADR-0038 D1); `UploadWell` gains a pick mode (the skin texture, handed to the parent as a `File`) and a deferred-commit mode (the art image, PUT then "Ready to save." — the form sends the commit).
- Skin DOWNLOAD PNG → `/api/download/[fileId]` with kind `skin` (`lib/files.ts resolveDownloadable`) → RPC `record_skin_download` increments `skins.downloads` → 302 to the public texture object with a download filename (ADR-0002 C8; 00-O-17 DECIDED); RPC `record_skin_download(uuid)` created here. As built (ADR-0048 D1 — closes ADR-0027 D5): `Downloadable` is a two-arm union carrying its own `counter` + `urlKind`, the route's counter call is one `rpc(d.counter, counterArgs(d, ctx))` and its URL step one `urlKind` ternary — no per-kind chain; the `download` attribute on the stage's DOWNLOAD PNG plus Supabase's `Content-Disposition: attachment` for `?download=` (proven by T-RLS-121) saves the file.
...
**Acceptance criteria**
1. S1.7.AC1 — Admin uploads a 64×64 PNG → skin created as draft; a 128×128 or JPEG is rejected with the plain error; publish → appears on `/skins` (a wrong-size PNG → "Skins need to be 64×64.", a JPEG → `validateUpload`'s type copy — ADR-0048 D7).
2. S1.7.AC2 — `/skins` main panel is a live 3D model of the selected skin (skinview3d) — never a flat texture and never a profile picture; controls spin / walk / front-back work; `prefers-reduced-motion` stops idle spin (Spin / Walk are `aria-pressed` toggles, Front ⇄ Back one button whose label flips; reduced motion also disables Walk — ADR-0048 D14).
3. S1.7.AC3 — Grid slots show rendered busts (from `render_bust_path`, or client-render fallback if null) in 3:4 with the source PNG pinned small at integer scale with `image-rendering: pixelated` (the fallback is `SkinViewer3D variant="bust"` — one WebGL frame, no controls; the seed's `seed-skin-b` exercises it — ADR-0048 D15).
4. S1.7.AC4 — DOWNLOAD PNG goes through `/api/download/[fileId]` kind `skin`: 302 to the public texture object with a download filename, `skins.downloads` +1 per request via `record_skin_download`, draft skin → 404, rate limit → 429 (ADR-0002 C8; T-ACT-76, T-RLS-129); Slim toggle switches the model (the union `Downloadable` — ADR-0048 D1; the Slim state is local to the stage and resets to each skin's own model — D16).
5. S1.7.AC5 — skinview3d is lazy-loaded (`/skins` first-load JS excludes it until the viewer mounts; `next build` route table + `frontend-reviewer`; T-E2E-8 reads the requested chunks on `/`, `/projects` and `/skins` for the `skinview3d` marker — ADR-0048 D21).
6. S1.7.AC6 — `/art` masonry renders each image at natural aspect (`height:auto`, no crop); filter row filters by `kind`; lightbox shows title, year, download only when `downloadable` (the `ArtGallery` island owns `?kind=`; a kind the bar does not offer falls away to ALL — ADR-0048 D17).
7. S1.7.AC7 — Art upload >10 MB rejected with the number; accepted image records `width/height` (server-derived by sharp, client keys stripped; the final extension follows the sniffed type — ADR-0048 D8).
8. S1.7.AC8 — Empty states: "NO SKINS YET / Working on it. Check the projects meanwhile." and "NO ART HERE YET / Nothing in this filter. Try \"all\"." (the skins one is unreachable on seed data — a described `/dev/components` fixture; the art one is reached by ICONS at 0 in T-E2E-9 — ADR-0048 D16 / D17 / D24).
...
10. S1.7.AC10 — `ExclusiveBadge` on exclusive skins; alt text on every skin/art image; axe zero serious/critical at 1280 + 390 on `/skins`, `/art`, admin forms (T-E2E-7 / 9 on both smoke projects; T-E2E-42's `/admin/skins` + `/admin/art` legs at both widths — ADR-0048 D24).
11. S1.7.AC11 — `scripts/render-skins.mjs` renders busts for all skins missing `render_bust_path` (idempotent — T-UNIT-45; `pnpm exec node scripts/render-skins.mjs [--force]`, proven against the local stack — ADR-0048 D11).

**Tests required:** 05 §8 row S1.7 — T-RLS-53..62, 121..122, 129 (`record_skin_download`); T-ACT-56..61 (bust render, dimensions/type/size), 73 (art two-phase), 76 (skin download); T-UNIT-17 (all), 18 (`skin`, `art`), 19, 45 (`render-skins.mjs` idempotency); T-E2E-7, 8, 9, 38; seed SEED-7, 8, 13. Added at Session A (ADR-0048 D24, no new ids): T-RLS-129 (`record_skin_download`, `reorder_skins`, `reorder_art` — catalog + behaviour), T-ACT-73 (the art leg), T-ACT-76 (in `tests/db/routes/download.test.ts`), T-E2E-42 (`/admin/skins` + `/admin/art` at 1280 + 390), T-E2E-46 (the placeholder clause retired — `placeholders.spec.ts` deleted), T-E2E-48 (the SKINS + ART gallery area); amended in place: SEED-7 / 8 (pinned columns — D20), T-RLS-55 / 60 (constraint legs), T-RLS-121 / 122 (in `skins-bucket.test.ts` / `art-bucket.test.ts`, incl. the `?download=` header cell), T-ACT-56 / 58 / 59 / 61, T-UNIT-45, T-E2E-7 (three tests) / 8 (chunk method) / 9 (three tests). Id-less files (ADR-R9): `tests/unit/skins.test.ts`, `art.test.ts`, `skins-render.test.ts` (ADR-0047), `ts-loader.test.ts`, `tests/db/data/skins.test.ts`, `art.test.ts`.
...
**Risks / unknowns:** headless bust rendering on Vercel serverless — CLOSED by ADR-0047: `gl` is not added (no Node 24 prebuilt, needs X11 + Mesa); the renderer is a software rasterizer running inside the action's function, the fallback is the live `SkinViewer3D variant="bust"` (real and tested — T-E2E-7); WebGL in Playwright CI — the SwiftShader launch args in `playwright.config.ts`, and T-E2E-7 FAILS loudly on `data-state="unsupported"` rather than accepting the fallback (ADR-0048 D21); skinview3d bundle size (~150 KB gz) exceeds the 50 KB stop-and-ask threshold — pre-approved by `docs/framework-decision.md` (lazy-load); record it in the PR `## Bundle` section (§1.3); the `/skins` `<Suspense>` fallback mounts its own viewer, swapped out after hydration before the chunk lands (a warm-cache visit could build one GL context that is disposed at once — ADR-0048 D16); `/dev/components` under `pnpm dev` 500s on six pre-existing over-long specimen labels (T-E2E-48 runs a production build — ADR-0048 D26).

---

### S1.9 — Stats *(was "Stats + Support" — the Support half moved to S1.5b, ADR-0036)*

*(Built 2026-09-26 on `feat/S1.9-stats` — the Session A build pass; ADR-0049 (Proposed, D1–D37) records the findings and the as-built glosses below are its; tag `v0.10` at merge. No AC removed, weakened or moved.)*
...
- Vercel Analytics custom events complete the set (`lib/analytics.ts` `trackEvent` allowlist via 03 `TrackedLink`, live since S1.2 — ADR-0002 A10; C12 — only these four in v1): wired here *(`tip_click` moved to S1.5b — ADR-0036)* `tip_click {amount?, from}` (payload per 04 §5.6 / ADR-0002 A16; nav Support button is a plain link, no event) and `sign_in {from}` (`GoogleSignInButton` predates `TrackedLink`) *(verified only at S1.9 — wired since S1.1 / ADR-0014, observed by T-E2E-16; no event code changed — ADR-0049 D18)*; already live `download {project, source, from}` (S1.2) and `video_play {youtube_id, kind}` (S1.6) (names per 01 INV-59, payloads per 04 §5.6); dashboard toggle happens in S1.10.
...
**Acceptance criteria**
1. S1.9.AC1 — Authorized `/api/cron/stats-snapshot` writes one row per (metric, source, entity) for today; running twice yields the same rows (upsert); `project_downloads` older than 90 days are purged (T-ACT-55 with seeded old rows); an uncommitted upload object older than 24 h is removed and a committed one kept (U1 — T-ACT-75); `users` metric is a single aggregate count (ADR-0002 #68); 401 without secret. *(As built — ADR-0049 D1–D8, D21, D29–D31: T-ACT-33 / 55 / 75 in `tests/db/routes/cron-stats-snapshot.test.ts` + `tests/db/jobs/snapshotStats.test.ts`; the second run rewrites the same keys with `updated_at` advanced; the 91-day row goes, the 89-day row stays; three aged orphans removed, seven aged referenced objects kept, `capped: false`; `users/odsens` = one head-count; the 401 writes nothing and calls nothing.)*
2. S1.9.AC2 — `/admin/stats` (role ≥ moderator) shows four tiles with numbers derived from `stats_daily` deltas (tips tile = `0`); the chart stacks three fixed source colours with swatch + word legend; phone shows 15 bars and says so. *(As built — ADR-0049 D10–D14, D22, D34: on the pristine seed `45` "First week counted." / `4.2K` "Modrinth 4.1K · CurseForge 120 · direct 7" / `0` / `0` "No data yet."; after a run `2` + "1 held" and `0` + "Ko-fi isn't counted yet."; fills proven equal to the computed `--emerald` / `--orange` / `--indigo-lift`; at 390 the compact figure with 15 columns and "15 bars, two days each", `scrollWidth` 390 — T-E2E-40.)*
3. S1.9.AC3 — Chart is hand-rolled SVG (no chart library in `package.json`), 0 radius, no gradients, Silkscreen 11 px axis labels. *(As built — ADR-0049 D12 / D27 / D28: `components/primitives/FlatBarChart.tsx`, a Server Component with no dependency; `rect` only, no `rx` / stroke / gradient / `<text>`; the axis labels are HTML spans at `--text-pixel-11` so 11 px holds at every width; T-E2E-40 asserts the zero counts, `tests/unit/stats.test.ts` pins the markup.)*
4. S1.9.AC4 *(moved to S1.5b.AC1 — ADR-0036)* — `/support`: one TIP ON KO-FI button and no amount picker of ours (ADR-0042 D1 — was "picker preselects $3, CONTINUE ON KO-FI"); the button swaps the gold slab for the `KofiPanelSlot` iframe in place for `site_settings.kofi_page` (from `site_settings_public`) — one box, never both, and "← Back" returns the slab (ADR-0042 D2) — no new tab (ADR-0002 C19); the "on Ko-fi ↗" ghost link opens the page; the Ko-fi iframe loads only on `/support` (a code rule, 01 INV-58 — the `frame-src` entry itself is site-wide, ADR-0041 D1) and only after the click; with `kofi_page` empty the button is disabled and the line "Tips open soon." shows (`--gold-ink` on the gold slab — ADR-0041 D5); editing `kofi_page` in Settings updates `/support` after `revalidateTag('settings')` (T-E2E-11).
...
8. S1.9.AC8 *(the `tip_click` clause moved to S1.5b.AC6 — ADR-0036)* — Custom events fire (`track` calls observed with `@vercel/analytics` stubbed): `download {project, source, from}` on a download button, `tip_click {amount?, from}` on the TIP ON KO-FI button (ADR-0042 — was "the picker button"; no emitter sends `amount` now), `video_play {youtube_id, kind}` on a facade click, `sign_in {from}` on Sign in; payload keys exactly as ADR-0002 C12 / 04 §5.6; nothing else is accepted by `trackEvent`; no PII in payloads (T-UNIT-38 + custom-events smoke inside T-E2E-16/31/6/49). *(Verified at S1.9, nothing built — ADR-0049 D18: `sign_in {from}` has fired from `GoogleSignInButton` since S1.1 and T-E2E-16 observes `{from:'nav'}`; T-E2E-31 `download`, T-E2E-6 `video_play`; T-UNIT-38's allowlist unchanged.)*
9. S1.9.AC9 — RLS: `stats_daily` admin-read, service-role write. *(As built — ADR-0049 D1 / D2: the `sync_runs` matrix — select D D D D A A, insert D D D D **D** A (no insert grant / policy for any JWT role), update / delete D D D D A A; T-RLS-107..110, 28 cases; the RPC `list_stale_objects` is `service_role`-only — T-RLS-129.)*
10. S1.9.AC10 *(the `/support` clause moved to S1.5b.AC6 — ADR-0036)* — axe zero serious/critical on `/support`, `/admin/stats` at 1280 + 390; chart has a text alternative (table or `aria-label` summary). *(As built — ADR-0049 D12 / D34: T-E2E-42 runs axe and shoots `admin-stats@1280.png` / `admin-stats@390.png`; T-E2E-40 runs axe at both widths and as moderator; the text alternative is the one visually-hidden `<table data-testid="flat-bar-chart-table">` (30 rows, Day · Modrinth · CurseForge · Direct · Total) and each `svg[role="img"]` is named by the page's h2 through `aria-labelledby`.)*
...
**Tests required:** 05 §8 row S1.9 — T-RLS-107..110; T-ACT-33 (stats route), 55 (snapshot idempotency/purge), 75 (orphan cleanup); T-UNIT-38 (`trackEvent` allowlist), 42 (`lib/stats.ts` bucketing); T-E2E-40 *(T-E2E-11 and 49 moved to S1.5b — ADR-0036)*; custom-events smoke inside T-E2E-16/31/6 (`window.va` stub sees `sign_in`, `download`, `video_play`; `tip_click` is S1.5b's); seed SEED-12. Legs added at Session A on existing ids (ADR-0049 D35, 05 H-13): T-RLS-129 (`list_stale_objects`), T-ACT-42 (`stats` runs the job), T-ACT-45 / 70 / 74 (`snapshotStats` legs), T-E2E-42 (`/admin/stats` at 1280 + 390), T-E2E-48 (the three `FlatBarChart` specimens); files `tests/db/rls/stats_daily.test.ts`, `tests/db/jobs/snapshotStats.test.ts`, `tests/db/routes/cron-stats-snapshot.test.ts`, `tests/unit/stats.test.ts`, the T-E2E-40 describe + T-E2E-42 leg in `tests/e2e/admin/projects.spec.ts`; `scripts/check-test-ids.mjs` `SHIPPED_SLICES` gains `S1.9` from this PR.
...
**Demo script**
1. Hit `/api/cron/stats-snapshot` twice → `/admin/stats` shows tiles + chart. *(As built — ADR-0049 D22: `curl -H "Authorization: Bearer $CRON_SECRET" …` twice → both 200 with equal `items` (35 on the seed); the page then reads `2` + "1 held" on the comments tile and the `stats` SYNC row LIVE; on production the first rows arrive with the 03:00 UTC cron or a "Sync now" on the `stats` row.)*
2. Resize to 390 → 15 bars + label. *(As built: the compact figure with "15 bars, two days each" under it, the full figure gone, no horizontal scroll — ADR-0049 D12.)*
...
**Risks / unknowns:** *(Ko-fi items moved to S1.5b — ADR-0036; the page name `odsens` is set since 2026-09-11)* Ko-fi account/page not yet created (setup to-do; `KOFI_PAGE=oddsense` unconfirmed) — build against a test page name and record; Ko-fi preset-amount behaviour unverified (`docs/design-review.md` #13; verify the amount param once the account exists — ADR-0002 #50) *(verified 2026-09-18: no such parameter — ADR-0042 D1)*; first days of `stats_daily` have no deltas (tiles show `0` + "No data yet.", ADR-0002 #29). *(S1.9 as built — ADR-0049: the 7-day tile reads "First week counted." until a second week of snapshots exists and every tile reads "No data yet." again after 38 days without a run — the SYNC board's `stats` row is the alarm (D11); the seed's `stats_daily` pair is dated at reset time, so a local stack reset before UTC midnight and read after it shows it one day older — tolerated by the windows (D26); the first production run at 03:00 UTC is the first real `channels.list` unit against the corrected `YOUTUBE_CHANNEL_ID` (D20); the Storage `remove()` failure arm of the sweep has no test hook yet (D32).)*
...
**Scope IN**
- Supabase Branching + Vercel integration verified end-to-end (the persistent `staging` branch serves every preview — ADR-0010; migrations promote on merge to `main`; production DB has all migrations; `supabase/config.toml` remote `site_url`/redirects correct).
...
- Vercel: Deployment Protection **off for production** only (previews stay Standard); Web Analytics enabled in the dashboard + `<Analytics />` mounted **since fix pass 3 (ADR-0038 D4)** — S1.10 verifies it and adds Speed Insights; `vercel.json` cron list = 6 v1 routes verified in the dashboard; env var names complete for production (`.env.example` parity incl. `CURSEFORGE_API_KEY`, `RESEND_API_KEY`, `DISCORD_WEBHOOK_URL`); production `NEXT_PUBLIC_SITE_URL=https://odsens.com`.
...
8. S1.10.AC8 — Supabase: production project has every migration applied (`supabase migration list` matches repo); a test PR after launch deploys a preview that runs against the persistent `staging` branch, whose migrations match the PR branch after the push-to-staging step (ADR-0010; wording amended from "spins a preview branch automatically").
...
| 00-O-5 | Admin UI for hiding a video (no `/admin/videos` route in the registry). | Videos list on the `/admin` dashboard calling `updateVideo`; no new route. (Built S1.6: Hidden + Short toggles, "Auto", "Saved." — ADR-0043 D11.) | ADR-0002 #20 | DECIDED (ADR-0002 #20) |
...
| 00-O-14 | Skin bust render location (serverless WebGL vs. client fallback). | ~~`skinview3d` in headless WebGL via `gl` (native dep → dependency ADR at S1.7)~~ → a pure-TypeScript software rasterizer (`lib/skins/render.ts`, `sharp` only; no `gl`, no native dependency) running inside the action's function; fallback = the live `SkinViewer3D variant="bust"` in the card while `render_bust_path` is null (ADR-0047). | ADR-0002 C22; ADR-0047 | DECIDED (ADR-0002 C22) as amended by ADR-0047 (2026-09-25) |
...
| 2026-08-17 | v0.4 | ADR-0002 Amendment A | A1 `CommentThread` one client file (no `CommentList`); A2 `moderator_thread` RPC noted; A5 T-RLS-126/T-ACT-69 → S1.4; A7 `ProjectGrid` client island (S1.2); A8 `sync.*` emission starts S1.5 (S1.2 logs only); A9 robots S0 (T-E2E-45a) / sitemap S1.2 (T-E2E-45b); A10 `TrackedLink` + `download` event from S1.2, `video_play` S1.6, `tip_click`/`sign_in` S1.9; A11 feature/hide/reorder on `/admin/projects` + `curateProject` batch `reorder`, extras on `[id]`; A13 S0 CI `build` uploads `build-output.txt`; A14 `HASH_SECRET` from S1.1, `/auth/callback` sets `email_hash` (trigger does not); A15 `lib/auth.ts` = `getViewer/requireRole/safeNext`, no `getSession`; A17 auth routes under `app/auth/**`, S1.7 depends S1.1+S1.3; A18 `InlineConfirm` first use S1.1; §7 notes marked resolved. |
...
| 2026-08-17 | v1.0 | ADR-0006 | ADR-0006 — S0 preview env fallback (placeholders; Branching required before S1.1): S0 Scope IN "Supabase Branching enabled + integrations installed" and S0.AC12 stand as written; at S0 the Vercel preview carries the production `NEXT_PUBLIC_SUPABASE_URL` (public) + placeholder anon/service-role values (not the production values named as the S0 Risks fallback); Branching + GitHub/Vercel integrations are a David-side prerequisite before S1.1; preview `NEXT_PUBLIC_SITE_URL` set per branch by `ship` until S1.1 decides. |
| 2026-08-20 | v1.0 | ADR-0009 | ADR-0009 — middleware file is `proxy.ts` (Next 16 convention): the S1.1 middleware lives in `proxy.ts` at the repo root with the named export `proxy` + `config.matcher`; 02 §3 M1–M8, RP-19/RP-20 and 01 INV-30/32 apply unchanged; every `middleware.ts` in this doc (S1.1 Scope IN, gate focus) reads as `proxy.ts`. Addendum 2026-08-21 (David's preview review): proxy redirects (M1/M5/M6) apply to GET/HEAD only — a Server Action POST passes through after the refresh (new 02 §3 row M3b; 01 INV-30; 05 T-ACT-10); a 307 on an action POST made the browser re-POST the action to `/` ("Something broke" on a second DONE). |
| 2026-08-20 | v1.0 | ADR-0010 | ADR-0010 — preview env from the persistent `staging` Supabase branch (supersedes ADR-0006): Branching + the Supabase GitHub/Vercel integrations are live (David, 2026-08-20); every preview runs on the persistent `staging` branch (git branch `staging`; automatic per-PR branching off); the Vercel Preview env carries staging's keys under `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` / `SUPABASE_SECRET_KEY`, accepted by `lib/env.ts` as aliases for the canonical names; `ship` pushes each PR branch to `staging` (fast-forward, never force) before the preview is reviewed; preview `NEXT_PUBLIC_SITE_URL` is derived from `VERCEL_BRANCH_URL` (no per-branch `ship` step); the S0 placeholder values are removed by `vercel-ops`. |
| 2026-08-20 | v1.0 | ADR-0011 | ADR-0011 — OAuth redirect allow-list narrowed to `https://odsens-git-*-studiobing.vercel.app/**` (S1.1 Risks sentence rewritten; bare `*.vercel.app` removed from `[remotes.production.auth]`; the base `[auth]` block, inherited by Supabase preview branches, gains the same pattern). |
| 2026-08-20 | v1.0 | ADR-0012 | ADR-0012 — `HASH_SECRET` boot-required from S1.1: the boot-required env set is 9 names (S0.AC5's 8 + `HASH_SECRET`, `z.string().min(32)`); 04 SC-16 / 05 T-UNIT-16 / `_registry.md` Env aligned with 01 INV-36. |
| 2026-08-20 | v1.0 | ADR-0014 | ADR-0014 — profile page island and own-row columns: `ProfilePanel` (`components/accounts/ProfilePanel.tsx`, C) is the `/profile` client island (03 C-16a row, `_registry.md` Accounts, 03 §2.5 / §3); `lib/auth.ts` `getProfile()` own-row select adds `handle_changed_at` (server-side only; `ViewerProvider` unchanged — 01 INV-45, 04 SC-04); `AdminShell.viewer.avatarUrl?`; `GoogleSignInButton` `data-variant="outlined"` / `"chalk"` + Google-mark brand fills inline (03 C-08 exception); additive props in 03 C-03; `safeNext('/admin')` → `/` stands (D4, `docs/questions.md`); 05 §1.1 harness note on `playwright.config.ts`. Addendum 2026-08-20 (gate round 1, frontend #9): `ViewerProvider` publishes viewer state through a module-level external store read by `useSyncExternalStore` (`useViewer()` shape unchanged) instead of a React context, so a still-dehydrated `loading.tsx` boundary is never marked and `/profile` hydrates in place (03 §2.5 `ViewerProvider` row, C-01 amended). |
| 2026-08-20 | v1.0 | ADR-0015 | ADR-0015 — `profiles` mutations on other rows via the service client: Postgres applies the own-row select policy to an admin JWT's UPDATE/DELETE of another user's row (0 rows), so 05 T-RLS-8/9 admin = D (svc = A) and T-RLS-5 admin = own row; schema as built (`supabase/migrations/20260820120000_profiles.sql`); admin/mod writes to other users' profiles (`banUser`, `renameUserHandle`, `setUserRole`) run through the service client after `requireRole` (04 SC-06); `docs/data-model.md` §4 `profiles` amended. Addendum 2026-08-20 (gate round 1, security #16): `profiles.avatar_path` is constrained to the owner's folder (DB CHECK `profiles_avatar_path_own`, migration `20260820120400_profiles_avatar_path_check.sql`) and every service-role Storage delete re-checks ownership (`lib/files.ts` `isOwnAvatarPath`; `updateProfile` / `deleteAccount` delete only objects under `avatars/{caller_id}/`); the `site_settings` singleton is created by migration `20260820120500_site_settings_default_row.sql`, not only by `seed.sql` (data-model §2.1 / §2.4 / §2.11, 04 §1.1, 01 INV-53 / INV-97, 05 SEED-1 amended). |
| 2026-08-20 | v1.0 | ADR-0016 | ADR-0016 — `/auth/callback` writes `email_hash` with the service client (04 §2.1 A3, ADR-0002 A14): `app/auth/callback/route.ts` is the single non-API `app/**` file allowed to import `@/lib/supabase/admin` (01 INV-12/INV-14/INV-84; its own `eslint.config.mjs` block); the INV-45/INV-46 greps exclude that one file; the address is never logged or returned. |
| 2026-08-21 | v1.0 | ADR-0018 | ADR-0018 — profile menu items: `ProfileMenu` lists Your profile (`/profile`) · Admin (role ≥ moderator, 03 N-06) · Sign out; the "Change handle" → `/profile#handle` and "Change picture" → `/profile#picture` items are removed (David, S1.1 preview review 2026-08-21 — all three opened the same page; DESIGN.md v1.4 §11.1, 03 §2.5, 02 RP-12 + `/profile` row); `/profile` keeps its `#handle` / `#picture` anchors; S1.1 demo-script step 4 and S1.1.AC6 stand as written. |
| 2026-08-21 | v1.0 | ADR-0017 | ADR-0017 — onboarding panel: no Skip button (leaving the picture empty and pressing DONE is the skip); DONE gated on the handle only; success leaves with a document navigation `window.location.assign(next)` instead of `router.replace` (the router's prefetch cache held `/` as the proxy's "307 → /welcome" answer, so the soft navigation landed back on `/welcome`); the onboarding wordmark link has `prefetch={false}` (DESIGN.md §11.3 #10, 03 §2.5, 02 §2.4, 05 T-E2E-22 amended). |
| 2026-08-21 | v1.0 | ADR-0019 | ADR-0019 — banned accounts land on `/banned` (David, S1.1 preview review 2026-08-21: "a banned user's Google login directs them to a banned page with no functionality"): new route `/banned` in the `(onboarding)` shell — the #18 admin-gate slab, "YOU'RE BANNED", one line; the wordmark and Sign out are the only controls (DESIGN.md §11.3 #19); `proxy.ts` M4 reads `handle, is_banned` and the new M4b 307s every navigation of a banned session to `/banned` except `/banned`, `/auth/*`, `/api/*` (before M5, so a banned null-handle account never reaches `/welcome`); `lib/auth.ts` `requireUser()` / `requireOnboarded()` throw `banned`, so `checkHandle`, `completeOnboarding`, `updateProfile`, `deleteAccount` all refuse a banned caller (04 SC-05 = every user action; 04 §1.1 "banned is not checked" withdrawn); 05 T-ACT-1/4/7/10/65 banned cells + the T-E2E-32 banned flow. |
| 2026-08-21 | v1.0 | ADR-0020 | ADR-0020 — reserved handles + bans bind the owner's direct `profiles` write (security gate round 3: row 19 must-fix, advisory row 20): the H3 list was enforced only by the `check_handle` RPC / the actions, so a PostgREST PATCH with the anon key + own JWT could set a FIRST handle to `oddsense`, `admin`, `moderator`, … (owner/staff impersonation; the unique index would then also block the owner-bootstrap SQL), and `profiles_guard` was ban-unaware. Migration `20260821090000_profiles_guard_reserved_and_banned.sql`: new pure helper `is_reserved_handle(text)` (the 22-entry list in SQL once), `check_handle` calls it, `profiles_guard` refuses a reserved first handle and every own-row write by a banned account (42501) for non-admin JWTs — no-JWT, service and admin sessions pass, so the owner bootstrap is unchanged; `lib/supabase/types.ts` regenerated. 05 T-RLS-4 banned = D, T-RLS-5 reserved / banned = D and svc A incl. a reserved first handle; T-ACT-7 parity reads `is_reserved_handle`; T-RLS-129 grant row; data-model §2.1 / §2.11 / §4, 04 H3 / H5, 01 INV-49 / INV-97, `_registry.md` SQL line, `supabase-ops` Owner bootstrap amended. |
| 2026-08-27 | v1.0 | ADR-0021 | ADR-0021 — banned accounts may delete themselves (David's S1.1 merge decision, built as the follow-up PR `fix/S1.1-banned-delete`): `requireOnboarded({allowBanned:true})` — the one SC-05 exception, passed only by `deleteAccount`; `BannedDelete` (C) Delete account control on `/banned` (onboarded banned only; document navigation out — ADR-0017 lesson); no schema change (the ADR-0020 guard binds UPDATEs; the delete path is service-role). 05 T-ACT-65 banned = A (NULL handle = D `onboarding_required`), T-E2E-32 delete leg; 04 SC-05 / §1.0 / §1.1, 02 §1.2, DESIGN.md v1.5 §11.3 #19, registry Accounts row. |
| 2026-08-27 | v1.0 | ADR-0022 | ADR-0022 — `project_is_visible(uuid)` (security definer): the data-model §4 visibility predicate as one SQL helper called by every S1.2 SELECT policy — mutually-referencing inline policies recurse (42P17); `projects_public` inlines the same predicate; grants anon/authenticated/service_role. data-model §2.11 + §4, `_registry.md` SQL line amended. Migration `20260827090000_projects.sql`. |
| 2026-08-27 | v1.0 | ADR-0023 | ADR-0023 — CI `build` job starts local Supabase (`supabase/setup-cli@v1` → `supabase start` before `pnpm build`, stop after): from S1.2 the build reads the DB (`generateStaticParams` 02 §2.3 + INV-38 ISR prerender); the frozen DB-less CI-4 job cannot build the app truthfully. 05 §4 CI-4 row amended; workflow `build` job mirrors the proven `e2e` pattern. |
| 2026-08-27 | v1.0 | ADR-0024 | ADR-0024 — page-scoped `'use server'` glue on dynamic `app/admin/**` routes (module-private, input mapping + exactly one `lib/actions` call + optional `redirect()`; no own auth/zod/DB/fetch): the Next PRG/callback idiom the S1.2 admin pages use; moving glue into `lib/actions` would violate SC-02/SC-03 instead. 01 INV-04 + 04 SC-01 amended (two-set grep target). |
| 2026-08-27 | v1.0 | ADR-0025 | ADR-0025 (interim) — unknown ISR slug routes answer the 404 shell body with status ∈ {200, 404} while Next #45801/#76474 stands (streamed shell commits 200 before `notFound()` under an RP-10 `loading.tsx`); accept-and-monitor, `app/not-found.tsx` gains `robots: noindex`; static unmatched routes still 404. 02 SM-04 + 05 T-E2E-14/T-E2E-34 amended; questions.md OPEN item decided; revert when upstream fixes. |
| 2026-08-27 | v1.0 | ADR-0026 | ADR-0026 — version identity split (the S1.2 real-catalogue finding): `project_versions` unique moves from `(project_id, version_number)` to `external_id` (plain unique — the 04 §3.1 idempotency key becomes a DB constraint; Modrinth allows duplicate `version_number`s per project) plus a partial unique `(project_id, version_number) where external_id is null` for exclusives (`uploadProjectFile` find-or-create); `syncModrinth` matches by `external_id` only (the ping-ponging `version_number` fallback removed). Migration `20260827200000_project_versions_identity.sql`; data-model §2.2 + 05 §12 note (T-ACT-48 duplicate case) amended. |
| 2026-08-27 | v1.0 | ADR-0027 | ADR-0027 — S1.3 contract clarifications (verification-sweep findings): `/admin/projects/new` = create-only metadata form, uploads/preview on `[id]` after the redirect (02 §1.3 row + this doc's demo step 1 amended — the SC-21 paths need a `project_id`); `/api/download/**` sends `Referrer-Policy: no-referrer` per 04 D6 (01 INV-76 carve-out; scoped `next.config.ts` rule); 04 §1.4 update accepts `null` to clear `license`/links, create's `body_md`/arrays optional-with-empty-defaults; §1.4.5 commit buffers (≤ caps, in-memory) and media U3 idempotency is content-based; the `Downloadable` counter-payload/URL-kind generalisation for kind `skin` deferred to S1.7; `UploadWell` `disabled`/`disabledTitle` additive props (C-03 precedent); `ProjectFileWell` registered. Related 00 §S1.3 wording left as-is and read through this row: AC7/Scope-IN "(gold)" for the GetItPanel primary — 03 owns the variant (primary; gold reserved for hero/support, ADR-0002); Scope-IN "config.toml raised in this PR" — the limit has been `100MiB` since S1.1. |
| 2026-09-03 | v1.0 | ADR-0028 | ADR-0028 — S1.4 contract clarifications: D1 a deleted root keeps its "Deleted." slot only while it has visible replies (00 AC8 amended to 05 T-E2E-3 / 03 §2.2 / DESIGN §11.2); D2 `reportComment` accepts reports on any non-deleted comment of a visible target, auto-hold only for published non-mod rows (04 §1.2 precondition row amended to §5.1 M8); D3 `profiles_guard()` + `comments_guard()` pass nested-trigger writes (`pg_trigger_depth() > 1`) so the `comment_count` / `like_count` triggers work under a JWT session (migration `20260903090400_profiles_guard_nested_triggers.sql`); D4 schema names — helper `comment_target_visible(text, uuid)`, trigger fns `comments_guard()`, `comments_bump_comment_count()`, `comment_likes_count()`; `comments_public` = the ten T-RLS-128 columns; `comments.author_id` nullable `on delete set null`; D5 `commentBodySchema` lives in `lib/actions/comments.schema.ts` over the zod-free `validateBody` (ADR-0008 D3); D6 `ModActionRow surface?: 'thread' \| 'admin'` (additive); D7 `/admin/comments` needs no moderator-read widening. |
| 2026-09-03 | v1.0 | ADR-0029 | ADR-0029 — builds wait for the database schema: `pnpm build` = `node scripts/wait-for-schema.mjs && next build`; the script polls PostgREST (OpenAPI `definitions` + RPC `migration_versions()`) until every repo migration is applied and every created relation is in the schema cache (20 min deadline on Vercel, 60 s elsewhere; `SKIP_SCHEMA_WAIT=1` bypass) — ends the S1.2/S1.3/S1.4 merge race that needed a manual `vercel redeploy`; the redeploy stays the documented fallback. |
...
| 2026-09-25 | v1.0 | ADR-0045 | S1.8 Session A build findings (Proposed): §S1.8 Scope IN glosses (Table, Adapters, Actions, Job, Public, `MentionCard`, Admin bullets), AC1 / AC4 / AC5 / AC6 / AC7 / AC8 / AC12 glosses, Tests required (the legs added at Session A), Demo step 6, Risks. No AC removed, weakened or moved. |
| 2026-09-25 | v1.0 | ADR-0047 | S1.7 bust renderer (Proposed, `Kind: dependency`): `gl` is NOT added — the renderer is a pure-TypeScript software rasterizer in `lib/skins/render.ts` (`sharp` only), the fallback is the live bust variant of `SkinViewer3D`; §S1.7 Scope IN Job bullet, Risks, §5 00-O-14 → DECIDED as amended. |
| 2026-09-25 | v1.0 | ADR-0048 | S1.7 Session A build findings (Proposed, D1–D26): the `Downloadable` union (closes ADR-0027 D5), the owner-scoped `*_path` CHECKs (closes the S1.2 rule), the schema / RPCs / actions / job / script / readers as built, the five new islands, `UploadWell`'s two modes, `/skins` + `/art` + the admin pages as built, seeds pinned, the software-GL e2e flags — §S1.7 dated note, Scope IN glosses, AC1–AC8 / AC10 / AC11 glosses, Tests required, Risks. No AC removed, weakened or moved. |
| 2026-09-26 | v1.0 | ADR-0049 | S1.9 Session A build findings (Proposed, D1–D37): `stats_daily` as built (`entity_id not null` + the sentinel, text CHECKs, the `sync_runs` RLS matrix), the read-only RPC `list_stale_objects` for the U1 sweep, SEED-12 pinned (4099 / 120 / 7 · 4059 / 117 / 5), `snapshotStats` gather → write → housekeeping with the `ok` rule and the D7 summary, `direct_downloads_day` dated day − 1, the route + `vercel.json` row, `triggerSync` runs `stats`, `lib/stats.ts` deltas / windows / tiles, `FlatBarChart` both variants CSS-toggled with HTML axis labels and one hidden table (9-unit bars), `/admin/stats` for admin and moderator (`Stats · Admin`, the mute line, SYNC over five sources), `sign_in` verified only, the test legs — §S1.9 dated note, Scope IN glosses, AC1 / AC2 / AC3 / AC8 / AC9 / AC10 glosses, Tests required, Demo, Risks. No AC removed, weakened or moved. |
...
| Finding | Resolution |
|---|---|
| S1.1 "Tests required" listed T-UNIT-27 as `27 (may land here or S1.5)` (CI fix round 1, 2026-08-20). | Mirrored 05 §8 (05 owns IDs, §0.5): T-UNIT-27 dropped from the S1.1 list, stays in S1.5 — `lib/notify/matrix.ts` / `matrixDefaults` lands with `notification_matrix`; `scripts/check-test-ids.mjs` would otherwise require it now (05 H-12). Prose clarification, no rule change (§1.5 F-2). |
