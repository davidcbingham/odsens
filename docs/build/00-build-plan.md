...
- Components: `ViewerProvider` (+ `useViewer()`, `components/accounts/`, client — reads only the viewer's own `profiles` row under RLS via `lib/supabase/client.ts`; ADR-0002 C1), `HandleField` (all §11.1 states), `AvatarUpload` (+ crop, §11.1 "Picture upload"), `ProfileMenu` (§11.1; "Admin" item for role ≥ moderator per 03 N-06), `OnboardingPanel`, `SignInPrompt` (§5, reused by S1.4), `GoogleSignInButton` (client), `AdminGate` (anon only), `AdminShell` (+ `AdminNav`; sidebar per 02 RP-14; Orders item hidden until S2.2; Settings item admin only, live from S1.5), `InlineConfirm`. `Table` (S1.2), `Field` (S1.3), `Toggle` (S1.4) arrive with their first use — none is needed here.
...
**Tests required:** 05 §8 row S1.1 — T-RLS-1..15, 115..116, 125..127, 129 (`check_handle` grant + `rate_limit_ok`), 130 (`rate_limit_hits`), 132 (`site_settings_public`); T-ACT-0..10, 65 (`deleteAccount`), 69 (audit line, for actions present); T-UNIT-1, 2, 17 (png/jpg/webp), 18 (`avatar`), 27 (may land here or S1.5), 37 (scopes present so far); T-E2E-12, 13, 16, 21, 22, 23, 32, 33 (gate; sidebar can be a stub), 46; seed SEED-3.
...
- Admin: `/admin/projects` (`Table` of all projects incl. hidden, empty copy per 02 §1.3 — ADR-0002 #40; feature/hide toggles + `ReorderableList` reorder; `SyncStatus` = `Table`+`StatusPill`+`Button`+`SourceSwatch` from `sync_runs` — ADR-0002 #56; `StatusPill` fills per ADR-0002 #47; "Sync now" → `triggerSync`), `/admin/projects/[id]` **curate view for synced projects** (02 §1.3: overrides form — feature/order/hidden/title + description override/notes/comments toggle, CurseForge id field → `setProjectLink`); S1.3 extends this route with the exclusive edit form and gallery upload.
...
**Tests required:** 05 §8 row S1.2 — T-RLS-16..43, 111..114; T-ACT-33 (modrinth + curseforge routes), 40, 41, 42, 45..52, 70 (lock), 71 (curseforge no-key); T-ADP-1..8 (T-ADP-2 = mapping edge cases: datapack loader, paper/spigot/bukkit/purpur/folia/velocity/bungeecord → plugin, resourcepack, default mod; run-twice idempotency; 429 retry + UA), 20 (no `process.env` in adapters); T-UNIT-10, 11, 13, 14, 20, 21, 30, 31, 32, 39 (`groupGameVersions`); T-E2E-1 (hero + featured), 2, 3 (except comments/SEEN ON), 5 (gallery/lightbox part), 18, 34 (curate part), 41; seed SEED-4..6, SEED-12. (T-ACT-38/73 belong to S1.3 — ADR-0002 C10.)
...
- Route handler: `/api/download/[fileId]` per 04 §2.3 D1..D8 — **GET only** (HEAD/POST/others → 405; ADR-0002 C17) — verify project published + file has `storage_path` → rate limit 30 / min / `ip_hash` via `assertRateLimit` → `rate_limit_ok`, counted on `project_downloads.ip_hash` (04 D3; `rate_limit_hits` is for skin downloads only) (429 with JSON `{ok:false,error:{code:'rate_limited',message}}` + `Retry-After: 60` — ADR-0002 C14) → increment `project_files.download_count` + `projects.downloads_direct` + insert `project_downloads` in one RPC → 302 to a 60 s signed URL with `Content-Disposition: attachment`; unpublished/missing/synced → 404. Scope resolver `lib/files.ts resolveDownloadable` (04 D2/D8) is generic over bucket + owner for S2.3.
...
**Engineering docs implemented:** 01 §11 (uploads, INV-51 two-phase baseline), §12 (downloads), §17 (rate limiting); 02 §1.3 (`/admin/projects/new`, `/admin/projects/[id]` exclusive edit), §1.4 + §2.9 (`/api/download/[fileId]`), §8 row S1.3; 03 §2.2 (`ExclusiveBadge`, `StatusPill`, `Field`, `Select`), §2.10 (`UploadWell`); 04 §0 SC-17..SC-21, §1.4 (`createExclusiveProject`, `updateExclusiveProject`, `publishProject`), §1.4.5, `uploadProjectMedia`, `uploadProjectFile`, §2.3 (D1..D8), SC-16/SC-17 (`HASH_SECRET`), §2.3 D4 (`record_download`), §10 OPEN-10 (storage limit); 05 §8 row S1.3.
...
11. S1.9.AC11 — `FloatingSupportButton` e2e (T-E2E-49 — to be aligned to DESIGN.md §5 / 04 §5.6 `from:'floating'`; see §7 Review notes): present on `/` and a project detail, absent on `/support`, `/welcome`, `/admin/*`; hides on scroll-down and returns on scroll-up; 52 px square at 390; `TipPanel` on detail and the Home compact panel link to `/support` (always rendered; only `/support` reacts to an empty `kofi_page`, 04 §5.7).

**Tests required:** 05 §8 row S1.9 — T-RLS-107..110; T-ACT-33 (stats route), 55 (snapshot idempotency/purge), 75 (orphan cleanup); T-UNIT-38 (`trackEvent` allowlist), 42 (`lib/stats.ts` bucketing); T-E2E-11, 40, 49 (`FloatingSupportButton`/`TipPanel`); custom-events smoke inside T-E2E-16/31 (`window.va` stub sees `sign_in`, `download`, `tip_click`, `video_play`); seed SEED-12.
...
| `HASH_SECRET` timing vs the S1.1 `auth.users` trigger — v0.3 claimed `HASH_SECRET` exists from S1.1. | Claim removed: ADR-0002 C13, `_registry.md` Env, 04 SC-16 and 05 §8 all place `HASH_SECRET` at **S1.3**. **Genuine gap, not settled here:** ADR-0002 C13 defines `email_hash = HMAC-SHA256(HASH_SECRET, lower(email))` computed by the S1.1 Postgres trigger `handle_new_user()`, but a Postgres trigger cannot read a Vercel env secret, and 01 INV-50 / 05 T-RLS-125 still say plain `sha256(lower(trim(email)))` while data-model §2.1 / 04 SC-17 say HMAC. Needs an ADR before S1.1 (options: move `HASH_SECRET` into the S0 boot set and pass it to Postgres as a DB setting/vault secret; or keep the trigger at plain sha256 for `email_hash` and HMAC only `ip/ua`), aligning 01/04/05/data-model in the same PR. |
| S1.9.AC11 cites T-E2E-49 for FSB scroll hide/return, `href` `/support`, 52 px phone square. | 05's T-E2E-49 currently asserts a session dismiss control, FSB click → `TipPanel` + `AmountPicker`, `from:'fsb'` and a compact bar at 600–899 — several of which contradict DESIGN.md §5 and 04 §5.6 (`from ∈ 'support'|'tip-panel'|'floating'`). 00 keeps DESIGN.md's behaviour and cites T-E2E-49 as "to be aligned"; 05 must be amended (or a new 05 ID added) in the reconciling PR — not settled here. |
